// Project Mimikyu — supabase/functions/justtcg-price-refresh-set/pricing-source-lookup.ts
// Resolução do pricing_source_id da fonte JUSTTCG — cópia literal de
// supabase/functions/justtcg-price-refresh/pricing-source-lookup.ts (mesmo motivo de
...
export async function resolveJusttcgPricingSourceId(
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error || !data) {
    logError("JUSTTCG_PRICE_REFRESH_SET_SOURCE_LOOKUP_FAILED", {
