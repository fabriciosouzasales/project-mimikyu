...
Query.......: 860B - Seed Card Variant ME2
Versão......: 1.0
Status......: CANÔNICA
...
Descrição resumida:
Cadastra e atualiza explicitamente as 214 variantes editoriais oficiais das
130 Cards da coleção ME2 - Fogo Fantasmagórico.

Abordagem:
A matriz editorial explícita é armazenada em uma variável JSONB local ao bloco.
Nenhuma tabela temporária ou auxiliar é criada. Nenhuma variante é inferida
durante a execução a partir da raridade da Card.
...
NOTA DE DOCUMENTAÇÃO: segunda Query de carga real de card_variant, mesma
arquitetura homologada em 860A (matriz JSONB embutida, sem tabelas
temporárias, validação em 9 passos, rollback automático). Confirmada por
Fabrício como executada com sucesso ("Show! Resultado esperado após
execução. Vamos em frente."), com o resultado batendo exatamente com a
distribuição documentada (74/56/84/214).
===============================================================================
*/
...
BEGIN
    ...
    IF v_game_id IS NULL THEN
        RAISE EXCEPTION
            'Não foi possível executar a Query 860B: o Game POKEMON não está cadastrado.';
    END IF;
    ...
    IF v_card_set_id IS NULL THEN
        RAISE EXCEPTION
            'Não foi possível executar a Query 860B: o Card Set ME2 do Game POKEMON não está cadastrado.';
    END IF;

    IF v_base_set_size <> 94 OR v_total_set_size <> 130 THEN
        RAISE EXCEPTION
            'Não foi possível executar a Query 860B: ME2 possui base_set_size % e total_set_size %, mas os valores esperados são 94 e 130.',
            v_base_set_size,
            v_total_set_size;
    END IF;
    ...
    IF v_card_count <> 130 THEN
        RAISE EXCEPTION
            'Não foi possível executar a Query 860B: ME2 possui % Cards, mas o esperado é 130.',
            v_card_count;
    END IF;
    ...
    IF v_variant_type_count <> 3 THEN
        RAISE EXCEPTION
            'Não foi possível executar a Query 860B: os tipos STANDARD, HOLO e REVERSE_HOLO devem estar cadastrados para POKEMON.';
    END IF;
    ...
    IF v_matrix_count <> 214 THEN
        RAISE EXCEPTION
            'A matriz editorial de ME2 possui % linhas, mas o esperado é 214.',
            v_matrix_count;
    END IF;
    ...
    IF v_distinct_card_count <> 130 THEN
        RAISE EXCEPTION
            'A matriz editorial de ME2 referencia % Cards distintas, mas o esperado é 130.',
    ...
    IF v_default_error_count <> 0 THEN
        RAISE EXCEPTION
            'A matriz editorial de ME2 contém % Cards sem exatamente uma variante padrão.',
    ...
    LOOP
        ...
        IF v_card_id IS NULL THEN
            RAISE EXCEPTION
                'A Query 860B foi interrompida: a Card % não foi encontrada em ME2.',
                v_collector_number;
        END IF;
        ...
        IF v_variant_type_id IS NULL THEN
            RAISE EXCEPTION
                'A Query 860B foi interrompida: o Card Variant Type % não foi encontrado.',
                v_variant_type_code;
        END IF;

        IF v_variant_order <= 0 THEN
            RAISE EXCEPTION
                'A Query 860B foi interrompida: variant_order inválido para a Card %.',
                v_collector_number;
        END IF;
    END LOOP;
    ...
    IF v_registered_count <> 214 THEN
        RAISE EXCEPTION
            'A Query 860B foi interrompida: ME2 possui % Card Variants, mas deveria possuir exatamente 214.',
            v_registered_count;
    END IF;

    IF v_standard_count <> 74
       OR v_holo_count <> 56
       OR v_reverse_count <> 84 THEN
        RAISE EXCEPTION
            'Distribuição incorreta em ME2. STANDARD: %/74; HOLO: %/56; REVERSE_HOLO: %/84.',
            v_standard_count,
            v_holo_count,
            v_reverse_count;
    END IF;
    ...
    IF v_additional_count <> 0 THEN
        RAISE EXCEPTION
            'A Query 860B foi interrompida: ME2 possui % variantes adicionais fora da matriz canônica.',
            v_additional_count;
    END IF;
    ...
    IF v_divergent_count <> 0 THEN
        RAISE EXCEPTION
            'A Query 860B foi interrompida: % variantes divergem da matriz canônica.',
            v_divergent_count;
    END IF;
    ...
    IF v_invalid_default_count <> 0 THEN
        RAISE EXCEPTION
            'A Query 860B foi interrompida: % Cards de ME2 não possuem exatamente uma variante padrão.',
            v_invalid_default_count;
    END IF;

    RAISE NOTICE
        'Query 860B concluída: 214 variantes cadastradas em ME2 — 74 STANDARD, 56 HOLO e 84 REVERSE_HOLO.';
END;
$$;
