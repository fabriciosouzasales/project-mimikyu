...
SOURCE_SET_SCOPED, com propagação na MESMA transação.

Os dois caminhos públicos (Query 2195) delegam aqui. Não existe
segunda implementação da regra.

--------------------------------------------------------------
POR QUE UM WORKER NOVO, E NÃO ESTENDER O QUE EXISTE
--------------------------------------------------------------
Auditoria de 2026-09-14 sobre as três funções que hoje fazem esta
classe de operação encontrou contratos DIVERGENTES:

  |                          | resolve_ | resolve_   | create_profile |
  |                          | mapping  | printing_  | _with_backfill |
  |                          | (2150)   | mapping    | (2190)         |
  |--------------------------|----------|------------|----------------|
  | filtra STAGED            |   sim    |    sim     |      sim       |
  | filtra decision=PENDING  |   sim    |    sim     |      sim       |
  | aceita VALID + NR        |   NAO    |    sim     |      NAO       |
  | filtra persistence       |   NAO    |    NAO     |      sim       |
  | checa resulting_variant  |   NAO    |    NAO     |      NAO       |
  | recalcula counters       |   NAO    |    NAO     |      sim       |
  | FOR UPDATE               |   NAO    |    sim     |      sim       |

NENHUMA das três checa `resulting_variant_id`. Duas de três não
filtram `persistence_status`. Duas de três não recalculam counters.

O padrão correto já existe — espalhado. Este worker CONVERGE o que
está certo em cada uma e acrescenta o único guard que falta em
todas (`resulting_variant_id`). Não é invenção; é consolidação.

As três funções antigas seguem existindo e NÃO são alteradas aqui
(a 2150 é reconciliada na Query 2195 para delegar; as outras duas
recebem apenas o lookup com precedência na Query 2196). Convergir
integralmente os contratos delas é trabalho posterior, registrado
como dívida no README desta proposal.

--------------------------------------------------------------
CONTRATO
--------------------------------------------------------------
- mapping + propagation na MESMA transação: ou os dois, ou nenhum.
- COEXISTÊNCIA: jamais UPDATE do mapping global. O override é
  linha nova; remover a linha restaura o comportamento global.
- fail-closed: se internal.variant_type_mapping_impact() devolver
  block_reason, ABORTA antes de qualquer escrita.
- SEM force flag, por decisão 8 do mandato.
- locks em ordem determinística JOB -> ROW.
- action log: REUTILIZA o par já válido
  (CARD_VARIANT_TYPE_EXTERNAL_MAPPING, CARD_VARIANT_TYPE_EXTERNAL_MAPPING_CREATED)
  — confirmado na CHECK ck_catalog_admin_action_log_action_entity_match
  em 2026-09-14. NÃO há widen do action log nesta proposal.

Pré-requisitos:
- Query 2191 - coluna/FK/índices de escopo.
- Query 2192 - contrato de leitura (scope + impact).
