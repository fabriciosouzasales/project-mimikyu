...
Query.......: 5130 - Create set_slot_lock()
Versão......: 4.0
...
Descrição...:
Liga ou desliga o Lock de um ou mais Slots. UMA assinatura cobre
Lock/Unlock, individual e em lote (Bulk Lock/Unlock da Tool Rail) —
"operações em lote aplicam a mesma propriedade uniformemente a um
conjunto de Slots, independente da ocupação de cada um" (C-43).

Lock é propriedade do SLOT (C-43): pode ser aplicado a Slot vazio, sem
Expected Content e sem ocupação. Nenhuma pré-condição de conteúdo é
verificada — seria contradizer a decisão.

Substituir a Physical Card de um Slot NUNCA transfere o Lock para ela
(C-43): o Lock vive nesta tabela e não é tocado por nenhuma operação
de Assignment.

Set/unset do próprio Lock continua possível mesmo com o Slot locked —
caso contrário o Unlock seria impossível. O trigger 5118 protege
collection_layout_slot_assignment; esta RPC opera em
collection_layout_slot e não é afetada por ele.
...
CREATE OR REPLACE FUNCTION public.set_slot_lock(
    p_slot_ids UUID[],
    p_locked   BOOLEAN
)
RETURNS INTEGER
...
BEGIN
    ...
    IF p_locked IS NULL THEN
        RAISE EXCEPTION 'p_locked não pode ser NULL';
    END IF;
    ...
    UPDATE public.collection_layout_slot
       SET locked = p_locked
     WHERE id = ANY (v_ids)
       AND locked IS DISTINCT FROM p_locked;
...
REVOKE EXECUTE ON FUNCTION public.set_slot_lock(uuid[], boolean)
    FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.set_slot_lock(uuid[], boolean)
    TO authenticated;
