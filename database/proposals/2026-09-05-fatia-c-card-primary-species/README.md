...
| **Status** | **PROMOVIDO** — Esta pasta é mantida apenas como histórico do staging original (auditoria, revisões, riscos, alternativas rejeitadas) — a fonte canônica é `database/schema/6112_create_card_primary_species_table.sql`, `6113_create_card_primary_species_triggers.sql`, `6114_create_admin_resolve_card_primary_species_function.sql` e `6115_create_resolve_card_primary_species_bulk_function.sql`. A Query `2159` **deixou de ser canônica em 2026-09-18** (`BULK-STP-01-CANONICAL-RECONCILIATION-IMPLEMENTATION-01`): é alteração incremental de objeto já existente, foi movida para `database/migrations/2159_widen_catalog_admin_action_log_for_card_primary_species.sql`, e o estado terminal das três CHECK de `catalog_admin_action_log` passou a viver em `database/schema/2010_create_catalog_admin_action_log.sql` v2.0. Promovido em `COLLECTIONS-POKEDEX-FATIA-C-CANONICAL-CLOSEOUT-01` (2026-09-05), após `IMPLEMENTATION-01-RESUME` (aplicação real) e `INCREMENTAL-IMPLEMENTATION-01` (integração com os dois callers reais + backfill, ver README irmão `2026-09-05-fatia-c-incremental-integration/`). |
