...
| 11-BIS | **`2224` GUARD SAME-GAME do 3º eixo** · ✅ **EXECUTADA / LIVE VALIDATED / CLOSED** em 2026-09-22 (`BATCH9-2224-CLOSEOUT-01`), blob executado `5bae844b…`, publicado em `2fcd6231`, aplicada **direto no SQL Editor**, POSTCHECK read-only **20/20 GATEs · GLOBAL PASS**, zero drift `card_variant` (24.893 → 24.893 · EC não-nulo 0 → 0). Criada por `BATCH9-2217-READINESS-CORRECTION-01`. Cria `internal.enforce_card_variant_edition_context_profile_game()` + `trg_card_variant_edition_context_profile_game` (`BEFORE INSERT OR UPDATE OF card_id, edition_context_profile_id`), espelhando a `2170`. **Razão de existir:** a `BATCH9-2217-READINESS-AUDIT-01` provou que **não havia** proteção server-side contra `edition_context_profile_id` de outro Game — `validate_card_variant_game_consistency` (`161`) só compara Card × Variant Type, e a `2220` **não** a estende, ao contrário do que a documentação afirmava. Roda **ANTES** da `2217`: o guard precede a assinatura capaz de gravar a coluna | os três eixos passam a ter guard same-Game dedicado | **não** — só acrescenta uma recusa; zero dado alterado. O PASSO 2 prova que nenhuma Variant existente viola, sem cardinalidade assumida |
| 12-A | **`2217` EXPAND** · ✅ **EXECUTADA / LIVE VALIDATED** em 2026-09-23 (`BATCH9-2217-LIVE-VALIDATION-CLOSEOUT-01`), blob `c9abf5d7…`, POSTCHECK **35/36** com o único STOP (G6.a, hash bruto do corpo) adjudicado **TRANSPORT/EOL-ONLY, NÃO MATERIAL** — corpo idêntico após normalizar CRLF→LF; ver `EXECUTION-BATCHES.md` · pré-requisito `2224` satisfeito — cria `write_card_variant` de **7 args** e **PRESERVA** a de 6. Não dropa nada | duas assinaturas coexistem; o confirm antigo segue chamando a de 6 | **não** — aditivo puro. Nenhum caller aponta para assinatura ausente |
| 12-B | **`2218` SWITCH** · ✅ **EXECUTADA / LIVE VALIDATED / CLOSED** em 2026-09-25 (`BATCH9-EDITION-CONTEXT-WRITER-CLOSEOUT-01`), v1.3 via **MCP `execute_sql`**, artefato executado blob `6f4dbd9c…` (corpo LF `b83f7708…`), após LIVE PRECHECK 38/38 e performance 6/6; 1ª tentativa v1.2 falhou no parse (42601) sem efeito físico — confirm: matching quádruplo + lock + `unique_violation`; passa a chamar **somente** a de 7 | writer fornece 4, identidade física ainda distingue 3 | **não — mas NÃO por ser "superconjunto compatível"** (correção do Gate A): duas Variants diferindo só em contexto colidiriam nos índices antigos e o writer levantaria `CARD_VARIANT_IDENTITY_CONFLICT_UNRESOLVED`. É o **FREEZE da etapa 3** que fecha a janela |
| 12-C | **`2223` CONTRACT** · ✅ **EXECUTADA / LIVE VALIDATED / CLOSED** em 2026-09-25 (`BATCH9-EDITION-CONTEXT-WRITER-CLOSEOUT-01`), v1.1 endurecida via **MCP `execute_sql`**, artefato executado blob `72ba009e…`, LIVE PRECHECK 9/9; writer de 6 args **AUSENTE** no LIVE — **Batch 9 CLOSED**; ~~próximo: **13 · `2209` — READINESS**~~ *(superado: etapa 13 executada em 2026-09-25)* — prova database-wide que ninguém chama a de 6, então `DROP` dela | uma única assinatura, a de 7 | **não** — fail-loud se sobrar caller legado; sem wrapper de compatibilidade |
| 13 | **`2209`** · ✅ **EXECUTADA / LIVE VALIDATED** em 2026-09-25 (`BATCH10-2209-LIVE-VALIDATION-CLOSEOUT-01`), 1x via **MCP `execute_sql`**, artefato executado blob `39084850…`, após JIT PRECHECK 15/15 — `CREATE UNIQUE INDEX` (normal, `ShareLock`) + `ADD CONSTRAINT … USING INDEX` (`AccessExclusiveLock` até o `COMMIT`), **atômicos entre si**. LIVE: `uq_card_variant_identity` índice + constraint `u`, `NULLS NOT DISTINCT`, 4 colunas; antigas intactas; 24.893 · EC 0 · dup 0/0 · 11 índices. Ledger 0 (rastreabilidade). **Batch 10 CLOSED**; ~~próximo: **14 · `2215` — READINESS** (NÃO autorizada)~~ *(superado: etapa 14 executada em 2026-09-26)*. Liberada pelo T1 | identidade nova existe; **as antigas ainda existem e ainda mandam** | **não** — conviver é seguro: as antigas são mais restritivas |
| 14 | **`2215`** · ✅ **EXECUTADA / LIVE VALIDATED / CLOSED** em 2026-09-26 (`BATCH11-2215-LIVE-VALIDATION-CLOSEOUT-01`), v1.1 fail-closed e não idempotente, 1x via **MCP `execute_sql`**, blob executado `426b3555…` (publicado antes em `e375c886…`), após JIT PRECHECK 16/16. LIVE: antigas ausentes; `uq_card_variant_identity` única autoridade (índice 221012 / constraint 221013); 9 índices; 24.893 · EC 0 · dup 0/0. Ledger 0 (rastreabilidade). ~~**Batch 11 segue aberto**; próximo: **15 · `2216` — READINESS** (NÃO autorizada).~~ *(superado: etapa 15 executada em 2026-09-26)* FREEZE ATIVO — `DROP` das duas antigas de `card_variant`, com prova antes e depois | **só a identidade nova** — o eixo passa a existir de fato | **não** — `2215` recusa rodar se a nova não for CONSTRAINT válida |
| 15 | **`2216`** · ✅ **EXECUTADA / LIVE VALIDATED / CLOSED** em 2026-09-26 (`BATCH11-2216-LIVE-VALIDATION-CLOSEOUT-01`), v4.1 fail-closed e não idempotente, 1x via **MCP `execute_sql`** (retorno `[]`), blob executado `3fc30f42…` (publicado antes em `a446797d…`), após JIT PRECHECK 18/18 — `DROP` das duas antigas de staging + prova funcional S4 (α/β coexistem, γ rejeitada por `uq_cvir_row_identity`) em `SAVEPOINT` desfeito. LIVE (postcheck 29/29): antigas ausentes; `uq_cvir_row_identity` (192899) única identidade única além da PK; 8 índices, 0 não saudáveis; staging 26.127 sem drift; jobs 145 preservados; resíduo S4 0. Ledger 0 (rastreabilidade). **Batch 11: execução concluída, closeout documental registrado**; próximo: **16 · `2830` — READINESS** (NÃO iniciada / NÃO autorizada). FREEZE ATIVO | só a identidade nova | **não** — pré-condição **job-aware**: as 415 `CANCELLED` não bloqueiam |
