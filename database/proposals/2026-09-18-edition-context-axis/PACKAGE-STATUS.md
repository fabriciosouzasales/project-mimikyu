...
| **Batch 9 — `2218`** | **EXECUTED / LIVE VALIDATED / CLOSED** (2026-09-25) — SWITCH v1.3 via **MCP `execute_sql`**. Artefato **executado**: blob `6f4dbd9c5ffe15bbf093a0747b7e77e8cc525553` · md5 `bedb5e32…` · corpo LF `b83f7708ca2b7498b753b394d768f66e` (20.095 B / 389 LF). Antes: LIVE PRECHECK 38/38 + performance 6/6; 1ª tentativa (v1.2) falhou no parse, **zero efeito físico** provado. Depois da execução o arquivo recebeu **só** comentário de closeout (blob documental novo, executável idêntico) | `BATCH9-EDITION-CONTEXT-WRITER-CLOSEOUT-01` |
...
| **Batch 10** | **CLOSED / PUBLISHED** — closeout publicado em `c85c7717…`. *(Histórico: entre `2209` e `2215` coexistiram três garantias UNIQUE; superado pela `2215`.)* | `EXECUTION-BATCHES.md` |
| **Batch 11 — `2215`** | **EXECUTED / LIVE VALIDATED / CLOSED** (2026-09-26) — `DROP … RESTRICT` das duas identidades antigas de `card_variant` via **MCP `execute_sql`**, 1 execução após JIT PRECHECK 16/16 (`gate_pass = true`). Artefato **executado**: blob `426b35557be77eeec7a4cddd8c63e3fafea070f1` · md5 `7fd1ba48…`, publicado antes em `e375c886…`. LIVE: antigas ausentes; `uq_card_variant_identity` (índice 221012 / constraint 221013) é a **única** autoridade física; 9 índices, 0 não saudáveis; UNIQUE = pkey, card_order, id_card, identity, one_default; 24.893 · EC 0 · dup 0/0; writer e confirm preservados. Depois, **só** comentários no arquivo | `BATCH11-2215-LIVE-VALIDATION-CLOSEOUT-01` |
| `2215` no ledger | **0** — rastreabilidade, mesma classe das anteriores. Não reconciliar manualmente | POSTCHECK LIVE |
| **Batch 11** | **ABERTO** — `2215` ✅; `2216` (staging) pendente. O eixo Edition Context é fisicamente expressivo em `card_variant`, mas **não** autoriza importação/revisão: FREEZE ATIVO até o UNFREEZE | `EXECUTION-BATCHES.md` |
| Próximo estágio | **Batch 11 — `2216` — READINESS** · **NÃO EXECUTADA / NÃO AUTORIZADA** — exige readiness audit e mandato explícito de Fabrício. **FREEZE ATIVO** | `EXECUTION-BATCHES.md` |
