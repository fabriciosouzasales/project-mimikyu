...
Status......: CONFIRMADO EXECUTADO — EVIDENCIA HISTORICA DE STAGING
               (2026-09-12). A copia canonica vive em
               database/migrations/2164_harden_admin_confirm_catalog_
               variant_import_contract.sql. Corpo executavel IDENTICO —
               a unica diferenca esta no cabecalho. A versao executada
               e a v1.1; a v1.0 nunca foi executada em lugar nenhum.
               Validado pela Query 2822, Secoes 1 e 3.
...
`array_length` NAO aparecem no prosrc LIVE da funcao.

Consequencias do estado atual:
1. Array multidimensional passa direto; ANY() varre todos os elementos.
2. Array vazio vira no-op silencioso — a funcao move o job para
   CONFIRMING, nao processa nada e devolve contadores como se tivesse
   trabalhado. Contrato ambiguo.
3. Sem teto, uma chamada grande mantem locks de linha abertos durante
   todo o loop.
...
--
-- Como validar:
--   Query 2822 v1.1 - Validate Variant Import Bulk Contract,
--   Secoes 1 e 3.
-- ================================================================
