...
Query.......: 5020 - Create Storage Container Table
Versão......: 1.0
Status......: CANÔNICA
...
Sequenciamento: esta Query precede Collection (Incremento 2B, ainda não
iniciado) porque collection.default_storage_container_id será NOT NULL
desde a criação (C-36) — criar a tabela collection antes de Storage
existir geraria estado fisicamente incompatível com C-36 (ver
...
Não auto-provisionado um Storage Container "padrão"/genérico por
Inventory (correção explícita de
COLLECTIONS-PHYSICAL-MODELING-03-FINAL-01, item 5) — Storage Container
representa unidade física real do acervo do usuário, nunca um
placeholder fictício. Requisito de UX futuro, ainda não implementado
em UI: na criação da Collection, o usuário poderá selecionar um
Storage Container já existente ou criar um novo dentro do próprio
fluxo.
...
Supabase 20260901002931). Estrutura, UNIQUE(id, inventory_id), RLS e
grants confirmados fisicamente contra o banco (information_schema/
pg_constraint) na mesma rodada. Validação funcional ao vivo:
create_storage_container() cria sempre no Inventory do próprio
chamador; usuário A não vê Storage Container de B (RLS); INSERT direto
e acesso anon negados (permission denied) — ver
