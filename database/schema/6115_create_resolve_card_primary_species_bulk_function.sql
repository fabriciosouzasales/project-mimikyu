...
Data........: 2026-09-05 (staging em
               ...
               ...-GATE-4-FIX-01; executada no banco real em
               ...-IMPLEMENTATION-01-RESUME; promovida para
...
INTEGRAÇÃO COM IMPORT INCREMENTAL (mandato desta Fatia, item 5 — ver
Query 6116, promovida junto com esta): o ponto de integração real não
está dentro de import-catalog-cards nem de admin_confirm_catalog_
import() — é a função resolve_card_primary_species_for_catalog_import_
job() (Query 6116), chamada DEPOIS que admin_confirm_catalog_import()
já persistiu as Cards (resulting_card_id definido), em RPC separada,
sem nunca reverter a confirmação em caso de falha.

COMPORTAMENTO DE BACKFILL (mandato item 1 — executado em
COLLECTIONS-POKEDEX-FATIA-C-BACKFILL-APPLY-01, 5675 Cards resolvidas):
um script/Edge Function futuro lê card_id + raw_data->>'dexId' de
catalog_import_row para as Cards com evidência sobrevivente, monta
p_evidence_batch no mesmo formato e chama esta função (em um ou mais
lotes). As 760 Cards sem nenhuma evidência sobrevivente permanecem
UNRESOLVED — candidatas a reconciliação 100% editorial via Query 6114,
não a este caminho.
