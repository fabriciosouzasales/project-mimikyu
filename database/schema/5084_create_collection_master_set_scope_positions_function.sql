...
`LANGUAGE SQL`, `STABLE` — mesma linguagem de `5070`/`5071`/`5083`,
nunca PL/pgSQL (ver nota documental em `5083`).

Performance real medida (COLLECTIONS-PHYSICAL-INCREMENT-02F-
PERFORMANCE-01, `5813` v2.0): positions sobre Scope de 10.000 Card
Variants (pool combinado, `only_missing=false`) mediu ~93ms, 10.000
linhas retornadas; positions sobre Scope de ~300 Variants com
`only_missing=true` mediu ~8ms, 90 linhas — sem spill de sort/hash em
nenhum plano capturado.
