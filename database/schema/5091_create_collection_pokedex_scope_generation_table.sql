...
confirmou colunas/tipos/defaults, PK composta, FKs
(collection_pokedex_reference RESTRICT/CASCADE, pokemon_generation
RESTRICT/RESTRICT), RLS habilitado com a policy SELECT própria, GRANT
SELECT restrito a authenticated, REVOKE de TRUNCATE/REFERENCES/TRIGGER/
MAINTAIN de anon/authenticated/service_role. Validado funcionalmente por
uma Collection GENERATION_FILTERED real, criada com 2 Generations em
BEGIN/ROLLBACK. Zero resíduo.
