...
Versão......: 3.0 (estende 5070 v2.0, 02E)
Status......: CANÔNICA
...
Mesmo padrão de arquivo "update" separado já usado em `5068`/`5069`
(02E, estendendo `5034`/`5065` de incrementos anteriores) — `5070`
permanece intocado em `database/schema/`, este arquivo é a extensão
posterior aplicada sobre o mesmo objeto físico.
...
`STANDARD_SET`, e reconfirmado por medição real em `5813`/workload F
(2.000 Physical Cards concentrados em 10 posições, mesmo tempo/mesmo
custo de buffer do baseline sem duplicatas).
...
Segurança inalterada da v2.0: `SECURITY DEFINER`, `STABLE`, `LANGUAGE
SQL` (correção documental — este objeto NUNCA foi PL/pgSQL; ver nota
abaixo), `SET search_path = ''`, ownership reconstituído manualmente
na CTE `target` (`c.owner_user_id = (select auth.uid())`, sempre
primeiro passo), `(select auth.uid()) IS NOT NULL` explícito, nunca
`is_admin()`. Não-enumeração preservada: Collection inexistente/de
outro Owner/`OPEN_CURATION`/`NONE` -> 0 rows, mesma forma nos quatro
casos. Catálogo Editorial permanece fechado — nenhuma policy nova em
`card`/`card_variant`/`card_set`, esta função só os lê através da
própria projeção `SECURITY DEFINER`. `REVOKE ALL FROM PUBLIC`/`anon`
+ `GRANT EXECUTE TO authenticated`, inalterado.

CORREÇÃO DOCUMENTAL (COLLECTIONS-PHYSICAL-INCREMENT-02F-CANONICAL-
PROMOTION-01): o relatório de performance de `5813` (COLLECTIONS-
PHYSICAL-INCREMENT-02F-PERFORMANCE-01) descreveu inicialmente esta
função como PL/pgSQL ao interpretar o nó "Function Scan" retornado
pelo `EXPLAIN` da chamada externa. Confirmado por leitura direta de
`pg_proc`/`pg_language` no banco físico: `collection_completion_
summary()` é e sempre foi `LANGUAGE SQL` (mesma linguagem de `5070`/
`5071`/`5084`) — só `apply_master_set_scope_diff()` (5079) e as RPCs
de transição/mutação (`5080`-`5082`) são `plpgsql`. O `EXPLAIN`
aparece como "Function Scan" para qualquer chamada `SELECT * FROM
funcao(...)`, independentemente da linguagem interna — o benchmark não
forneceu (e não alega) visibilidade do plano interno da função em
nenhum dos dois casos.

Performance real medida (COLLECTIONS-PHYSICAL-INCREMENT-02F-
PERFORMANCE-01, `5813` v2.0): summary sobre Scope de 10.000 Card
Variants (pool combinado, acima do maior Card Set real hoje) mediu
~57ms; summary sobre a mesma Collection com o Inventory do Owner em
32.210 Physical Cards mediu ~56ms — praticamente idêntico, confirmando
que o custo escala com o tamanho do Scope, não com o volume total do
Inventory.
