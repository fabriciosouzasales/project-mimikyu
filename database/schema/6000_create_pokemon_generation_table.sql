...
-- físico confirmou colunas/tipos/defaults, PK, UNIQUE(code),
-- UNIQUE(ordinal_number), os três CHECKs e RLS habilitado sem
-- nenhuma policy. Testes comportamentais (BEGIN...ROLLBACK)
-- confirmaram rejeição de ordinal_number e code duplicados.
-- ================================================================

-- ================================================================
-- Correção documental (2026-09-04, achado de auditoria externa,
-- COLLECTIONS-PHYSICAL-INCREMENT-02G-SECURITY-CLOSEOUT-FIX-01):
-- o texto de "Regras de Negócio" acima ("formato GENERATION_
-- <algarismo romano>") e o COMMENT ON COLUMN original de code
-- davam a entender que o CHECK estrutural valida especificamente
-- algarismo romano. Não é verdade — ck_pokemon_generation_code_format
-- é genérico (^[A-Z][A-Z0-9_]*$); GENERATION_I/II/III... é convenção
-- canônica de nomenclatura, não uma regra imposta pelo banco. A
-- constraint NÃO foi alterada. O COMMENT ON COLUMN de code foi
-- corrigido pela Query 6002 (nova Query, sem reescrever este
-- arquivo/histórico retroativamente). Este bloco de correção não
-- reabre o conteúdo normativo de 6000 — apenas sinaliza a imprecisão
-- textual encontrada.
-- ================================================================
