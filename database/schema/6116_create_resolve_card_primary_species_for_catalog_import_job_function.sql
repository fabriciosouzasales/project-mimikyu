...
Versão......: 1.2 (CONFIRMADO EXECUTADO E PROMOVIDO)
...
Data........: 2026-09-05 (staging em
               ...
               ...-FINAL-CHECK-01; executada no banco real em
               ...-INCREMENTAL-IMPLEMENTATION-01; promovida para
               database/schema/ em ...-CANONICAL-CLOSEOUT-01 — corpo
               SQL byte-idêntico ao executado, apenas cabeçalho
               Status/Versão/Data atualizados. Callers reais: A.
               confirmarImportacao() em web/app/catalogo/importar-
               cartas/tcgdex/actions.ts; B. index.ts de
               supabase/functions/revalidate-catalog-import-rows —
               ambos chamam de forma não-bloqueante, pós-confirmação)
...
que tem hoje EXATAMENTE DOIS callers reais (REVISION-01), ambos já
integrados (ver header de promoção acima):

  A. confirmarImportacao() (Server Action, web/app/catalogo/importar-
     cartas/tcgdex/actions.ts) — rota específica de TCGdex; na prática
     só processa jobs source='TCGDEX', mas 6116 não confia nisso (ver
     correção v1.2 acima). Chama esta função uma única vez, depois que
     TODO o loop de confirmação em lotes já terminou.
  B. confirmCatalogImport() (supabase/functions/revalidate-catalog-
     import-rows/services/database.ts), orquestrado por index.ts —
     GENÉRICO, não filtra por source; é exatamente por causa deste
     caller que o guard de source em 6116 é necessário, não decorativo.
     index.ts chama esta função com o client escopado pelo JWT do
     administrador (userClient), nunca o client de service role,
     somente depois que confirmCatalogImport() já teve sucesso.
...
ERRO NÃO-BLOQUEANTE ("REQUISITO CRÍTICO", REVISION-01, confirmado por
implementação real nos dois callers): chamada em RPC SEPARADA, depois
que admin_confirm_catalog_import() já commitou — em NENHUM dos dois
callers uma falha aqui reverte a confirmação. Fluxo A e Fluxo B
checam `{ error }` diretamente (nunca uma função auxiliar que lança
exceção) e apenas logam a falha via console.error — nunca alteram o
resultado retornado ao usuário nem o status do job/importação. O guard
...
sempre seguro. Confirmado por teste funcional real (26 cenários,
INCREMENTAL-IMPLEMENTATION-01).
