...
-- em species_id), as duas UNIQUE, o CHECK de position_number > 0, RLS
-- habilitado sem nenhuma policy, e zero privilégio efetivo
-- (has_table_privilege) para anon/authenticated. Validação
-- comportamental (Seção 2.2) confirmou as duas UNIQUE, o CHECK, as
-- duas imutabilidades e o RESTRICT em species_id. Advisor de
-- performance pós-implementação sinalizou (INFO) a ausência de índice
-- cobridor em species_id — aceito deliberadamente, ver racional acima.
-- Script completo permanece em database/proposals/2026-09-04-pokedex-
