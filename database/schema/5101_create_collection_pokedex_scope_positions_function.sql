...
Versão......: 1.0
Status......: CONFIRMADO EXECUTADO / PROMOVIDO / SUPERADO LIVE por 5103
...
STATUS DESTA QUERY: CONFIRMADO EXECUTADO / PROMOVIDO / SUPERADO
LIVE por 5103.

Aplicada ao banco real em 2026-09-06
(`COLLECTIONS-POKEDEX-FATIA-E-IMPLEMENTATION-01`) e validada por `5814`
v1.3 — 87/87 PASS, zero residuo. O corpo vivo de
`collection_pokedex_scope_positions()` foi posteriormente SUBSTITUIDO
por `5103` v2.0 (remediacao de performance, Alternativa B), aplicada em
`COLLECTIONS-POKEDEX-FATIA-E-PERFORMANCE-REMEDIATION-IMPLEMENTATION-01`.
Este arquivo e mantido em `database/schema/` como historico incremental
NAO foldado (`5101` -> `5103`): registra o contrato original de 5 campos
do read model, que `5103` preserva integralmente — a mudanca de `5103` e
exclusivamente de forma de juncao, nunca de resultado (equivalencia
provada por `5816` v1.1, 13/13). Promovido em
