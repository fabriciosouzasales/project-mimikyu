...
Versão......: 1.0
Status......: CONFIRMADO EXECUTADO em 2026-09-07 via Supabase MCP
               (apply_migration, ledger 20260907231143)
...
Aplicada ao banco real em 2026-09-07 via Supabase MCP (apply_migration),
ledger `20260907231143`, na rodada
`PRICING-PAYLOAD-CARDINALITY-HARDENING-01-IMPLEMENTATION`. Validada pelo
harness `3860` v1.1 em 18 TOTAL / 18 PASS / 0 FAIL / 0 NOT PROVEN, sendo
`N06` (multidimensional `dim1=2`, `cardinality=600`, rejeitado) a prova do
fechamento. Lógica de negócio provada byte-a-byte idêntica à baseline
efetiva `3968` (md5 `858e53ff9740b1d3f0ffff829d2986e2`).

Promovida para `database/migrations/` — destino canônico da faixa `3xxx` —
em `SCHEMA-PROMOTION-RECONCILIATION-01` (2026-09-08). NÃO houve fold-in em
`3968`: por decisão de Fabrício, esta é uma migration independente, e a
cadeia histórica `3903` -> `3904` -> `3918` -> `3924` -> `3968` -> `3972`
permanece intacta e legível como sequência. O harness `3860` NÃO é
promovido, por convenção. A cópia histórica permanece em
`database/proposals/2026-09-07-pricing-payload-cardinality-hardening/`.

A promoção alterou apenas cabeçalho/rodapé: o corpo executável
(`BEGIN;` .. `COMMIT;`) permanece byte-idêntico ao da cópia em proposals.

NOTA DE ESTILO: os demais arquivos de `database/migrations/` usam
comentário de linha (`--`) no cabeçalho; este preserva o bloco `/* */` do
staging, deliberadamente, para que a promoção seja auditável como
alteração apenas de cabeçalho, sem reescrita.
================================================================
*/
