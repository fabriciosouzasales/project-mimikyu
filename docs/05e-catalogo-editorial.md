...
## Auto-refresh ao focar a aba em Importar Imagens (2026-08-11)

`ImportarImagensView` (`web/components/catalogo/importar-imagens-view.tsx`) passou a chamar `router.refresh()` sempre que a aba volta a ficar visível (listener em `document.visibilitychange`, disparando quando `document.visibilityState === "visible"`), além dos pontos que já disparavam refresh após cada ação (importação concluída, upload manual concluído). Cobre o cenário de uma aba deixada aberta por muito tempo — sem exigir mais um F5 manual para os indicadores/combobox refletirem o estado real do banco. `tsc --noEmit` confirmado limpo.

## `fetchAllRows()` — correção real de paginação silenciosa (2026-08-11, mesmo dia, rodada seguinte)

**Correção de premissa em relação à entrada anterior**: o auto-refresh acima resolvia um cenário real (aba parada), mas não era a causa completa. Fabrício reportou, logo depois de uma importação manual, Coleções já 100% importadas — `ME5`, `SV8`, `SV8.5`, `SV9`, `SV10`, `SV10.5B`, `SV10.5W` (confirmadas 100% em EN e PT-BR direto no banco) — aparecendo com contagem parcial/errada mesmo num render recém-atualizado por `router.refresh()`, sem qualquer relação com cache de aba.

Causa raiz real, em `fetchAllRows()` (helper de paginação compartilhado por toda leitura de `card`/`card_asset`/`catalog_import_job` sem filtro pequeno, `web/lib/catalogo/queries.ts`): o laço decidia "última página" comparando `data.length` contra a CONSTANTE assumida `SUPABASE_MAX_ROWS_PAGE_SIZE` (1000) — presumindo que o `db-max-rows` real do projeto Supabase é exatamente 1000. Essa configuração vive no dashboard (Settings → API), fora do controle deste código, e não há garantia de tempo de execução de que bate com a constante. Quando o cap real do servidor é menor que 1000, cada página já vem truncada pelo próprio PostgREST antes de chegar ao código — `data.length` nunca alcança 1000, então o laço concluía "acabou" já na PRIMEIRA página, descartando todo o resto silenciosamente. `getImagesImportadasPorCardSet()` soma o `card_asset` de TODAS as Coleções com carta cadastrada numa única chamada (não só as pendentes) — um volume grande o bastante para estourar qualquer cap abaixo de 1000, o que explica a contagem parcial aparecer em várias Coleções ao mesmo tempo, sem padrão óbvio de qual "damos sorte" de caber na primeira página.

Mesma classe de bug do incidente de 2026-08-01 (Card Set MEE) documentado no comentário de `fetchAllRows()` — causa raiz diferente: ali faltava paginação; aqui a paginação existente parava cedo demais por presumir um tamanho de página que pode não ser o real.

**Correção**: o laço agora avança `from` pelo tanto que REALMENTE veio na resposta (`data.length`), não pela constante assumida, e só para quando uma página vem vazia — correto para qualquer cap real do servidor, conhecido ou não, sem precisar confirmar o valor exato de `db-max-rows` (não foi possível consultá-lo por SQL; é uma configuração de plataforma, não uma GUC de banco). Correção num único ponto (`fetchAllRows()`), beneficia todas as suas 11 chamadas no arquivo — inclusive `getCardCountsForSets()` (contagem de Cards por Coleção, usada em toda a Visão Geral/Cartas/Card Sets) e `getCartasCatalogoStats()` (indicador global "Sem imagens" da tela Cartas), que corriam o mesmo risco silencioso. `tsc --noEmit` confirmado limpo.

## `collector_number` sem preenchimento de zeros — corrigido na origem e no dado existente (2026-08-11, mesmo dia, rodada seguinte)

Fabrício, a partir de captura de tela da galeria de Cartas: cartas apareciam como "#1/185" em vez de "#001/185" — "por padrão precisamos registrar a identificação no formato xxx/xxx... nunca devemos permitir 1/185, 11/185, 010/88".

Causa raiz: `resolveCatalogImportRow()` (`supabase/functions/_shared/catalog-normalization/resolve-row.ts`, núcleo compartilhado pela importação inicial via TCGdex e pela revalidação — Ciclo 2/`ADR-024`) gravava `collector_number` direto do `localId` recebido da TCGdex (`String(rawCard.localId)`), sem nenhum preenchimento — a TCGdex já entrega o valor sem zeros à esquerda. Cadastro manual (`admin_create_card()`/`admin_update_card()`, `ADR-023`) não é afetado por esta causa — lá o número é digitado diretamente pelo administrador, fora do escopo desta rodada.

**Correção na origem**: nova função `padCollectorNumber(rawNumber, collectorTotal)` no mesmo módulo — preenche com zeros à esquerda até a MESMA quantidade de dígitos de `collectorTotal` (nunca um número fixo de dígitos: "01/88" para uma Coleção de 88 cartas, "001/185" para uma de 185 — nunca "010/88", exatamente o formato que Fabrício pediu para nunca permitir). Só atua em números puramente numéricos (`^[0-9]+$`), preservando sem alteração qualquer formato alfanumérico que já venha pronto da própria fonte (ex.: "TG01", "SWSH001"). `collectorTotal` nulo mantém o valor bruto, sem largura de referência. `collectorOrder` continua derivado do `localId` original (antes do padding) — `Number("001") === Number("1")`, resultado idêntico de qualquer forma. Correção aplicada em código no repositório; **deploy da Edge Function pendente**, por processo normal de Fabrício (não usada a ferramenta de deploy do MCP Supabase nesta rodada — substituiria a função inteira a partir de uma leitura local, com risco de dessincronizar do que está publicado se houver alguma diferença não capturada no repositório).

**Correção do dado já cadastrado incorretamente — Query `2129` (CONFIRMADO EXECUTADO)**: 837 Cards em toda a base tinham `collector_number` mais curto que os dígitos de `collector_total`. Risco de colisão avaliado antes da execução: só uma Coleção (`SVP`) tem `collector_total` inconsistente entre suas Cards (225 para as cartas originais da TCGdex, 226 para as 8 cadastradas manualmente depois da correção do total real — ver seção "SVP" anterior) — mas as duas larguras são idênticas (3 dígitos), então o padding não diverge nem colide. `UPDATE` com `LPAD()`, restrito a números puramente numéricos e mais curtos que o alvo (nunca trunca). Validação de confirmação rodada por Fabrício: `0` linhas restantes. Ver `database/migrations/2129_backfill_card_collector_number_padding.sql`.

## Card Set: base_set_size/total_set_size editáveis na tela de Coleções (2026-08-11, mesmo dia, rodada seguinte)

Fabrício, a partir de captura de tela da tabela de Coleções: "A tela de edição das coleções deve permitir alterar dois campos: base_set_size e total_set_size". Até esta rodada, `admin_update_card_set()` deliberadamente excluía os dois campos (ver v3.0 acima) — "campos estruturais de baixa frequência de mudança, correção rara e deliberada". Decisão revertida: motivação real por trás do pedido é a mesma da correção de `total_set_size` de SVP (225 → 226, feita via SQL direto por não existir ainda esta via de UI) — poder corrigir um total oficial errado sem depender de intervenção manual no banco.

**Query `2130` (CONFIRMADO EXECUTADO)** — `admin_update_card_set()` v4.0, assinatura ampliada de 6 para 8 parâmetros (`p_base_set_size`, `p_total_set_size`). Validação mirra `admin_create_card_set()` (Query 2051): base positiva, total ≥ base — já reforçada por `ck_card_set_base_size_positive`/`ck_card_set_total_size_valid`, checada aqui antes por mensagem administrativa clara. **Sem trava condicional por Cards já cadastradas** (diferente de `code`, Migration 2091) — os dois campos ficam livres para correção a qualquer momento, exatamente o cenário do pedido. Regra de PROMO (base = total) passa a validar os valores SENDO ENVIADOS na chamada, não mais os já gravados na linha — diferença desta versão, já que agora podem mudar na mesma chamada que muda `set_type`. `DROP FUNCTION` + `CREATE` explícitos (não `CREATE OR REPLACE` sozinho) porque a assinatura muda, mesmo padrão já usado pela Migration 2091. Assinatura final validada por Fabrício via `pg_get_function_identity_arguments()`. Ver `database/schema/2048_create_admin_update_card_set_function.sql` (v4.0), `database/migrations/2130_widen_admin_update_card_set_function_for_sizes.sql`.

**Frontend**: `EditCardSetDialog`/`EditCardSetForm` (`web/components/catalogo/card-set-dialogs.tsx`) ganharam dois `Input type="number"` (Quantidade base/Quantidade total), mesmo padrão visual de Ordem de lançamento/Data de lançamento — `defaultValue` de `cardSet.baseSetSize`/`cardSet.totalSetSize`. `updateCardSet()` (`web/app/catalogo/card-sets/actions.ts`) valida os dois campos com a mesma regra de `createCardSet` (inteiro positivo; total ≥ base) antes de repassar à RPC. `tsc --noEmit` confirmado limpo.
