...
**Identidade e Acesso — Incremento 1 ("Meu Perfil"): banco de dados e documentação CONCLUÍDOS; implementação de interface PENDENTE.** Ao testar o cadastro real, Fabrício percebeu a ausência de qualquer gestão de perfil associada — motivou a modelagem formal de `user_profile`, separada de `auth.users` (`ADR-020-user-profile-and-username-identity-model.md`), inaugurando o Modelo Modular de Numeração do STD-001 (milhar `1000`–`1999`). Tabela, triggers de imutabilidade/normalização, RLS, function de auto-provisionamento no cadastro (`handle_new_user()`), checagem de disponibilidade de username (`username_available()`) e bucket de avatares — tudo criado, confirmado no Supabase e documentado (`1000`–`1040`/`1710`/`1800`–`1840`, ver `05-modelo-de-dados.md`). **Fabrício aprovou esta camada como encerrada em 2026-07-26** — nenhuma alteração adicional de modelo de banco está prevista para este incremento, salvo bloqueio real identificado durante a integração com o frontend. O trabalho restante do Incremento 1 é só de interface: formulário de cadastro coletando `username`/`display_name` e a tela `/perfil` real (hoje placeholder) — ver Task 356, em andamento. **Incremento 2 (lista administrativa de usuários) e qualquer modelo de papéis/permissões seguem fora de escopo, aguardando aprovação própria** — decisão explícita de Fabrício de não os antecipar.
