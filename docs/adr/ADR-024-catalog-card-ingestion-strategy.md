...
## Emenda (2026-08-07) — Raridade: mapeamento self-service e revalidação

Contexto real que motivou esta emenda: `import-catalog-cards` (processador TCGdex deste ADR) sempre dependeu de `RARITY_NAME_ALIASES`, um mapa hardcoded em `services/normalize.ts`, para traduzir o valor bruto de raridade devolvido pela TCGdex ("Rara Holo", "Hyper Rare" etc.) para o `rarity_id` canônico. Toda vez que a TCGdex introduzia um valor novo, sem alias correspondente, a linha nascia `validation_status = NEEDS_REVIEW` e a única correção possível era um deploy de código alterando o mapa — inclusive para Coleções já staged, que precisavam ser reimportadas do zero para se beneficiar da correção. Esta emenda substitui o mapa hardcoded por dado administrável via UI, sem deploy, com revalidação de linhas já staged.

### Modelo: `rarity_external_mapping`

Nova tabela `public.rarity_external_mapping` (`game_id`, `asset_source_id`, `external_value`, `normalized_external_value`, `rarity_id`) — traduz um valor bruto de raridade de uma Fonte externa para a Raridade canônica correspondente, por Game+Fonte. `UNIQUE(game_id, asset_source_id, normalized_external_value)`: o mesmo valor bruto normalizado nunca mapeia para duas Raridades diferentes na mesma combinação Game+Fonte. `external_value` preserva o texto original exato (auditoria/exibição); toda busca/comparação usa `normalized_external_value`.

Normalização via `normalize_external_catalog_value()` (nova função `sql STABLE`): remove acento (`extensions.unaccent()`, requer a extensão `unaccent`), colapsa espaços múltiplos e converte para maiúsculas — torna a comparação insensível a caixa/acento/espaçamento, mas **não** a variação de redação ("Hiper Rara" não casa com "Hyper Rare" automaticamente; cada variante de texto precisa do seu próprio mapeamento explícito). A mesma função passa a ser usada tanto pelo cadastro self-service quanto pelo processador TCGdex (`_shared/catalog-normalization/`, módulo extraído de `import-catalog-cards/services/normalize.ts` para ser reutilizável por outra Edge Function, ver seção de revalidação abaixo) — elimina a duplicação de lógica de normalização entre backend SQL e Edge Function.

### Cadastro administrativo de Raridade

Nova família de funções (`SECURITY DEFINER`, checando `is_admin()`), acessível pela tela `/catalogo/raridades`:

- `admin_create_rarity()` / `admin_update_rarity()` — CRUD de `rarity` (Query 130) via UI, substituindo o cadastro exclusivamente por SQL manual usado até aqui. `game_id`/`code` permanecem imutáveis após a criação (mesmo princípio de campos estruturalmente protegidos já aplicado a `card_set_id`/`collector_number` em `ADR-023`).
- `admin_create_rarity_external_mapping()` / `admin_update_rarity_external_mapping()` — CRUD de `rarity_external_mapping`, vinculando um valor bruto pendente a uma Raridade já cadastrada ("Raridade existente" na tela).
- `admin_create_rarity_with_external_mapping()` — wrapper atômico sobre as duas funções acima, para o caso "Nova raridade" (o valor pendente não corresponde a nenhuma Raridade já cadastrada): cria a Raridade e o mapeamento na mesma transação.

Todas gravam `catalog_admin_action_log` (`RARITY_CREATED`/`RARITY_UPDATED`/`RARITY_EXTERNAL_MAPPING_CREATED`/`RARITY_EXTERNAL_MAPPING_UPDATED`) — mesma auditoria editorial já usada pelo restante do módulo (`ADR-023`).

### Revalidação sem reimportação

Corrigir um mapeamento não tinha efeito sobre linhas já staged de um job existente — a única forma de aplicar a correção era reimportar a Coleção inteira. Esta emenda adiciona um caminho de revalidação: `svc_apply_catalog_import_revalidation()` (contrato público, chamado pela nova Edge Function `revalidate-catalog-import-rows` sob identidade `service_role`) recalcula `catalog_import_row` de um job já `STAGED`/`CONFIRMING`/`COMPLETED_WITH_ERRORS`, usando a mesma lógica de normalização/matching do processador original (via `_shared/catalog-normalization/`), sem tocar `decision_status` de nenhuma linha — decisões humanas já tomadas são preservadas integralmente, só `validation_status`/`match_status`/`normalized_data`/`matched_card_id` são recalculados.

A escrita em lote fica isolada em `internal.persist_catalog_import_revalidation()` (camada canônica, `EXECUTE` revogado de `authenticated`/`anon` — só `svc_apply_catalog_import_revalidation()` chama, mesmo princípio de isolamento de `internal.write_card()`). Linhas que haviam falhado (`persistence_status = FAILED`) exclusivamente por raridade não mapeada, e que a revalidação torna `VALID`, são destravadas de volta para `PENDING` — um job `COMPLETED_WITH_ERRORS` com pelo menos uma linha destravada volta para `CONFIRMING`, nunca fica preso em "concluído com erro" depois que a causa raiz foi corrigida.

### Backfill e migração do mapa hardcoded

Os 25 aliases que viviam em `RARITY_NAME_ALIASES` foram migrados para `rarity_external_mapping` via backfill (Query 2104) — mesmo conjunto de valores, sem mudança de comportamento observável na importação TCGdex. `import-catalog-cards` foi adaptado para ler o mapeamento via `rarity_external_mapping`/`normalize_external_catalog_value()` (módulo compartilhado) em vez do mapa em código; regressão validada contra importações reais sem diferença de resultado.

### Tela `/catalogo/raridades`

Nova tela administrativa: lista as Raridades cadastradas, lista valores externos pendentes (staged sem mapeamento correspondente) com ação "Resolver raridade" (Raridade existente ou Nova raridade — ambas culminam nas funções acima), e um botão "Revalidar tudo" que aciona `svc_apply_catalog_import_revalidation()` para todos os jobs elegíveis. Acesso restrito a administradores, mesmo padrão de guarda (`requireCatalogoAdmin`) já usado pelas demais telas do módulo.

Ver `05e-catalogo-editorial.md` (seção "Raridade — mapeamento self-service e revalidação") e `database/schema/2094`–`2106`/`database/migrations/2098`,`2104`,`2110`–`2113` para a implementação completa.

## Emenda (2026-08-08) — Encerramento do canal PDF (Ciclos 3/4)

Decisão explícita de Fabrício: os Ciclos 3 (prova técnica do processador de PDF) e 4 (fluxo PDF completo) **não serão implementados**. O contrato `fonte → processador → linhas de staging` e o Princípio da Fonte Canônica permanecem válidos e aplicáveis a qualquer fonte futura — o que se encerra é especificamente a intenção de construir um processador que interpreta o checklist oficial em PDF, não a arquitetura de ingestão como um todo.

Motivação registrada por Fabrício: o canal TCGdex (Ciclo 2, confirmado executado e validado de ponta a ponta) já cobre a maior parte das Coleções com uma taxa de sucesso considerada suficiente; a direção futura já confirmada de multi-provider (múltiplas fontes externas estruturadas, decisão de escopo de 2026-08-07 — ver `06-pipeline-importacao.md`) tende a fechar as lacunas de cobertura remanescentes com mais confiabilidade do que a extração de layout gráfico de um checklist; e os dois canais manuais já implementados e validados em produção — cadastro individual de Card (`ADR-023`) e importação manual de imagem via UI ou script (`ADR-026`) — já resolvem, ainda que sem automação, o caso residual de uma Coleção fora da cobertura de qualquer fonte estruturada.

Essa decisão foi tomada com base em evidência técnica direta, não apenas em estimativa: uma prova técnica exploratória (fora do escopo deste ADR, não versionada como código de produção) testou extração de número/nome/raridade/acabamento contra dois checklists reais (`ME04_Card_List_PTBR.pdf`, `ME02pt5_Card_List_PTBR.pdf`, ambos em `assets/reference-sources/`). O resultado confirmou que o mecanismo é tecnicamente viável sem OCR ou IA visual — raridade e acabamento são recuperáveis a partir de geometria vetorial (posição e cor) comparada contra a legenda da própria página —, mas também que a robustez cai de forma real entre produtos: o checklist mais simples (`ME04`, 3 colunas, 122 cartas) chegou a 100% de acerto; o checklist maior (`ME02pt5`, 6 colunas, 295 cartas, uma raridade adicional, nomes que quebram em duas linhas) exigiu detecção dinâmica de grade de linhas/colunas por produto para não degradar a precisão — engenharia real, não um ajuste incremental. Esse achado quantifica o custo que motivou a decisão: automatizar o canal PDF exigiria manutenção contínua de calibração por checklist, para um ganho que os canais manuais já existentes cobrem sem automação.

Nenhuma mudança de schema ou de código de produção decorre desta emenda — nenhuma linha de `catalog_import_job`/`catalog_import_row` chegou a ser criada para o canal PDF, e a prova técnica exploratória não foi incorporada ao repositório.
...
- O canal PDF (Ciclos 3/4) não será implementado — encerrado por decisão explícita de Fabrício em 2026-08-08 (ver Emenda acima). Não é uma limitação técnica do contrato de processamento, que continua válido para qualquer fonte futura.
...
| 1.2 | **Emenda (2026-08-07) — "Raridade: mapeamento self-service e revalidação".** Substitui `RARITY_NAME_ALIASES` (mapa hardcoded em `import-catalog-cards/services/normalize.ts`) por `rarity_external_mapping` (Query 2096), tabela administrável via nova tela `/catalogo/raridades` — cadastro de Raridade e de mapeamento passam a existir sem deploy de código (`admin_create_rarity()`/`admin_update_rarity()`/`admin_create_rarity_external_mapping()`/`admin_update_rarity_external_mapping()`/`admin_create_rarity_with_external_mapping()`, Queries 2099–2103). Normalização (`normalize_external_catalog_value()`, Query 2095) compartilhada entre backend SQL e Edge Function via novo módulo `_shared/catalog-normalization/`. Adiciona revalidação sem reimportação — `svc_apply_catalog_import_revalidation()` (Query 2106) + `internal.persist_catalog_import_revalidation()` (Query 2105) recalculam linhas já staged depois de uma correção de mapeamento, preservando `decision_status` já decidido e destravando linhas que haviam falhado só por raridade não mapeada. Backfill (Query 2104) migra os 25 aliases pré-existentes sem mudança de comportamento observável. Motivado por gap real observado em produção: correção de um alias exigia deploy e reimportação completa da Coleção. |
| 1.3 | **Fechamento documental do Ciclo 2 (2026-08-07).** Consolida numa seção própria (`05e-catalogo-editorial.md`, "Ciclo 2 — Fluxo vertical completo via TCGdex") o que estava disperso desde 2026-08-01 nas revisões `1.1`–`1.50` de `05-modelo-de-dados.md` (antes da divisão de 2026-08-06): o processador (`import-catalog-cards`, sem Query SQL própria — reaproveita integralmente `admin_start_catalog_import()`/`admin_decide_catalog_import_row()`/`admin_confirm_catalog_import()` do Ciclo 1), o módulo compartilhado `_shared/catalog-normalization/`, o frontend (`/catalogo/importar-cartas`/`/catalogo/importar-imagens`) e a emenda de continuação automática cartas→imagens (`admin_start_asset_import_run()`, Query `2092` v1.0–v1.3, Migration `2093`). Nova validação `2818`, somente leitura sobre dado real de produção (diferente da `2814` do Ciclo 1, que usou dado sintético porque na época nenhum processador existia) — escrita e apresentada a Fabrício, aguardando execução e confirmação antes de o Ciclo 2 poder ser marcado como formalmente validado. Divergência encontrada durante esta auditoria, sinalizada e não resolvida unilateralmente: o cabeçalho do arquivo canônico da Query `2092` (v1.3) ainda diz "PROPOSTA — AGUARDANDO EXECUÇÃO", contradizendo a Revision History de `05-modelo-de-dados.md` e o uso real em produção (seletor de idioma EN/PT-BR já ativo) — a Query `2818` (item 1) resolve isso diretamente contra o banco. Nesta mesma rodada, decisão explícita de Fabrício: manter "multi-provider" (múltiplas fontes externas estruturadas além da TCGdex) como implementação futura, fora do escopo desta sprint — registrada em `06-pipeline-importacao.md` (revisão `1.7`), não neste ADR; nenhuma das sub-decisões técnicas já listadas ali (providers candidatos, fallback, licenciamento etc.) foi resolvida, só a pergunta de escopo/prioridade. |
| 1.4 | **Bug real corrigido — Query `2082` v1.1 + Migration `2118` (2026-08-07), encontrado pela validação `2818` (item 4).** `admin_confirm_catalog_import()` v1.0 calculava o status final do job filtrando a checagem de pendência só dentro de `decision_status IN ('APPROVED', 'SKIPPED')`, tornando invisíveis linhas com `decision_status = 'PENDING'` (nunca decididas por um administrador — tipicamente `CONFLICT` nascida `PENDING` e nunca revisada) e permitindo `COMPLETED` com decisão humana pendente, violação direta da regra já descrita neste ADR ("o job permanece (ou retorna a) `STAGED`" enquanto houver `decision_status = PENDING` ou `persistence_status = PENDING`). Dois jobs reais afetados em produção (`0a067e94-b665-4d74-b47f-2635d12e22a9`, 9 linhas; `3ea4752c-cf6d-4fb9-8228-224f96c11030`, 1 linha). Um terceiro job investigado na mesma rodada (`bae2f19b-223f-42da-9acd-4283da8fc7b3`, 270 linhas `persistence_status = PENDING`) foi descartado como falso alarme — `decision_status = 'REJECTED'` em todas, comportamento correto por desenho (linha rejeitada nunca entra no laço de gravação, não bloqueia nem deveria bloquear a conclusão do job). Corrigido com uma contagem própria de `decision_status = 'PENDING'`, sem o filtro que escondia o problema, checada antes de qualquer outra condição (Query `2082` v1.1, `CREATE OR REPLACE`, mesma assinatura) e uma migration de reparo retroativo (`2118`) que devolveu os 2 jobs reais para `STAGED`. Ambas confirmadas executadas por Fabrício em 2026-08-07 — validado por query de confirmação, os 2 jobs com `status = 'STAGED'` e as mesmas contagens de linha pendente (`9` e `1`). As 10 linhas ficam disponíveis na tela de Revisão para decisão manual normal, sem prazo. |
| 1.5 | **Fechamento dos 2 jobs reabertos pela emenda `1.4` (2026-08-07, mesma rodada).** Fabrício reportou que as Coleções (SV2, SV5) já não apareciam no seletor de `/catalogo/importar-cartas` mesmo com os 2 jobs em `STAGED`. Causa (não um novo bug, comportamento existente desde 2026-08-01): `getLatestImportJobIncompleteFlags()` só considera o job mais recente de cada Card Set, não qualquer job com linha pendente. Diagnóstico confirmou que, para os dois Card Sets, um job **ainda mais recente** que os dois reabertos já tinha completado a importação inteira com sucesso (SV2: `279/279` linhas processadas; SV5: `218/218`) — ou seja, uma reimportação completa rodou depois dos jobs com bug e já resolveu o Card Set inteiro; as 10 linhas `PENDING` provavelmente correspondem a Cards já cadastradas por essa reimportação posterior, confirmado visualmente por Fabrício na galeria de Cartas para várias delas. **Decisão de Fabrício**: deixar os 2 jobs como estão — são histórico de tentativas obsoletas, sem efeito no catálogo real, nenhuma ação adicional de código ou dado necessária. Não afeta a validade da correção da `1.4`: o bug era real e a Query `2082` v1.1 continua correta para qualquer novo job futuro; este achado só esclarece que, neste caso específico, os 2 jobs afetados já não têm consequência prática pendente. |
| 1.6 | **Query `2818` fechada — Ciclo 2 formalmente validado de ponta a ponta (2026-08-07).** Últimos 3 dos 8 itens (jobs presos em `CONFIRMING`; job confirmado sem auditoria correspondente em `catalog_admin_action_log`; Card Set com job terminal de sucesso sem `card_set_external_reference` ativa) confirmados por Fabrício, todos com 0 linhas — nenhuma divergência nova encontrada, além da já corrigida na emenda `1.4`. Ciclo 2 de `ADR-024` (fluxo vertical completo via TCGdex) atinge o mesmo padrão de validação formal já dado ao Ciclo 1 pela Query `2814`. |
| 1.7 | **Emenda (2026-08-08) — Encerramento do canal PDF (Ciclos 3/4), decisão explícita de Fabrício.** Motivada por três fatores: taxa de sucesso já suficiente do canal TCGdex (Ciclo 2); direção futura de multi-provider (decisão de escopo de 2026-08-07) como caminho de maior confiabilidade para cobrir lacunas remanescentes do que extração de layout gráfico; e os canais manuais já existentes e validados (cadastro individual de Card via `ADR-023`, importação manual de imagem via `ADR-026`) já cobrindo o caso residual sem automação. Decisão embasada por prova técnica exploratória real (fora do repositório) contra dois checklists (`ME04`, `ME02pt5`): confirmou viabilidade técnica sem OCR/IA visual (raridade/acabamento recuperáveis por geometria vetorial e cor comparada contra a legenda da própria página), mas também quantificou custo real de engenharia — checklists maiores/mais densos exigem detecção dinâmica de grade por produto para manter precisão, não um ajuste incremental. Nenhuma mudança de schema/código de produção — nenhuma linha de staging chegou a ser criada para PDF. Contrato de processamento e Princípio da Fonte Canônica permanecem válidos para fontes futuras; apenas a intenção de implementar um processador de PDF é encerrada. |
