...
- Esta emenda cobre exclusivamente `Game`. `Expansion` e `Card Set` **não** recebem `admin_delete_*` por esta emenda — se o mesmo padrão for desejado para eles, é uma decisão própria, a ser registrada quando (e se) o ciclo vertical de cada um chegar a essa necessidade.
- Números de Query: `2041` (adiciona `GAME_DELETED` ao `CHECK` de `catalog_admin_action_log`) e `2042` (`admin_delete_game()`), ambos no milhar `2000`–`2999` já reservado (`STD-001` v1.17 §10) — não reabre a numeração já comunicada para os demais ciclos (Expansion/Card Set/Card permanecem em `2033`–`2040`).
...
- `Game`/`Expansion`/`Card Set` seguem sem caminho de **desativação** (`is_active`) por UI — se isso se tornar necessário, é uma decisão futura própria, fora deste ADR. `Game` passou a ter **exclusão real** via UI (`admin_delete_game()`, emenda 2026-07-26) — as duas coisas são distintas; `Expansion`/`Card Set` não recebem nem uma nem outra por esta emenda.
