...
- `Game`/`Expansion`/`Card Set` seguem sem caminho de **desativação** (`is_active`) por UI — se isso se tornar necessário, é uma decisão futura própria, fora deste ADR. `Game` (`admin_delete_game()`, emenda 2026-07-26), `Expansion` (`admin_delete_expansion()`, emenda 2026-07-31) e `Card Set` (`admin_update_card_set()`/`admin_delete_card_set()`, emenda 2026-07-31) passaram a ter **atualização**/**exclusão real** via UI — distintas de desativação, que nenhuma das três entidades recebe ainda.
