...
- Regras de `card_set` (`database/schema/120_create_card_set_table.sql`) antecipadas como erro administrativo claro: `code` único por Expansion, `release_order` único por Expansion, `set_type` em `REGULAR`/`SPECIAL`/`PROMO`, `total_set_size >= base_set_size`, e — quando `set_type = PROMO` — `base_set_size = total_set_size` (`ck_card_set_promo_size`) e no máximo um `PROMO` por Expansion (`uq_card_set_expansion_promo`).
