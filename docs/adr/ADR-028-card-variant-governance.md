...
Essa lacuna se tornou relevante em 2026-08-09, quando Fabrício decidiu que Card Variant é um bloco intermediário obrigatório entre o Catálogo Editorial e Coleções (`ROADMAP.md`): `Physical Card` (o exemplar físico possuído pelo colecionador; à época deste ADR, `Collection Item`, `ADR-013`; nome vigente desde 2026-08-30, ver `concept-decisions.md` C-47/C-48) referencia uma `Card Variant`, não a `Card` genérica — a identidade colecionável precisa estar correta e sob controle exclusivamente editorial antes que o domínio de posse física seja modelado sobre ela.
...
O colecionador final (usuário autenticado não-administrador) nunca cria, altera, ativa/inativa ou exclui uma `card_variant`. A única interação prevista é **seleção**: ao cadastrar uma Physical Card no futuro Inventário, o usuário escolhe entre as variantes já existentes de uma Card — a variante em si é sempre um dado editorial, mantido exclusivamente por administradores, igual a `Game`/`Expansion`/`Card Set`/`Card`.
...
Rejeitada: Card Variant tem uma implicação de governança específica (é a base de identidade referenciada por Physical Card, futuro domínio de posse do usuário) que justifica registro e rastreabilidade próprios, mesmo reaproveitando integralmente o modelo de `ADR-022`/`ADR-023` — mesmo raciocínio já usado para não emendar `ADR-006` na criação de `ADR-027`.
...
| 1.4 | **Governança da Taxonomia de Card Variant Type — Incremento 3 (Criar novo tipo + Resolver mapping na mesma operação), 2026-08-15.** Fecha o desenho original da revisão `1.2` ("A) mapear para um card_variant_type existente; OU B) criar um novo card_variant_type canônico e então associar o mapping"). `admin_create_card_variant_type_with_import_mapping()` (Query 2158, `SECURITY DEFINER`, admin-only) é um wrapper transacional fino sobre `admin_create_card_variant_type()` (2154, revisão `1.2`) e `admin_resolve_catalog_variant_import_mapping()` (2150, revisão `1.1`) — mesmo padrão já validado por `admin_create_rarity_with_external_mapping` (Query 2103): nenhuma lógica própria, só orquestra as duas chamadas na mesma transação implícita da RPC. `game_id` nunca é parâmetro — resolvido a partir da própria linha de staging (`card → card_set → expansion`), mesma resolução já usada pela 2150, eliminando qualquer possibilidade de criar um tipo num Game diferente do da combinação que originou a ação. Criação de tipo continua decisão 100% explícita do administrador (code/name/description/display_order preenchidos manualmente no Dialog "Resolver mapeamento", nunca inferidos da combinação externa) — reafirma a regra "criar novo Variant Type nunca deve ser automático" do pedido original desta frente. Atomicidade comprovada em dry-run (`BEGIN...ROLLBACK`, 8/8 asserções): uma segunda tentativa com `code` já usado por outra linha falha por inteiro (nenhum tipo órfão criado, linha alvo permanece intocada); `display_order` duplicado também rejeitado; caminho de sucesso cria o tipo, resolve o mapeamento e revalida a linha para `VALID` numa única chamada. Frontend: `ResolverMapeamentoDialog` (`revisao-importacao-variantes-table.tsx`) ganhou um seletor de modo ("Tipo existente" / "Novo tipo canônico"), reaproveitando os campos de `CreateTipoVariacaoForm` (revisão `1.3`); nova Server Action `criarTipoVariacaoEResolverMapeamento()` chama só a RPC. `npm run typecheck`/ESLint limpos. |
| 1.5 | Convergência terminológica (2026-08-30): as quatro referências downstream a "Collection Item" (Decisão, Context, Decision, Alternatives Considered) atualizadas para "Physical Card" — ver `concept-decisions.md` C-47/C-48. Nenhuma decisão de governança de Card Variant foi alterada. |
