...
Fabrício confirmou explicitamente: concluir a importação manual de `MEP` (`60` Cards, `en`+`pt-BR`) é **prioridade mais alta do que iniciar o Ciclo 3 de `ADR-024`** (prova técnica do processador de PDF). Registrado aqui e em `ROADMAP.md`, Trilha 1 — não é uma reordenação de Ciclo 3 em si (que seguia não iniciado, sem cronograma, à época), apenas a confirmação de que esta funcionalidade tinha precedência quando ambas competissem por atenção.

**Nota (2026-08-08): esta comparação de prioridade está superada, não apenas resolvida.** O Ciclo 3 de `ADR-024` não foi apenas adiado — foi encerrado definitivamente por decisão explícita de Fabrício, motivada em parte pela conclusão bem-sucedida deste mesmo canal Manual via UI (emenda abaixo), que já resolve o gap prático que o PDF endereçaria. Ver `ADR-024`, emenda "Encerramento do canal PDF (Ciclos 3/4)", para o racional completo.
...
| 1.2 | Nota adicionada (2026-08-08) à seção "Prioridade confirmada": a comparação de prioridade contra o Ciclo 3 de `ADR-024` está superada, não apenas resolvida — o Ciclo 3 foi encerrado definitivamente (não apenas adiado), motivado em parte pela conclusão bem-sucedida deste canal Manual via UI. Ver `ADR-024`, emenda "Encerramento do canal PDF (Ciclos 3/4)". Nenhuma mudança de código; texto histórico das revisões `1.0`/`1.1` preservado sem alteração. |
