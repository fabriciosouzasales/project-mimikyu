...
### Correção — modo imersivo de pesquisa mobile (revisão `1.5`)

O combobox mobile original (`MobileSearchTrigger`, extinto nesta revisão) abria um `DialogPrimitive.Content` fixo no topo, mas sem substituir de fato o header (menu/identidade/avatar/tema continuavam tecnicamente montados atrás), sem botão "Filtros" externo (só o botão de pesquisa avançada sobreposto dentro do próprio campo, herdado do dropdown desktop), sem texto de orientação antes de digitar, e sem integração com o botão Voltar do navegador. Fabrício apontou a divergência com uma referência funcional explícita: pkmn.gg — "ao tocar na lupa, o próprio header se transforma numa experiência dedicada de pesquisa", rejeitando explicitamente três alternativas (modal central, drawer lateral, expansão simples do campo dentro do header normal), e reforçando que a referência é só de COMPORTAMENTO — "não copiar logotipo, cores ou proporções do pkmn.gg".

**Extração de lógica compartilhada**: `useCardSearchSuggestions()` (novo hook interno de `global-search.tsx`) concentra debounce (280ms), `AbortController`/descarte de resposta obsoleta, seleção de carta e navegação para pesquisa avançada — antes duplicado entre as variantes `inline`/`overlay` de um único `SearchCombobox`. `SearchCombobox` (desktop, dropdown flutuante) e o novo `MobileSearchOverlay` (mobile, painel de tela cheia) consomem o mesmo hook, cada um decidindo só COMO apresentar o resultado — nenhuma duplicação de regra de negócio entre as duas experiências.

**`MobileSearchOverlay`** — reescrito do zero como painel de tela cheia via `@radix-ui/react-dialog` (primitives diretos, não o wrapper `components/ui/dialog.tsx`, que é centralizado): `DialogPrimitive.Content` fixo (`fixed inset-x-0 top-0`), com a mesma altura de linha do header real (`h-14`) na barra superior — visualmente ocupa e substitui a barra do header (nenhuma coordenação de estado com o `Header` Server Component é necessária: o painel simplesmente cobre tudo, com `z-50` acima de header/sidebar/drawer via `Portal`). Estrutura conforme a hierarquia pedida — campo de busca (`flex-1`) + botão "Filtros" (`SlidersHorizontal`, navega para `/pesquisa` ou `/pesquisa?q=`) + botão "Fechar" (`X`) — ambos os botões com toque de 44×44px (`h-11 w-11`), fora do campo (ao contrário do botão sobreposto do dropdown desktop). Painel de sugestões abaixo ocupa o espaço restante (`min-h-0 flex-1 overflow-y-auto`), com rolagem própria — nunca a da página. Altura total do `Content` limitada a `max-h-[100dvh]` (mesma unidade de viewport dinâmico já usada em `AppShell`/`AuthHeroShell`, sem fallback `vh` — sem precedente disso no projeto), que se ajusta automaticamente ao viewport reduzido pelo teclado virtual; `env(safe-area-inset-top/bottom)` aplicado via padding para iPhones com notch/Dynamic Island e barra de gestos.

Antes de haver termo digitado, o painel mostra uma orientação curta ("Pesquise pelo nome, número ou código do Card Set.") em vez de ficar vazio e desproporcional — estado novo (`phase === "hint"`), que não existia no dropdown desktop (que simplesmente permanece fechado até haver termo). Estados de carregamento/erro/vazio/resultados compartilhados via o mesmo hook, cada item de sugestão com altura mínima de 44px de toque; região `aria-live="polite"` oculta anuncia a quantidade de resultados a cada mudança de status, para leitor de tela.

**Fechamento e histórico**: além de botão `X`, `Escape` (comportamento nativo do Radix Dialog — propositalmente não interceptado no campo, diferente do combobox desktop, que primeiro fecha só sua lista local antes do Escape poder fechar algo maior) e navegação concluída, um `history.pushState` marcador (mesma URL, só um `state` extra) é empilhado ao abrir, especificamente para que o botão Voltar do navegador feche o modo imersivo em vez de sair da página — um `popstate` listener intercepta e fecha sem navegação real. Ao fechar sem navegar, a marca é desfeita via `history.back()`; ao fechar navegando para `/pesquisa`, via `history.replaceState` (evita desfazer a navegação que está prestes a acontecer). Guarda explícita contra o duplo-mount do React Strict Mode (dev) evita empilhar dois marcadores ao abrir uma vez só. `Radix Dialog` cobre, sem código adicional, foco automático no campo ao abrir, restauração de foco no botão que abriu ao fechar, bloqueio de scroll do `body`, e fechamento ao tocar fora (backdrop) sem conflitar com teclado/lista/acessibilidade (a área "fora" nunca inclui campo, botões ou lista, todos dentro do `Content`).

Clicar numa sugestão continua navegando para `/pesquisa?card=<uuid>&q=<termo>` — nunca abre o preview ampliado diretamente (confirmado: `pesquisa-view.tsx` só usa `card` para fixar/priorizar a carta nos resultados via `search_cards()`, nunca para abrir `zoomCard` automaticamente).

Nova animação dedicada (`tailwind.config.ts`): `search-panel-in`/`search-panel-out` (fade + leve deslize vertical, `translateY(-8px)→0`) — deliberadamente distinta de `drawer-in`/`out` (deslize lateral, usado pelo menu mobile) e `dialog-in`/`out` (zoom centralizado), porque o painel de pesquisa não é nem um drawer lateral nem um modal centralizado.
...
- **Divergência pré-existente de rastreabilidade em `database/migrations/` — parcialmente reconciliada em 2026-08-17** (descoberta durante a introspecção deste incremento, não causada por ele): a faixa `3000`–`3090` (Pricing P1–P6, `ADR-029`) e a migration `2050` estavam confirmadamente aplicadas no Supabase (`list_migrations`), mas sem arquivo `.sql` correspondente no repositório. Na direção oposta, `2098`–`2153` (~18 arquivos) existem no repositório mas não aparecem no histórico interno do Supabase (`supabase_migrations.schema_migrations`) — o schema real bate com o que esses arquivos criam, sugerindo aplicação por execução direta em algum momento, fora do mecanismo que grava histórico. Sinalizado a Fabrício em chat nesta sessão; decisão explícita dele foi prosseguir com este incremento e registrar a pendência, sem reconciliar tudo naquele ciclo. **Em ciclo posterior, mesma data, a pedido explícito de Fabrício, a faixa `3000`–`3090` foi reconciliada retroativamente**: os 22 arquivos ausentes extraídos verbatim de `supabase_migrations.schema_migrations` (somente leitura) e versionados em `database/migrations/`, cada um verificado por `md5()` contra o statement armazenado (22/22 correspondências exatas), com cabeçalho de reconstrução informando origem e data. `2050` e `2098`–`2153` seguem não reconciliadas. Ver `docs/log.md` e `docs/development/HANDOFF-2026-08-17.md`, seção 4.
...
- **Modo imersivo de pesquisa mobile (revisão `1.5`) não foi testado em dispositivo/navegador real** — mesma limitação de sandbox sem navegador; `npx tsc --noEmit` confirmado limpo, mas o comportamento de `100dvh` com o teclado virtual aberto, `env(safe-area-inset-*)` em iPhones com notch/Dynamic Island, o listener de `popstate` (botão Voltar físico/gesto) e o Safari iOS especificamente (motor de renderização distinto do Chrome/Chromium) exigem validação manual de Fabrício antes de considerar a frente encerrada.
...
| 1.5 | **Reescrita — modo imersivo de pesquisa mobile (2026-08-17, mesma data, referência funcional pkmn.gg pedida por Fabrício).** `MobileSearchTrigger` (Dialog fixo sem substituir de fato o header, botão de filtro só sobreposto no campo, sem orientação antes de digitar, sem integração com Voltar) substituído por `MobileSearchOverlay`: painel de tela cheia que ocupa a barra do header (`h-14`), campo + botão "Filtros" + botão "Fechar" externos (toque ≥44px), painel de sugestões com rolagem própria (`max-h-[100dvh]`, `env(safe-area-inset-*)`), orientação textual antes do termo mínimo, `aria-live` anunciando quantidade de resultados, fechamento por `X`/Escape/navegação/clique fora/botão Voltar (via `history.pushState` marcador + `popstate`, com guarda contra duplo-mount do Strict Mode). Lógica de busca (debounce, `AbortController`, seleção, navegação) extraída para `useCardSearchSuggestions()`, compartilhada com o `SearchCombobox` desktop (inalterado no comportamento) — elimina a duplicação que antes existia via prop `variant`. Nova animação dedicada `search-panel-in`/`out` em `tailwind.config.ts`. Clique em sugestão continua navegando para `/pesquisa`, nunca abre o preview ampliado diretamente. Identidade visual MMKYU integralmente preservada — pkmn.gg foi referência só de comportamento, não copiado visualmente. Ver seção "Correção — modo imersivo de pesquisa mobile". Teste em dispositivo/navegador real pendente (sandbox sem navegador). |
