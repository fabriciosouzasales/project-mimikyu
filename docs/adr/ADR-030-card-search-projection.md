...
`web/lib/pesquisa/format.ts` duplica deliberadamente 3-4 funções puras de formatação já existentes (não exportadas) em `components/catalogo/cartas-gallery.tsx` (`cartaFullNumber`, prioridade de imagem PT-BR→EN) — pequena duplicação preferível a acoplar `/pesquisa` (rota de produto) à galeria administrativa, ou a refatorar a galeria inteira dentro deste incremento. O zoom de carta em `/pesquisa` usa o `Dialog` genérico já existente (`components/ui/dialog.tsx`), não a transição View Transitions específica de `CartaZoomDialog` (mecanismo interno da galeria admin, não extraído).
...
- O zoom de carta em `/pesquisa` é um `Dialog` simples, não a transição View Transitions da galeria administrativa — ver seção "Compatibilidade".
