...
1. **UX North Star.** O usuário escolhe a **Position**; o sistema encontra as Physical Cards elegíveis. A direção do fluxo é Position → Card, não Card → Position.
2. **Allocation ≠ Position Assignment.** Alocar uma Physical Card a uma Collection (`collection_allocation`) e vinculá-la a uma Position da Pokédex (`collection_pokedex_position_assignment`) são atos distintos. Allocation sozinha nunca satisfaz uma Position.
3. **Match normal não exige confirmação adicional.** Depois que o usuário seleciona uma Physical Card elegível para a Position, o sistema registra a Assignment com `assignment_basis = SPECIES_MATCH` sem warning ou confirmação adicional. A seleção da Card e a Assignment continuam sendo atos explícitos; **não existe auto-assignment.**
4. **Escolha deliberadamente incompatível exige confirmação explícita.** O usuário pode vincular uma Card cuja Species não corresponde à da Position — mas apenas com confirmação explícita, registrada como `USER_OVERRIDE`.
...
8. **Estreitar o Scope preserva as Assignments que ficaram de fora.** Nada é destruído.
