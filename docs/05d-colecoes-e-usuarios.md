...
**Pokémon Species/Generation físicas desde 2026-09-04** (`pokemon_generation`, `pokemon_species`, `pokemon_species_external_reference` — Queries `6000`–`6021`/`6700`, ver `docs/standards/STD-001-database-standards.md`). Ainda não iniciados: Pokémon Form/Variety, Pokédex Position, Position Assignment, Primary Representative, e o pipeline de ingestão PokéAPI/TCGdex/reconciliação editorial. Próximo checkpoint: **POKEDEX POSITION PHYSICAL MODELING**. Sem bloqueio de nenhuma decisão já tomada.
