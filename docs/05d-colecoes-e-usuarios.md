...
**`IMPLEMENTED / VALIDATED / PERFORMANCE HEALTHY / CLOSED` (2026-09-07).** Décima primeira fundação física de Coleções, e a última antes da trilha de UX/frontend. Aplicada ao banco real, validada funcionalmente e medida em performance. **Ainda não promovida** para `database/schema/` — os arquivos vivem em `database/proposals/2026-09-06-binder-layout-foundation/`, com o registro de execução no README daquela pasta (§10-bis e §10-ter).
