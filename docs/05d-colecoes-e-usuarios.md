...
Aplicadas ao banco real na ordem `2159`→`6112`→`6113`→`6114`→`6115`→`6116`; postcheck estrutural/segurança e 26 cenários de teste funcional (`BEGIN...ROLLBACK`) confirmaram PASS. Edge Function `revalidate-catalog-import-rows` reimplantada com sucesso no projeto `qjfutqujxrbzgrtkpgkg` (versão 9, `ACTIVE`) contendo o novo Fluxo B. As 6 Queries promovidas para `database/schema/`; `database/proposals/2026-09-05-fatia-c-card-primary-species/` e `database/proposals/2026-09-05-fatia-c-incremental-integration/` mantidas integralmente como evidência histórica de staging. Nenhuma decisão conceitual/lógica reaberta. **Fatia C concluída.**

> **Ressalva de proveniência (levantada em 2026-09-12).** A afirmação original desta seção — *"corpo SQL byte-idêntico ao executado"* — **não se sustentou** para a `6116`: em 2026-09-12 o `prosrc` LIVE media `md5 9d6cd767…` / 3005 B contra `md5 fddacfc4…` / 4209 B do arquivo canônico (diferença essencialmente de linhas de comentário internas). O corpo histórico exato não está mais disponível e **nenhuma equivalência byte-level retroativa é alegada**. Dívida documental/de proveniência registrada — **não** classificada como defeito funcional. Verificar se outras funções promovidas divergem do canônico exige mandato próprio. Detalhe completo em `database/proposals/2026-09-12-primary-species-6116-timeout-fix/README.md`.

### Correção `6128` — escopo da CTE `evidence` por `p_job_id` (2026-09-12)

**CLOSED / PASS** (`PRIMARY-SPECIES-INCREMENTAL-HOOK-AUDIT-01` → `-FIX-01 GATE A` → `APPLY-01` (STOP por md5) → `RECOVERY-01` → `VALIDATION-01` (STOP por fixture) → `VALIDATION-FIX-01` → `PROMOTION-CLOSEOUT-01`).

**Defeito real, comprovado.** Três jobs TCGDEX chegaram a `COMPLETED` com Primary Species = 0 — `SMP`, `SM10`, `BW10`. Causa registrada pelo próprio caller no CSV de `web/scripts/bootstrap-cards/out/`: `primary_species falhou (nao bloqueante): canceling statement due to statement timeout`. Exatamente 3 ocorrências em toda a série — nenhum caso oculto. A CTE `evidence` de `6116` não repetia `r.job_id = p_job_id`, e `catalog_import_row` não tem índice em `resulting_card_id`: o planner varria a tabela inteira a cada chamada, estourando o `statement_timeout = 8s` do papel `authenticated`.

**Correção.** Uma cláusula — `AND r.job_id = p_job_id`. Equivalência funcional provada sobre os **199 jobs TCGDEX**: 18.234 pares `(job, card)` nas duas formas, **0 divergências**. Performance no job do SM10: **7663 ms → 262 ms (~29×)**, `Seq Scan` global eliminado, ambos os acessos a `catalog_import_row` via `ix_catalog_import_row_job`. **Nenhum índice novo. Nenhum caller alterado** — os três seguem não bloqueantes.

**Validação `6841`**: 17 PASS / 0 FAIL / 0 NOT PROVEN, fixtures integralmente revertidas. Permanece em `proposals/` como prova; **não** é promovida. `6128` promovida para `database/schema/` (**fonte canônica**); a cópia em `proposals/` permanece como **evidência histórica** da rodada de staging. Os cabeçalhos das duas divergem de propósito — cada um declara o próprio papel; o que é byte-idêntico entre proposal, schema e o objeto LIVE é o **corpo SQL executável** (`md5 7f2e7de8…` / 4539 B), não o arquivo inteiro.

**Cobertura global de Primary Species em 2026-09-12 (antes do sourcing ME*):** 16.657 / 17.536 = **94,99 %**. Residual de **879**, em duas classes distintas: **760 sem evidência durável persistida** (752 Cards ME* — sem `catalog_import_job`, com `external_card_id` presente, logo recuperáveis por sourcing; 8 Cards SVP manuais sem import row nem external reference) e **119 ambiguidades reais** (Cards multi-espécie — Tag Team GX, LEGEND, `Buried Fossil` — que exigem decisão editorial via `6114`). **PRIMARY SPECIES — INCREMENTAL HOOK = CLOSED / PASS.** O residual de 879 foi tratado na subseção seguinte.

### Sourcing ME* e fechamento de Primary Species (2026-09-12)

**CLOSED** (`PRIMARY-SPECIES-ME-SOURCING-01`: `AUDIT-01` → `BACKFILL-GATE-A-01`/`-REVISION-01`/`-FINAL-CHECK-01` → `6129` → `6842` Gate A → `6130` → `6842` Gates B/C → `DOCUMENTATION-CLOSEOUT-01`).

**Estado final:** Primary Species = **17.409 / 17.536 = 99,28 %**. Residual **127**. Baseline antes desta frente: **879** pendentes (752 ME* + 8 SVP + 119 ambiguidades) → **879 → 127**.

Os seis Card Sets ME* (`ME1`, `ME2`, `ME2.5`, `ME3`, `ME4`, `MEP`) passaram de 0 % para **752/752 = 100 %**:

- **751 `AUTOMATIC_DEXID`** — Query `6129`, chamada única a `6115` com payload de 751 itens, a partir de evidência TCGdex congelada de 752 pares `(external_card_id, dexId)` lida em 2026-09-12 pelo contrato já existente do projeto (`/v2/graphql` e `/v2/{lang}/cards/{id}`). Nenhuma integração nova, nenhuma tabela criada (nem temporária).
- **1 `EDITORIAL_RECONCILIATION`** — Query `6130`, `me01-086` ("Mega Absol ex") → **Absol / National Dex 359**, via `admin_resolve_card_primary_species()` com responsável nomeado. Primeira e única resolução editorial do acervo.

**`me01-086` — `SOURCE_DATA_ERROR` da TCGdex.** A fonte publica, para esta Card, `name = "Mega Absol ex"` e `dexId = [351]` (Castform) — as duas vias da própria fonte concordam entre si e contradizem o nome que ela mesma publica. O dado **observado (351)** foi preservado em `source_evidence.observed_dex_id`, ao lado da **decisão editorial (359)** em `resolved_national_dex`. O `351` permanece também na evidência congelada da `6129`, protegido pelo guard `G3b`. Em nenhum momento o payload automático da `6115` foi falsificado com 359.

**Residual de 127 — `KNOWN / INTENTIONAL UNRESOLVED`, `DEFERRED TO FUTURE EDITORIAL UI`.** Composição: **8 SVP** sem evidência durável suficiente e **119 ambiguidades históricas reais**. Decisão de produto de 2026-09-12: **não serão tratados agora**; ficam deliberadamente preservados como massa real de teste da futura funcionalidade editorial do frontend. **Não bloqueiam Card Variants.**

#### Regra de domínio permanente

> **Coverage nunca prevalece sobre correctness.**

`UNRESOLVED` / `AMBIGUOUS` é **estado válido** do modelo, não defeito. Uma Card só recebe associação automática quando a evidência é **determinística**.

Duas classes distintas, que nunca devem ser confundidas:

| Classe | Significado | Tratamento |
|---|---|---|
| **`SOURCE_DATA_ERROR`** | a Species é conhecida com segurança, mas a fonte externa publicou evidência errada | resolução editorial via `6114`, preservando a evidência errada ao lado da decisão. **Nunca** falsificar payload/evidência automática |
| **`AMBIGUOUS` / `UNRESOLVED`** | a Species **não pode** ser determinada com segurança | permanece sem associação até futura decisão editorial |

Os 127 atuais pertencem integralmente à **segunda** classe.

**Artefatos.** `6129` e `6130` promovidas para `database/migrations/` (transformações de **dados**, não definição canônica de schema — por isso não vão para `database/schema/`); `6842` (validação: manifesto + Gates A/B/C) permanece **somente** em `database/proposals/2026-09-12-primary-species-me-sourcing/`, junto com as cópias de staging de `6129`/`6130` como evidência histórica da rodada. Gates finais: **B1–B10 = 10/10 PASS**, **C1–C7 = 7/7 PASS**, duplicidade global de Primary Species = 0, universo elegível POKEMON = 17.536 inalterado.

**PRIMARY SPECIES = CLOSED** para esta etapa, salvo defeito material novo.
...
10. **Binder Slot ≠ Pokédex Position Assignment.** São relações de domínios diferentes e não foram confundidas nem unificadas. Desde 2026-09-07 o Binder é físico (ver a seção "Binder / Layout Foundation", adiante): `collection_layout_slot_assignment` aponta para `collection_allocation`, enquanto `collection_pokedex_position_assignment` aponta para uma Position do Pokédex — tabelas, invariantes e RPCs inteiramente distintos.

## Pendências / Próximos Passos

**Pokémon Species/Generation físicas desde 2026-09-04** (`pokemon_generation`, `pokemon_species`, `pokemon_species_external_reference` — Queries `6000`–`6021`/`6700`, ver `docs/standards/STD-001-database-standards.md`). **`pokedex`/`pokedex_position`/`pokedex_external_reference` físicas desde 2026-09-04** (Fatia A, Queries `6030`–`6051`) e **`pokemon_region`/`pokemon_region_external_reference`/`pokemon_generation.main_region_id` físicas desde 2026-09-04** (Pokémon Region Foundation, Queries `6060`–`6080`, ver subseção acima). **Estas tabelas não estão mais vazias**: o Initial Load do Pokémon Catalog via PokéAPI foi executado e fechado em 2026-09-05 — `11` Regions, `9` Generations, `1025` Species, `1` National Pokédex e `1025` Positions, com idempotência confirmada por segundo ciclo real. Contagens, `snapshot_hash`, run lifecycle e critérios PASS são canônicos em `docs/06a-pokemon-catalog-sourcing.md`; não duplicados aqui. **`collection_pokedex_reference`/`collection_pokedex_scope_generation` físicas e IMPLEMENTED/VALIDATED/CLOSED desde 2026-09-05** (Fatia B, Queries `5085`–`5099`, ver subseção acima) — 0 linhas cada, nenhuma Collection Pokédex real criada ainda. **`card_primary_species` física e CONFIRMADO EXECUTADO desde 2026-09-05** (Fatia C, Queries `2159`/`6112`–`6116`, ver subseção acima) — `5675`/`6435` Cards POKEMON resolvidas automaticamente (88,19%), `760` pendentes editoriais. **`collection_pokedex_position_assignment`/`collection_pokedex_position_primary_representative` físicas e IMPLEMENTED/VALIDATED/CLOSED desde 2026-09-06** (Fatia D, Queries `6117`–`6126`, ver subseção acima) — 0 linhas em produção (nenhuma Collection Pokédex real ainda). **`collection_completion_summary()` (ramo `REFERENCE_POSITION`) e `collection_pokedex_scope_positions()` vivas e IMPLEMENTED/VALIDATED/PERFORMANCE-MEASURED/CLOSED desde 2026-09-06** (Fatia E, Queries `5100`–`5103`, ver subseção acima) — nenhuma tabela/coluna/índice novos; 0 linhas em produção (nenhuma Collection Pokédex real ainda). Ainda não iniciado: Pokémon Form/Variety. **Binder/Layout Foundation `CLOSED` desde 2026-09-07** (ver a seção "Binder / Layout Foundation", adiante). **`PRICING-PAYLOAD-CARDINALITY-HARDENING-01` `CLOSED` desde 2026-09-07** (Query `3972`, harness `3860` v1.1 em 18/18 — ver `05f-pricing.md`). **Próxima frente do projeto: Bulk Collection Operations**, sem gate pendente à frente. Sem bloqueio de nenhuma decisão já tomada.

---

# Binder / Layout Foundation

## Status

**`IMPLEMENTED / VALIDATED / PERFORMANCE HEALTHY / CLOSED` (2026-09-07).** Décima primeira fundação física de Coleções, e a última antes da trilha de UX/frontend. Aplicada ao banco real, validada funcionalmente e medida em performance. **PROMOVIDA para `database/schema/` em 2026-09-08** (`SCHEMA-PROMOTION-RECONCILIATION-01`): `5104`–`5136` como arquivos individuais e `5137` com o corpo efetivo da correção `5141` foldada — `5141` **não** recebe arquivo próprio em `schema/`, seguindo a representação de uma definição por objeto (mesmo padrão de `5039`×`5048`, `5046`×`5064`, `5057`×`5092`). O hardening `5138`–`5140` foi foldado em `5024`/`5046`/`5047`. Os arquivos históricos permanecem em `database/proposals/2026-09-06-binder-layout-foundation/` e `.../2026-09-06-bulk-payload-cardinality-hardening/`, com o registro de execução nos READMEs daquelas pastas (§10-bis e §10-ter no do Binder); harnesses `5818`/`5819`/`5820` e o roteiro de concorrência **não** são promovidos.

## Modelo físico — objetos principais

Seis tabelas, todas `owner = postgres`, com RLS habilitado, uma policy por tabela e `SELECT` negado a `anon`:

| Tabela | Papel |
|---|---|
| `collection_layout` | o Binder de uma Collection: grid `1..10 × 1..10`, `0..1` por Collection |
| `collection_layout_page` | páginas do Binder, ordenadas por `page_number` sob UNIQUE DEFERRABLE (permite reorder em massa num único `UPDATE`) |
| `collection_layout_slot` | posições da grade dentro de uma Page, com `locked` |
| `collection_layout_slot_expected_content` | "esta posição espera esta carta" — o Slot planejado, ainda vazio |
| `collection_layout_slot_assignment` | a carta que ocupa o Slot; aponta para `collection_allocation` |
| `collection_layout_region` | agrupamento retangular de Slots numa Page (bounding box, sem GiST/EXCLUDE) |

Quinze RPCs `SECURITY DEFINER` com `search_path = ''`, `EXECUTE` para `authenticated` e negado a `anon` — Queries `5122`–`5137`. `assert_collection_layout_mutable()` (`5122`) é helper interno e **não** concede `EXECUTE` a `authenticated`, por desenho: centraliza a ordem de lock canônica **COLLECTION → LAYOUT** e a mensagem de não-enumeração (inexistente e de-outro-dono produzem a mesma mensagem).

Triggers de integridade estrutural: imutabilidade do grid quando existem Pages (`5107`), limites da grade para Slot (`5112`), integridade de Assignment (`5117`), enforcement de Lock (`5118`), integridade e overlap de Region (`5121`), e imutabilidade das FKs estruturais (`5136`).

## `5137` histórico × `5141` definição efetiva

`set_collection_layout_grid()` foi aplicada por `5137` com um defeito real: em PL/pgSQL os nomes de saída de um `RETURNS TABLE` são variáveis visíveis no corpo, e o `WHERE id = p_layout_id` do `UPDATE` colidia com a variável de saída homônima, abortando com `42702 column reference "id" is ambiguous`. Enquanto vigorou sozinha, a RPC estava **quebrada** — nenhuma alteração de grid era possível. O defeito foi **descoberto em runtime pelo próprio harness `5818`**, não por leitura de código.

Correção por migration incremental **`5141`** (`CREATE OR REPLACE`), que qualifica a coluna:

```sql
UPDATE public.collection_layout AS l
   SET grid_rows = p_grid_rows, grid_columns = p_grid_columns
 WHERE l.id = p_layout_id;
```

Assinatura, retorno, `SECURITY DEFINER`, `search_path = ''`, grants e regras de domínio inalterados. **`5137` não é editada** — permanece como definição histórica aplicada; `5141` é a definição efetiva. Mesma classe e mesma disciplina de `6109` (RETURNING ambíguo) e `6126` (ON CONFLICT ambíguo).
...
Os `205` do `5818` e os `28` do `5820` são contagens de **rótulos estáticos**, que incluem rótulos de ramo mutuamente exclusivo; os alvos de runtime alcançáveis são `196` e `27`. Zero resíduo confirmado por postcheck pós-execução em todos os três.

`5819` respeitou a regra de ouro do harness: **nenhum índice novo foi criado**. Os lookups por `page_id` usam o prefixo da UNIQUE de posição, e isso está registrado como resultado esperado, não como pendência. `W12` provou que a existência de um Layout **não altera nem degrada** o cálculo de Completion já LIVE — payload idêntico, cardinalidade idêntica, buffers `1342 → 1342`.

### `C04` / `R18` — concorrência provada externamente

Dois casos do `5818` exigem **duas sessões persistentes e simultâneas** e por isso ficaram `NOT PROVEN` no canal automatizado, nunca convertidos artificialmente em PASS. A prova foi executada manualmente (roteiro `CONCURRENCY-PROOF-C04-R18.sql`, duas sessões mutantes + uma observadora somente-leitura):

- **`C04` PASS** — serialização real COLLECTION → LAYOUT: a sessão B bloqueou, `pg_blocking_pids(B)` apontou o PID de A, B só prosseguiu após o `COMMIT` de A, e o estado final do Slot ficou `locked = false`.
- **`R18` PASS** — overlap concorrente de Region: B bloqueou, `pg_blocking_pids(B)` apontou A, e após o `COMMIT` de A a sessão B falhou com o erro de sobreposição; `regions_na_page = 1`.

Fixture `__C04R18__` com zero resíduo; role temporária removida.

## Hardening de cardinalidade de payload `UUID[]` (`5138`–`5140`)

Aplicado na mesma rodada, sobre `set_physical_cards_storage()` (`5024`), `allocate_physical_cards_to_collection()` (`5046`) e `deallocate_physical_cards_from_collection()` (`5047`). O teto de 500 itens era medido por `array_length(x, 1)`, que só enxerga a primeira dimensão: `array_fill(uuid, ARRAY[2,400])` tem `array_length(x,1) = 2` (passa) e `cardinality(x) = 800` (é o que o `unnest` processa). Passou a valer `cardinality()` + `array_ndims(...) <> 1`, avaliados **antes** de qualquer `unnest`/`DISTINCT`/resolve de ownership/lock/escrita, com **zero `array_length`** no corpo executável. As migrations históricas não foram editadas.

## Escopo da afirmação de segurança

A classe do bypass multidimensional está fechada **nas RPCs do escopo desta implementação**: as três bulk (`5138`–`5140`) e as quatro do Binder com parâmetro `uuid[]` (`reorder_layout_pages`, `clear_slot_expected_content`, `set_slot_lock`, `remove_slot_assignment`). **Não se afirma ausência do bypass em todo o sistema** — ver a pendência em `05f-pricing.md`.

## Pendências / Próximos Passos

- ~~**Promoção para `database/schema/`** ainda não autorizada.~~ **RESOLVIDO em 2026-09-08** (`SCHEMA-PROMOTION-RECONCILIATION-01`): `5104`–`5136` promovidos individualmente, `5137` promovido com o corpo de `5141` foldado, e `5138`–`5140` foldados em `5024`/`5046`/`5047`. Corpo executável byte-idêntico ao aplicado, provado por SHA-256; nenhum SQL executado nessa reconciliação.
...
- **Próxima frente canônica: `CATALOG-HISTORICAL-BOOTSTRAP-03` — etapa restante = `CARD VARIANTS — HISTORICAL BULK IMPORT`.** *(Reconciliação operacional, 2026-09-18, `BASEP + BASE3 — EDITORIAL CONVERGENCE DOCUMENTATION CLOSEOUT-01`: a descrição anterior — "CARDS + ASSETS PT-BR/EN" — está **desatualizada operacionalmente**. Cards e Assets históricos já foram majoritariamente carregados; o que resta da frente é **Variants**. Baseline LIVE datado: 200 Card Sets / 20.941 Cards / 30.261 Card Assets / 7.671 Card Variants; **169 Sets elegíveis** (170 com Cards e zero Variants, menos `ME5.5`, fora do denominador operacional) e **16.705** Cards. Os 4 Sets sem Cards e os Assets residuais são `KNOWN SOURCE-COVERAGE RESIDUALS / NON-BLOCKING`. A `CARD-VARIANTS — EDITORIAL CONVERGENCE` está `CLOSED`. Detalhamento em [`ROADMAP.md`](ROADMAP.md) e [`05b-cartas-e-raridade.md`](05b-cartas-e-raridade.md) revisão `1.4`.)* `CATALOG-HISTORICAL-BOOTSTRAP-01` (Expansions) e `-02` (Card Sets) já fecharam — 17 Expansions e **199 Card Sets** Pokémon, com índice `uq_card_set_expansion_promo` presente no banco físico e `release_order` definitivo; ver `05a-catalogo-base.md`, seção "Bootstrap histórico do catálogo de Card Sets". Sequência macro congelada restante: `CATALOG-HISTORICAL-BOOTSTRAP-03` → `CATALOG-VARIANT-DEFAULT-BACKFILL-01` → `BULK-04` → `BULK-05` → `BULK-06`.
...
| 1.25 | **Correção de estado corrente (2026-09-10, `CATALOG-HISTORICAL-BOOTSTRAP-02-CARD-SETS-CLOSEOUT-01`). Nenhuma decisão nova, nenhum SQL.** A linha "Próxima frente canônica" ainda apontava para `CATALOG-HISTORICAL-BOOTSTRAP` genérico como "ainda não modelada" — mas `-01` (Expansions) e `-02` (Card Sets) fecharam em 2026-09-10 (17 Expansions; 199 Card Sets; índice `uq_card_set_expansion_promo` live no banco físico via Query `2161`; `release_order` definitivo via Query `2162`; gate `921` 14/14). A próxima frente passa a ser **`CATALOG-HISTORICAL-BOOTSTRAP-03` — CARDS + ASSETS PT-BR/EN**, e a sequência macro congelada foi atualizada no mesmo ponto. Também corrigida a divergência pré-existente entre o campo **Versão** do cabeçalho (que estava em `1.22`) e a última linha desta tabela (`1.24`). Detalhe normativo em `05a-catalogo-base.md`, revisão `1.1`. |
| 1.26 | **`PRIMARY SPECIES` encerrado — sourcing dos Card Sets ME* (2026-09-12, `PRIMARY-SPECIES-ME-SOURCING-01 / DOCUMENTATION-CLOSEOUT-01`).** Nova subseção "Sourcing ME* e fechamento de Primary Species" sob "Card → Primary Species (Fatia C)". Cobertura **16.657 → 17.409 / 17.536 = 99,28 %**; residual **879 → 127**; Card Sets ME* de 0 % a **752/752 = 100 %** (751 `AUTOMATIC_DEXID` pela Query `6129`, a partir de evidência TCGdex congelada de 752 pares lida pelo contrato já existente do projeto; 1 `EDITORIAL_RECONCILIATION` pela Query `6130`, `me01-086` → Absol / National Dex 359). Gates `B1`–`B10` **10/10 PASS** e `C1`–`C7` **7/7 PASS**; duplicidade global de Primary Species = 0; universo elegível POKEMON = 17.536 inalterado. `me01-086` registrado como **`SOURCE_DATA_ERROR` da TCGdex** (fonte publica `Mega Absol ex` com `dexId 351`/Castform): o `351` observado ficou preservado ao lado da decisão editorial, e o payload automático da `6115` nunca foi falsificado. Os **127** residuais passam a **`KNOWN / INTENTIONAL UNRESOLVED`** e **`DEFERRED TO FUTURE EDITORIAL UI`** por decisão de produto — 8 SVP + 119 ambiguidades históricas, preservados de propósito como massa real de teste da futura UI editorial, **não bloqueantes para Card Variants**. Consolidada como **regra de domínio permanente**: *coverage nunca prevalece sobre correctness*; `UNRESOLVED`/`AMBIGUOUS` é estado válido; associação automática só com evidência determinística; `SOURCE_DATA_ERROR` (Species conhecida, fonte errada) nunca se confunde com ambiguidade (Species indeterminável). O parágrafo anterior de cobertura (16.657 / 94,99 % / residual 879) foi mantido, apenas marcado como "antes do sourcing ME*". `6129`/`6130` promovidas para `database/migrations/` (transformações de dados, não schema); `6842` permanece só em `proposals/`. Nenhum ADR/Standard criado nesta rodada. Ver `06a-pokemon-catalog-sourcing.md` revisão `1.6` (causa-raiz de sourcing) e `docs/log.md`. |
| 1.27 | **Reconciliação operacional do `CATALOG-HISTORICAL-BOOTSTRAP-03` (2026-09-18, `BASEP + BASE3 — EDITORIAL CONVERGENCE DOCUMENTATION CLOSEOUT-01`).** Único ponto alterado: o primeiro item de "Pendências / Próximos Passos" descrevia a próxima frente canônica como "CARDS + ASSETS PT-BR/EN" — descrição **desatualizada operacionalmente**, já que Cards e Assets históricos foram majoritariamente carregados. A etapa restante passa a ser explicitamente **`CARD VARIANTS — HISTORICAL BULK IMPORT`**, com baseline LIVE datado (200 Sets / 20.941 Cards / 30.261 Assets / 7.671 Variants; **169** Sets elegíveis / **16.705** Cards, `ME5.5` fora do denominador) e os 4 Sets sem Cards + Assets residuais como `KNOWN SOURCE-COVERAGE RESIDUALS / NON-BLOCKING`. Registra também que a `CARD-VARIANTS — EDITORIAL CONVERGENCE` está `CLOSED`. Nenhuma outra seção tocada; nenhum SQL executado. |
