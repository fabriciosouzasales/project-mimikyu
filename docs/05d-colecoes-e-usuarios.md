...
**Cobertura global de Primary Species em 2026-09-12:** 16.657 / 17.536 = **94,99 %**. Residual de **879**, em duas classes distintas: **760 sem evidência durável persistida** (752 Cards ME* — sem `catalog_import_job`, com `external_card_id` presente, logo recuperáveis por sourcing; 8 Cards SVP manuais sem import row nem external reference) e **119 ambiguidades reais** (Cards multi-espécie — Tag Team GX, LEGEND, `Buried Fossil` — que exigem decisão editorial via `6114`). **PRIMARY SPECIES — INCREMENTAL HOOK = CLOSED / PASS. PRIMARY SPECIES GLOBAL permanece EM ABERTO** enquanto houver residual.
