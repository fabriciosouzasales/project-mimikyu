...
> **Objetivo**: criar o serviço capaz de consultar um Set na TCGdex a partir de `card_set_external_reference`, com escopo final definido: `run_code` → `asset_import_run` → `card_set` → `card_set_external_reference` → (se existir) TCGdex → JSON do Set.
> **Critério de aceite**: `services/tcgdex.ts` criado, sem erros no editor; comportamento `NO_EXTERNAL_SET_MAPPING` definido para Sets sem referência externa.
> **Resultado**: 🟨 Em andamento. Arquivo criado e validado; ainda não integrado a `index.ts`, não deployado, sem chamada real testada.
> **Pendências descobertas**: mapeamento de `card_set` para identificador externo não existia — resolvido por uma extensão pontual do Bloco A (`card_set_external_reference`, ver `05-modelo-de-dados.md`), não por uma sprint do Bloco B. Um mapeamento de teste incorreto (`ME0`→`sv10pt5`) foi inserido e corrigido durante a validação da Query `241` — Seed `910` adiada até haver dados reais confirmados pela própria TCGdex.
