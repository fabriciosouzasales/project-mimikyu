...
*"Estamos chegando na parte mais interessante. Até agora tudo aconteceu dentro do nosso banco. Agora vamos sair dele pela primeira vez."* Fluxo planejado: `run` → `card_set` → `cards` → **TCGdex**. Esta é a primeira sprint que efetivamente inicia o "Pipeline Automático de Imagens" propriamente dito (busca em fonte externa). Nesta revisão, o escopo foi dividido em duas sprints mais granulares, para isolar a comunicação externa do processamento da imagem:

**B2.5A — apenas consultar a API.** Fluxo: `Carta 001` → `Consultar TCGDEX` → `Receber JSON` → `Encerrar`. Explicitamente: **sem download, sem Storage, sem banco** — apenas comunicação com a fonte externa e recebimento da resposta.

**B2.5B — a partir da consulta validada.** Fluxo: `JSON` → `Extrair URL da imagem` → `Download` → `Validar imagem`. Descrito como uma redução de complexidade em relação a tentar fazer tudo em uma única sprint. **Ver a nota sobre possível sobreposição com `B2.6` (Download) na seção "Roteiro vigente", acima — não resolvida unilateralmente.**

Nenhum código, deploy ou teste ainda para `B2.5A`/`B2.5B` — apenas o objetivo e a divisão foram anunciados; a sessão pareada sinalizou que, antes de baixar qualquer imagem, "faremos algo muito importante" primeiro, sem detalhar o quê nesta revisão.
