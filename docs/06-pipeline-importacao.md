...
| B2.5A | Integração com TCGdex — apenas consulta e recebimento do JSON (agora pelo `card_set`, via `card_set_external_reference`) | 🟨 Em andamento (`services/tcgdex.ts` escrito, não integrado/deployado) |
...
Primeiro arquivo real criado: `supabase/functions/import-card-assets/services/tcgdex.ts`, com uma função `findTcgDexSet(languageCode, externalSetId)` que monta a URL, faz o `fetch` com `Accept: application/json`, trata `404` retornando `null` (Set não encontrado, não é um erro), lança `TCGDEX_REQUEST_FAILED` para outras respostas não-`ok`, e retorna o JSON tipado como `TcgDexSet` (`id`, `name`, `logo?`, `symbol?`, `cardCount?: { total, official }`). **Confirmado sem erros no VS Code** (sem sublinhados vermelhos) — mesma disciplina de validação arquivo por arquivo adotada no Sprint B2.4.1. **Status real desta sprint, nesta revisão**: o arquivo foi criado e validado no editor, mas **`types.ts` e `index.ts` ainda não foram alterados** (instrução explícita: "Por enquanto, não altere `types.ts` nem `index.ts`") — a função ainda não está conectada ao fluxo da Edge Function, não foi deployada, e não fez nenhuma chamada real à TCGdex ainda. Seguindo o mesmo princípio de "copiar ao repositório apenas após confirmação" já usado em todo o projeto, `services/tcgdex.ts` **ainda não foi copiado para o repositório** — fica pendente até que a integração seja de fato testada. Próximo passo, ainda dentro de B2.5A: usar este serviço a partir de `card_set_external_reference` (uma vez que a Query `241` estiver completa) em vez de um identificador de teste isolado.
...
> **Objetivo**: criar o serviço capaz de consultar um Set na TCGdex a partir de um identificador externo e idioma, sem ainda conectá-lo ao fluxo da Edge Function.
> **Critério de aceite**: `services/tcgdex.ts` criado, sem erros no editor.
> **Resultado**: 🟨 Em andamento. Arquivo criado e validado; ainda não integrado a `index.ts`, não deployado, sem chamada real testada.
> **Pendências descobertas**: mapeamento de `card_set` para identificador externo não existia — resolvido por uma extensão pontual do Bloco A (`card_set_external_reference`, ver `05-modelo-de-dados.md`), não por uma sprint do Bloco B.
