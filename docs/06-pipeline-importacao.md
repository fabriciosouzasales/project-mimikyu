...
- **novo, Sprint B3, refinado no Sprint B3.9**: o endpoint `GET /sets/{id}/cards` da TCGdex (`TcgdexClient.getCardsBySet`) nunca foi chamado diretamente; uma lista resumida de cartas (`id`/`localId`/`name`/`image`) veio embutida na resposta de `getSet()` (`tcgdex_set.cards`, confirmado real, 188 itens para `ME1`) — não está confirmado se `getCardsBySet()` é sequer necessário, ou se é redundante com o que `getSet()` já devolve;
