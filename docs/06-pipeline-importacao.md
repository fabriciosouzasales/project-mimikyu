...
| 1.7 | **"Em Aberto" — multi-provider: decisão de escopo (2026-08-07).** Ao ser perguntado se o fechamento do Ciclo 2 de `ADR-024` deveria incluir uma decisão sobre múltiplas fontes externas, Fabrício confirmou explicitamente: manter como implementação futura, não desenvolver agora. A lista de sub-decisões já registrada (providers candidatos, fallback, rate limit, licenciamento etc.) permanece integralmente em aberto — esta revisão resolve apenas a pergunta de escopo/prioridade da sprint atual, não nenhuma das decisões técnicas de fato. |
