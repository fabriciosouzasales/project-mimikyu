...
> **Critério de aceite**: `services/tcgdex.ts`/`services/database.ts`/`index.ts` (v1.3.0) escritos, sem erros no editor; comportamento `NO_EXTERNAL_SET_MAPPING` definido para Sets sem referência externa; deploy confirmado retornando `NO_EXTERNAL_SET_MAPPING` (tabela ainda vazia).
> **Resultado**: 🟨 Em andamento. Código de `findCardSetExternalReference`, `getSet` (substituindo `findTcgDexSet`) e do rascunho v1.3.0 de `index.ts` escrito, mas **nenhum deploy ou teste real confirmado nesta revisão** — a sprint pivotou para resolver a falta de `external_set_id` reais antes de validar o deploy.
> **Pendências descobertas**: (1) mapeamento de `card_set` para identificador externo não existia — resolvido por uma extensão pontual do Bloco A (`card_set_external_reference`, ver `05-modelo-de-dados.md`); (2) um mapeamento de teste incorreto (`ME0`→`sv10pt5`) foi inserido e corrigido durante a validação da Query `241`; (3) os `external_set_id` reais da TCGdex para `ME0`-`ME5` são desconhecidos — descoberta delegada a um script administrativo (`scripts/discover-tcgdex-sets.ts`), criado mas ainda não executado nem movido para o local correto; (4) `ME5` mencionado pela primeira vez nesta revisão, sem confirmação de que já existe como `card_set` real no banco.
