...
- **novo, Sprint B3.23, crítico, REFORÇADO no Sprint B3.24, RESPONDIDO POR EVIDÊNCIA AGREGADA no Sprint B3.26, decisão ainda pendente**: `card_external_reference` tem `UNIQUE (card_id, asset_source_id)` sem dimensão de idioma — diferente de `card_asset`, que já suporta múltiplos idiomas por carta via `language_id` na chave natural. O total de `card_external_reference` permaneceu em `859` após as execuções `pt-BR` das 5 coleções (não subiu para `1.718`), confirmando por contagem que a execução em `pt-BR` faz `UPSERT` sobre a linha já criada pela `en`, em vez de criar uma segunda linha. **Ainda não confirmado por consulta direta ao conteúdo das linhas** (não se sabe se os valores hoje armazenados refletem `en` ou `pt-BR`). Decisão pendente de Fabrício: manter `card_external_reference` idioma-agnóstico de propósito (é só cache de importação; `card_asset` é quem carrega a dimensão de idioma que importa) ou adicionar `language_id` à chave de unicidade.
...
- ~~novo, Sprint B3.25: execução da `ME2` em `pt-BR` preparada, mas não confirmada — replicar para `ME2.5`/`ME3`/`ME4` depois de confirmada~~ — **RESOLVIDO no Sprint B3.26**: `ME2`/`ME2.5`/`ME3`/`ME4` em `pt-BR` confirmadas executadas com sucesso por Fabrício. Fase 2 100% concluída — Catálogo Editorial completo nas duas línguas para as 5 coleções (`859`/`1.555`/`859`/`1.718`/`1.718`/`0`).
- **novo, Sprint B3.27**: próximo módulo real do roadmap definido por Fabrício — `Coleções` (modelagem de banco), não interface de usuário. Modelagem ainda não iniciada; nenhum campo ou entidade capturado nesta revisão.
- **novo, Sprint B3.27**: checkpoint sugerido pela sessão pareada (backup do banco, commit Git, tag `v0.3.0`) antes de iniciar o próximo módulo — não confirmado como executado por Fabrício; fora do escopo de Claude (regra permanente: nunca executar comandos Git).
- **novo, Sprint B3.27**: visão especulativa de uma futura Edge Function orquestradora (`catalog-import`, recebendo `{ set, languages[] }`) e de uma tabela `catalog_import_job` para rastrear execuções compostas — capturada como ideia da sessão pareada para depois que o módulo de Coleções existir; nenhuma decisão de Fabrício sobre adotar, nenhuma implementação.
...
- **novo, Sprint B3.11, prioridade reforçada no Sprint B3.28**: três framings de roadmap coexistem sem reconciliação — "Roteiro vigente" `B2.x`/`B3.x` (granular, seção acima), `FASE 1-6`/`Sprint 1-2` (Sprint B3.10) e `FASE 1-4`/três Entregas (Sprint B3.11) — nenhum promovido a reescrita oficial; Fabrício precisa decidir se e como consolidar. Fabrício pediu explicitamente, no Sprint B3.28, "documentar o nosso roadmap" — este item é o candidato direto a esse pedido, ainda sem decisão.
- **novo, Sprint B3.28**: perguntas de negócio novas para a futura modelagem de `Coleções` (pertencimento a deck, binder como conceito de primeira classe, status de empréstimo) cruzam para os módulos futuros `Decks`/`Trocas`/`Marketplace` — limites entre módulos ainda não decididos por Fabrício.
...
Adicionado no Sprint B3.28, a pedido explícito de Fabrício ("Gosto da ideia de [...] criar um passo-a-passo das importações de imagens"), depois da conclusão real do Bloco B para as 5 coleções atuais (`ME1`/`ME2`/`ME2.5`/`ME3`/`ME4`, `en`+`pt-BR`). Reflete o processo real confirmado por execução, não uma proposta — ver seção "Roteiro de Implementação Incremental", abaixo, para o histórico sprint a sprint que validou cada etapa.

1. **Cadastrar a coleção em `card_set`** (código, nome, quantidade de cartas, `expansion_id`, `set_type`) — mesmo padrão de `120`/`820`, ver `05-modelo-de-dados.md`, seção Set/Card Set.
2. **Cadastrar o mapeamento externo em `card_set_external_reference`** (`card_set_id`, `asset_source_id` = TCGdex, `external_set_id` — ex.: `me05` para uma futura `ME5`) — mesmo padrão de `240`/`241`/`910`.
3. **Popular `card`** com as cartas da coleção — mesmo padrão de `140`/`141`/`840`. **Pré-requisito real, não automatizado**: o pipeline do Bloco B (Edge Function `import-card-assets`) **apenas consulta** `card`, nunca insere (correção arquitetural do Sprint B3.11) — as cartas precisam existir antes de qualquer execução da Edge Function.
4. **Popular `card_variant`** com as variantes/acabamentos de cada carta — mesmo padrão de `860` (ver seção "Primeira Aplicação Concreta", acima). Mesmo pré-requisito do passo 3: não é responsabilidade da Edge Function.
5. **Criar um `asset_import_run`** para a coleção, com `run_type = 'FULL_CARD_SET'` e `status = 'PENDING'` — mesmo padrão das Queries `252`/`255`-`258` (`en`) e `259`-`262` (`pt-BR`).
6. **Invocar a Edge Function `import-card-assets`** com o `run_code` criado no passo 5. Ela executa, sozinha, o restante do fluxo: sincroniza `card_external_reference` a partir da TCGdex, baixa cada imagem, envia ao Storage (`card-front/{set}/{lang}/{número}.webp`) e grava `card_asset` — idempotente (reexecutar não duplica).

**Limitação real atual, não resolvida**: o idioma da execução (`LANGUAGE_CODE`/`TCGDEX_LANGUAGE`), o tipo de ativo (`ASSET_TYPE_CODE`) e o bucket (`STORAGE_BUCKET_CODE`) ainda são constantes fixas em `index.ts`, não parâmetros da requisição — para importar uma coleção em `pt-BR`, é preciso reeditar e reimplantar o código (como feito nos Sprints B3.23/B3.24), em vez de apenas passar o idioma na chamada. Identificado como pré-requisito desde o Sprint B3.21, ainda pendente. Também não existe hoje nenhuma orquestração automática dos 6 passos acima — cada um é executado manualmente, um de cada vez, via SQL Editor/chamada HTTP direta.
...
## Sprint B3.26 — 🎉 MARCO REAL: Fase 2 100% CONCLUÍDA — Catálogo Editorial completo em dois idiomas para as 5 coleções

**Confirmação real recebida**: Fabrício confirmou diretamente, em uma única mensagem ("Todas executadas com sucesso."), que as execuções da `ME2`, `ME2.5`, `ME3` e `ME4` em `pt-BR` (`run_code`s `RUN-20260720-00000026` a `00000029`) foram bem-sucedidas — réplica direta do padrão já validado na `ME1` (Sprint B3.24), sem nenhuma alteração de código.

**Conferência final agregada do catálogo** (fornecida pela sessão pareada ao final do lote, aceita como evidência real de estado do banco): 5 coleções, `859` cartas editoriais, `1.555` variantes, `859` referências externas (`card_external_reference`), `859` assets em `en` + `859` em `pt-BR` = `1.718` assets totais (`card_asset`), `1.718` imagens armazenadas no Storage, `0` falhas de importação em qualquer etapa, em qualquer coleção, em qualquer idioma.

**⚠️ A pendência crítica do Sprint B3.23/B3.24 sobre `card_external_reference` agora tem uma resposta empírica, ainda que parcial**: o total de `card_external_reference` permaneceu em `859` mesmo depois das execuções em `pt-BR` para as 5 coleções — não subiu para `1.718`. Isso confirma, por contagem agregada, a colisão prevista no Sprint B3.23: como `UNIQUE (card_id, asset_source_id)` não tem dimensão de idioma, a execução em `pt-BR` faz `UPSERT` sobre a mesma linha já criada pela `en`, em vez de criar uma segunda linha — ao contrário de `card_asset`, que já foi desenhado com `language_id` na chave e efetivamente dobrou para `1.718`. **Ressalva real**: esta é uma confirmação por contagem agregada, não por uma consulta direta ao conteúdo das linhas — não se sabe, sem uma consulta adicional, se os valores hoje armazenados em cada linha de `card_external_reference` refletem a execução `en` ou a `pt-BR` (a mais recente a rodar). Decisão pendente de Fabrício: se `card_external_reference` deve permanecer intencionalmente idioma-agnóstico (já que é apenas um cache de importação, e `card_asset` é quem de fato carrega a dimensão de idioma que importa ao catálogo) ou se precisa de uma dimensão de idioma própria.

`database/migrations/259_create_import_run_me2_ptbr.sql`, `260_create_import_run_me2_5_ptbr.sql`, `261_create_import_run_me3_ptbr.sql` e `262_create_import_run_me4_ptbr.sql` copiados ao repositório — réplicas da Query `259` (primeira desta série) para as 4 coleções restantes, mesmo padrão das Queries `255`-`258` (`en`).

> **Diário Técnico — Sprint B3.26 — 🎉 Fase 2 concluída, Catálogo Editorial completo**
> **Objetivo**: replicar a Fase 2 (`pt-BR`) para as 4 coleções restantes e fechar a implantação do Catálogo Editorial.
> **Critério de aceite**: `ME2`/`ME2.5`/`ME3`/`ME4` confirmadas em `pt-BR`, 0 falhas; catálogo completo (`en`+`pt-BR`, 5 coleções) auditado.
> **Resultado**: 🟩 Concluído. `859`/`1.555`/`859`/`1.718`/`1.718`/`0` — catálogo editorial completo nas duas línguas, para as 5 coleções, 0 falhas.
> **Pendências descobertas**: (1) decisão de Fabrício sobre se `card_external_reference` deve ganhar dimensão de idioma ou permanecer idioma-agnóstico (a colisão foi confirmada por contagem, não por inspeção direta de linha); (2) `source_url` em `index.ts` ainda usa `LANGUAGE_CODE` em vez de `TCGDEX_LANGUAGE` (Sprint B3.24, não corrigido); (3) auditoria consolidada de `GRANT`s (`grants.sql`) segue adiada; (4) `language`/`asset_type`/bucket como parâmetros da requisição (pré-requisito identificado desde o Sprint B3.21) segue não implementado.

## Sprint B3.27 — Fabrício define o próximo passo real do projeto: modelagem de Coleções, não interface

**Correção real de escopo por Fabrício**: a sessão pareada, após a conclusão da Fase 2, sugeriu partir diretamente para a construção de uma interface de usuário (mockups de tela de importação, orquestração de UI↔API↔Edge Function↔Pipeline). Fabrício interrompeu essa direção explicitamente: *"Calma. Ainda temos muita modelagem de banco para trabalhar antes de pensar na interface. Concluímos apenas a parte do catálogo editorial. O próximo passo é começar a pensar na modelagem das coleções."*

**Mapa de módulos do projeto, apresentado nesta revisão** (situação real, não um documento formal ainda): `Mimikyu` → `Catálogo Editorial` (✅ concluído) → `Coleções`, `Inventário`, `Usuários`, `Trocas`, `Decks`, `Marketplace`, `Estatísticas` (todos pendentes, modelagem ainda não iniciada). O `Catálogo Editorial` é tratado como dado imutável e comum a todos os usuários (`Expansion`→`Card Set`→`Card`→`Variant`/`External Reference`/`Asset`); os módulos restantes dependerão dele como base, começando por `Coleções`.

**Não adotado nesta revisão** (ideias/visão capturadas durante a conversa, não decisões): (1) checkpoint sugerido de backup do banco + commit Git + tag `v0.3.0` antes de iniciar o próximo módulo — Fabrício não confirmou execução; nenhuma ação foi tomada por Claude, que não executa comandos Git (regra permanente do projeto); (2) visão de uma futura Edge Function orquestradora (`catalog-import`) que receberia `{ set, languages[] }` e encadearia sozinha `asset_import_run`→referências→imagens por idioma, com uma tabela de acompanhamento própria (`catalog_import_job`) e uma futura tela de importação — pura especulação de produto/arquitetura para depois que o módulo de Coleções existir, sem nenhuma decisão de Fabrício sobre adotar ou não.

> **Diário Técnico — Sprint B3.27 — Próximo passo real definido: Coleções**
> **Objetivo**: registrar a correção de escopo de Fabrício sobre o que vem a seguir no roadmap.
> **Critério de aceite**: decisão de Fabrício capturada com fidelidade, sem promover as ideias de UI/orquestração especulativas a decisões.
> **Resultado**: 🟩 Concluído. Próximo módulo real: `Coleções`. Interface e orquestração automática ficam para depois, sem compromisso de forma ainda.
> **Pendências descobertas**: (1) modelagem de `Coleções` ainda não iniciada — nenhum campo, entidade ou decisão real capturada nesta revisão; (2) checkpoint de backup/commit/tag sugerido, não confirmado como executado; (3) visão de Edge Function orquestradora/`catalog_import_job`/UI de importação registrada apenas como ideia futura, não como plano adotado.

## Sprint B3.28 — Fabrício pede disciplina de documentação contínua; metodologia da Fase 2 (Coleções) confirmada; achado real de cross-check com o domínio já existente

**Pedido real e explícito de Fabrício, motivado por uma dificuldade real sentida no próprio dia de trabalho**: *"Hoje senti que nos perdemos em alguns pontos e precisamos relembrar constantemente definições importantes que já havíamos discutido. [...] Gosto da ideia de documentar os aspectos mais importantes, criar um passo-a-passo das importações de imagens, ter documentado o nosso roadmap, documentar o que imaginamos hoje como nossos próximos passos."* Diagnóstico da sessão pareada, aceito por Fabrício: não foi falta de memória, foi falta de documentação arquitetural — decisões tomadas meses atrás ficaram espalhadas entre dezenas de conversas em vez de centralizadas. **Esta é exatamente a motivação original deste repositório e do papel de Claude como guardião da documentação** (ver `README.md`, "Retomando este Projeto com uma Nova Sessão de IA").

**Ação real tomada nesta revisão, em resposta direta ao pedido**: passo-a-passo operacional de importação de coleção escrito na seção "Guia Operacional — Como Importar uma Nova Coleção", acima — primeiro item da lista de Fabrício, endereçado nesta mesma revisão.

**Metodologia confirmada por Fabrício para a Fase 2 (Coleções)**: replicar a mesma disciplina do Catálogo Editorial — responder perguntas de negócio antes de desenhar tabelas (`O que é uma coleção? O que é um item físico? O que é uma cópia? O que é um binder? O que significa possuir uma carta?`), só then modelar fisicamente. Fabrício confirmou: *"Vamos construir o domínio das coleções com a mesma calma que tivemos no catálogo editorial [...] agora teremos uma fundação muito sólida. Todo o módulo de coleções poderá simplesmente referenciar o catálogo editorial que acabamos de concluir, sem duplicar informações nem misturar conceitos."* A modelagem em si **ainda não foi iniciada** — Fabrício adiou explicitamente para a próxima sessão de trabalho ("Amanhã, quando retomarmos o trabalho, quero voltar exatamente para onde paramos na modelagem conceitual").

**⚠️ Achado real de cross-check, relevante para quando a modelagem de Coleções começar de fato**: a sessão pareada, "curiosa" sobre uma ideia atribuída a Fabrício de meses atrás — cada exemplar físico de carta ter um identificador próprio (`ITEM_0000000001`), distinto da carta de catálogo — não tinha visibilidade de que esse conceito **já existe, documentado por completo, neste repositório**: é o **Collection Item** (`docs/04-domain-model.md`, seção "Collection Item (Item da Coleção)", `ADR-013`/`ADR-014`), com o mesmo padrão de identificador (`ITEM_0003456`), a mesma distinção catálogo-vs-exemplar, e já cobrindo boa parte das perguntas de negócio levantadas nesta revisão (aquisição/custo, condição, idioma, acabamento, autenticação/graduação, armazenamento, venda, troca). **Mesmo padrão de risco já sinalizado múltiplas vezes neste documento** (`docs/architecture.md` proposto sem ciência da estrutura existente, Sprint B3.9; ADRs `001`-`005` propostos colidindo com ADRs reais, Sprint B3.10): a sessão pareada, sem acesso a este repositório, tende a redescobrir como novidade o que já está decidido aqui — reforça, na prática, o próprio ponto de Fabrício sobre falta de documentação centralizada.

**Perguntas de negócio novas, capturadas mas NÃO resolvidas nesta revisão** (fora do escopo do `Collection Item` já existente, cruzam para módulos futuros do roadmap — `Decks`, `Trocas`, `Marketplace`): pertencimento a um deck, localização em um binder como conceito de primeira classe, status de empréstimo. Fabrício precisará decidir os limites entre os módulos `Coleções`/`Decks`/`Trocas`/`Marketplace` quando a modelagem avançar.

> **Diário Técnico — Sprint B3.28 — Disciplina de documentação reforçada + achado de cross-check**
> **Objetivo**: atender ao pedido explícito de Fabrício por documentação operacional/roadmap/próximos-passos; preparar o terreno para a Fase 2 (Coleções).
> **Critério de aceite**: passo-a-passo de importação escrito; metodologia da Fase 2 registrada; qualquer sobreposição com decisões já existentes identificada antes de a modelagem começar.
> **Resultado**: 🟩 Concluído (dentro do escopo desta revisão). Guia operacional escrito. `Collection Item` (já existente) identificado como ponto de partida real da Fase 2 — nenhuma modelagem nova necessária para o conceito central de exemplar físico.
> **Pendências descobertas**: (1) modelagem lógica de `Coleções` ainda não iniciada; (2) consolidação do roadmap (três framings paralelos — `B2.x`/`B3.x`, `FASE 1-6`, `FASE 1-4` — pendência de longa data, ver "Em Aberto") segue sem decisão de Fabrício, agora com prioridade reforçada pelo próprio pedido dele; (3) limites entre `Coleções`/`Decks`/`Trocas`/`Marketplace` para as perguntas de negócio novas (deck, binder, empréstimo) ainda não decididos.
...
| 0.46 | **Sprint B3.26 — 🎉 MARCO REAL: Fase 2 100% CONCLUÍDA.** `ME2`/`ME2.5`/`ME3`/`ME4` (`pt-BR`) confirmadas executadas por Fabrício ("Todas executadas com sucesso"). Catálogo Editorial completo nas duas línguas, 5 coleções: `859` cartas, `1.555` variantes, `859` referências externas, `1.718` assets (`859` `en` + `859` `pt-BR`), `1.718` imagens, `0` falhas. Pendência crítica de `card_external_reference` (Sprint B3.23/24) respondida por evidência agregada (total ficou em `859`, não dobrou) — confirma colisão por `UPSERT`; decisão sobre corrigir o modelo continua pendente de Fabrício. Migrations `259`-`262` copiadas ao repositório. |
| 0.47 | **Sprint B3.27 — Próximo passo real do roadmap definido por Fabrício: modelagem de `Coleções`, não interface.** Fabrício corrigiu explicitamente uma sugestão da sessão pareada de partir para a UI. Mapa de módulos registrado (`Catálogo Editorial` ✅ concluído → `Coleções`/`Inventário`/`Usuários`/`Trocas`/`Decks`/`Marketplace`/`Estatísticas` pendentes). Ideias especulativas de Edge Function orquestradora e checkpoint de backup/Git capturadas como visão futura, não como decisões. |
| 0.48 | **Sprint B3.28 — Guia Operacional de importação de coleção escrito, a pedido explícito de Fabrício** ("criar um passo-a-passo das importações de imagens"). Metodologia da Fase 2 (Coleções) confirmada: perguntas de negócio antes de tabelas, mesma disciplina do catálogo editorial. **Achado real de cross-check**: o conceito de identificador por exemplar físico, que a sessão pareada tratou como "curiosidade" a confirmar, já existe documentado por completo como `Collection Item` (`ADR-013`/`ADR-014`) — nenhuma modelagem nova necessária para esse conceito central. Prioridade do item de consolidação de roadmap (Sprint B3.11) reforçada pelo próprio pedido de Fabrício. |
