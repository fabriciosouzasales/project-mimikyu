...
- **novo, Sprint B3, refinado no Sprint B3.9, refinado novamente no Sprint B3.10**: o endpoint `GET /sets/{id}/cards` da TCGdex (`TcgdexClient.getCardsBySet`) nunca foi chamado diretamente; uma lista resumida de cartas (`id`/`localId`/`name`/`image`) veio embutida na resposta de `getSet()` (`tcgdex_set.cards`, confirmado real, 188 itens para `ME1`) — não está confirmado se `getCardsBySet()` é sequer necessário, ou se é redundante com o que `getSet()` já devolve. **Sprint B3.10 acrescenta uma segunda pergunta distinta**: mesmo que `getCardsBySet()` seja dispensável, a lista resumida não contém raridade/categoria/número total/validação de imagem — falta decidir se esses dados virão de um endpoint de **carta individual** (`getCard`) ou de algum payload mais completo ainda não identificado;
...
- **novo, Sprint B3.6**: não confirmado se o `GRANT` ausente encontrado em `card_set_external_reference` também afeta outras tabelas do projeto criadas por migration SQL direta — auditoria completa proposta pela sessão pareada, ainda não executada.
...
- **novo, Sprint B3.9**: implementar a persistência real de `card`/`card_external_reference` a partir das 188 cartas resumidas já confirmadas da TCGdex para `ME1` — fluxo definido (`asset_import_run`→`card_set_external_reference`→TCGdex→`card`→`card_external_reference`→`card_asset` depois), ainda não codificado.
