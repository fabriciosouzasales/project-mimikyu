...
│       ├── logical-model.md
│       ├── checkpoint-AAAA-MM-DD.md
│       └── <benchmark-ou-referencia-nao-normativa>.md
├── history/
│   ├── <artefato>.md
│   └── development/
...
**`domain-modeling/` — formalizado (2026-08-28), a partir de `docs/domain-modeling/collections/`, criado sem que a árvore documental o previsse.** Mesmo padrão de lacuna já resolvido para `development/` em 2026-07-26 (ver nota abaixo): um artefato real foi criado em uma sessão sem que a arquitetura documental o descrevesse formalmente. Motivo da lacuna: nenhuma categoria existente cobre um registro de decisões de modelagem (conceitual + lógica), produzido incrementalmente por checkpoints, ainda não reconciliado em prosa contínua num documento de arquitetura (`04-domain-model.md`/`05x-*.md`) — não é ADR (não é uma decisão única com alternativas e consequências), não é Standard (não é "como aplicar uma regra"), não é Architecture (não é diagrama de componentes/solução), não é History (o conteúdo é vigente, não superado). Ver a responsabilidade completa na tabela "Artifact Responsibilities", abaixo. Estrutura por domínio: `concept-decisions.md` (registro de decisões conceituais, ex.: `C-01`..`C-NN`), `logical-model.md` (checkpoint de modelagem lógica em evolução, ex.: `LDM-01`..`LDM-NN`), `checkpoint-AAAA-MM-DD.md` (um por reconciliação/rodada de decisões novas, sem reescrever os anteriores), e documentos de referência não normativa (ex.: benchmarks de produto/UX) quando aplicável. Quando o domínio amadurecer para modelagem física, o conteúdo é reconciliado em `04-domain-model.md`/`05x-*.md` no ciclo correspondente — `domain-modeling/` não substitui esses documentos, é o estágio anterior a eles.
...
| Domain Modeling | Registro incremental de decisões de modelagem de um domínio (conceitual + lógica), organizado por domínio em subpastas (`domain-modeling/<dominio>/`), produzido antes ou em paralelo à modelagem física em `04-domain-model.md`/`05x-*.md`. Contém registros de decisão numerados (ex.: `C-xx` conceitual, `LDM-xx` lógico), checkpoints datados de reconciliação, e documentos de referência não normativa (ex.: benchmarks). Não é ADR (não decide uma única questão com alternativas/consequências formais), não é History (conteúdo vigente, não superado) — é o estágio de trabalho anterior à prosa definitiva de um documento de arquitetura. Quando a modelagem física começa, o conteúdo é reconciliado nos documentos numerados correspondentes, que passam a ser a fonte definitiva; o material aqui permanece como registro de como se chegou lá. |
...
| 1.13 | **Formalizado o tipo de artefato `domain-modeling/` (2026-08-28), mesma classe de lacuna já resolvida para `development/` em 2026-07-26**: `docs/domain-modeling/collections/` (`concept-decisions.md`, `logical-model.md`, `pkmnbindr-benchmark.md`, `checkpoint-2026-08-28.md`) havia sido criado na mesma rodada, sem que a árvore documental o previsse. Adicionado à árvore de estrutura; nova linha "Domain Modeling" na tabela de Artifact Responsibilities; nova nota explicando o motivo da lacuna e a estrutura esperada por domínio. Motivado pela incorporação de três documentos de modelagem de Collections produzidos em repositório paralelo, marcando ADR-013/ADR-014 como Substituídos no mesmo ciclo. |
