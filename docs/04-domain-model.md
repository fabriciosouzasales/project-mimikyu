...
**SQL real recebida para `140` (tabela), `141` (função de validação + os dois triggers) e `940` (validação, 18 subconsultas) — texto verbatim, com header oficial completo (`Status: CANÔNICA`, `Versão: 1.0`).** Consistente com a prática do projeto de nunca presumir execução sem confirmação explícita: nenhuma mensagem de sucesso ("executado com sucesso"/screenshot de resultado real) foi recebida para `140`/`141`/`940` neste lote — apenas o SQL em si e uma descrição dos resultados *esperados* caso sejam executados numa tabela ainda vazia. Por isso, os três arquivos **não foram copiados para `database/`** ainda (`database/README.md`: "os arquivos aqui são copiados depois da execução confirmada... nunca antes") — texto completo documentado em `05-modelo-de-dados.md`, seção Card, "Modelo Físico — SQL real recebida (verbatim), execução não confirmada".

**Query `840 - Seed Card` — deliberadamente ainda não escrita.** Depende da escolha do primeiro Card Set cujo checklist oficial completo será importado. Fabrício enviou o PDF `P10346_ME01_Card_List_PTBR.pdf` (checklist oficial de ME1 em português — já arquivado em `assets/reference-sources/` desde um ciclo anterior) como candidato, mas a Query em si permanece em aberto para um próximo ciclo.
