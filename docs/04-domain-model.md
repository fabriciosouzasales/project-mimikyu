...
> **Proposta em aberto, não decidida:** foi sugerido, na sessão paralela, adicionar um campo `symbol` (símbolo textual da raridade, ex. `★`, `★★`) a esta entidade. A própria sugestão condicionou a mudança a um levantamento prévio das legendas oficiais dos checklists, para confirmar que o mesmo símbolo sempre representa a mesma raridade no catálogo atual. Fabrício ainda não confirmou. Ver `05-modelo-de-dados.md`, seção Rarity — "Proposta em Aberto — Campo `symbol`", para o registro completo. Nenhuma alteração de modelo foi feita a partir desta proposta.
