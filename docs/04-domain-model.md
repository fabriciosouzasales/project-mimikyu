...
### Pokémon Region (nota, 2026-09-04)

Region (ex.: Kanto, Johto, Hoenn) é registrada como entidade canônica própria, subordinada ao mesmo escopo mínimo deste documento (ADR-011) — independente de Generation: confirmado por auditoria direta da PokéAPI que existem Regiões sem nenhuma Generation principal associada (Orre, Hisui). Cada Pokémon Species tem exatamente uma Generation (acima); cada Generation, por sua vez, tem exatamente uma Main Region (N:1 — uma Region pode ser Main Region de 0..N Generations). Fundação física (`pokemon_region`, `pokemon_region_external_reference`, `pokemon_generation.main_region_id`) **CONFIRMADO EXECUTADO em 2026-09-04**; sourcing real ainda **SUSPENSO**. Detalhamento completo em `docs/domain-modeling/collections/logical-model.md`, LDM-186 a LDM-190, e `ADR-011` v1.3. Locations, Areas, Version Groups e o grafo de navegação entre Regiões permanecem fora de escopo.
...
> ⚠️ **SUBSTITUÍDO em 2026-08-28, nome atualizado novamente em 2026-08-30.** A modelagem nova (`docs/domain-modeling/collections/`) reverte esta renomeação: o nome vigente da identidade física é **Physical Card** (nomes anteriores, em ordem: "Collection Item" → "Inventory Item", nome adotado em 2026-08-28 → "Physical Card", nome canônico desde 2026-08-30, ver `concept-decisions.md` C-47/C-48). Physical Card referencia Card Variant, mas não carrega `owner_user_id` — a posse corrente é transitiva via um agregado `Inventory` (Acervo, no máximo um por Physical Card enquanto sob ownership rastreado). Ver `checkpoint-2026-08-28.md`, Seção 6, e `logical-model.md`, LDM-23 (revisada), para a reafirmação explícita de que Physical Card permanece a única identidade física (sem entidade paralela). **O corpo desta seção abaixo preserva a terminologia "Collection Item" tal como originalmente escrita, por rastreabilidade histórica — não foi reescrito.**
...
| 2.11 | **Nova subseção "Pokémon Region (nota, 2026-09-04)"** (`POKEMON-REGION-DOMAIN-MODELING-AUDIT-01` → `POKEMON-REGION-FOUNDATION-PHYSICAL-IMPLEMENTATION-01`/`-CANONICAL-PROMOTION-01`), inserida entre a seção Pokémon Species e a subseção Pokédex/`REFERENCE_POSITION` já existente. Registra Region (Kanto, Johto, Hoenn...) como entidade canônica própria, independente de Generation (existem Regiões sem Main Generation), e a cardinalidade N:1 de Generation → Main Region. Aponta para `docs/domain-modeling/collections/logical-model.md` (LDM-186 a LDM-190) e `ADR-011` v1.3 como fonte canônica completa, sem duplicar conteúdo aqui. Fundação física `CONFIRMADO EXECUTADO` em 2026-09-04; sourcing real ainda SUSPENSO. Nenhuma alteração na seção Pokémon Species em si. |
