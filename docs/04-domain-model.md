...
Três divergências em relação à proposta original, nenhuma resolvida ainda:

1. **Não existe `card_variant_id`.** A tabela física não permite associar um Card Asset a uma Card Variant específica — contradiz diretamente a proposta de "imagem específica da versão Reverse Holo" descrita acima. Ou a variante de imagem por acabamento não é (ainda) suportada fisicamente, ou é resolvida por outro mecanismo não identificado. **Não presumido — aguardando esclarecimento de Fabrício.**
2. **`storage_provider` (texto livre, proposto) não existe — em vez disso há `storage_bucket_id` (UUID, FK para a tabela `storage_bucket`)**, já listada em `06-pipeline-importacao.md` como parte da infraestrutura de ativos pré-existente. Mais estruturado do que a proposta original.
3. **Existe `language_id` (UUID, presumivelmente FK para a tabela `language`)**, ausente da proposta original. Sugere que um Card Asset pode variar por idioma — possivelmente relacionado à ainda-não-documentada entidade Card Translation (ver seção própria, `*Documentação pendente*`). **Não presumido — aguardando esclarecimento de Fabrício sobre a relação entre `card_asset.language_id` e Card Translation.**

### Sequência de Queries planejada (não executada)

```text
170 - Create Card Asset Type Table
180 - Create Card Asset Table
870 - Seed Card Asset Type
880 - Seed Card Asset (quando a carga de imagens for decidida)
```
