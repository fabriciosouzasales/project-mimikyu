...
### SQL recebida para `180`/`181`/`980` (Card Asset) — execução NÃO confirmada

Texto verbatim recebido para as três Queries (tabela, triggers, validação estrutural), regeneradas a pedido de Fabrício ("Vamos garantir que esse bloco não tenha erro. Gere novamente as querys 180... 181... 980"). **Nenhuma confirmação de execução foi dada** — consistente com `database/README.md`, não copiado para `database/` ainda.

> **Mesmo problema de formatação já sinalizado para `870`/`970` v1.0 persiste aqui**: cabeçalho fora do padrão STD-001 e texto de `COMMENT ON` em inglês, quebrando a convenção do projeto (campos legíveis por humanos em português, ver `150_create_card_variant_type_table.sql` como referência). Caso Fabrício confirme a execução desta versão, os mesmos ajustes de reformatação (cabeçalho + tradução de comentários, sem alteração de lógica) serão aplicados antes de escrever para `database/`.

### Sequência de Queries (`880` ainda não abordada)
