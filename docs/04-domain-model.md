...
**Query `940` — reexecutada uma vez sobre os dados reais, mas precisa ser reescrita antes de se tornar a versão canônica final.** A versão recebida no lote anterior (18 subconsultas genéricas, pensada para uma tabela vazia) foi rodada novamente após a carga das 859 Cards — resultados relatados: os cinco Sets em status `COMPLETE` com as quantidades corretas, sem duplicidades ou inconsistências. A sessão pareada notou que a Query, apesar de funcionar, ainda não tem uma seção canônica explícita comparando contra as quantidades esperadas dos cinco Sets e o total de 859 (no mesmo padrão já usado pela CTE de dados canônicos de `930`, de Rarity). Fabrício confirmou a intenção ("Vamos atualizar query 940 então"), mas essa reescrita **ainda não foi feita** — fica para o próximo ciclo.
