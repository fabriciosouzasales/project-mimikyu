...
### SQL recebida para `170`/`171`/`870`/`970` (Card Asset Type) — execução NÃO confirmada

Texto verbatim recebido para as quatro Queries (tabela, trigger de `updated_at`, seed com três tipos canônicos `CARD_FRONT`/`ARTWORK`/`CARD_BACK`, e validação em 18 blocos). **Nenhuma confirmação de execução foi dada nesta sessão** — consistente com `database/README.md`, não copiado para `database/` ainda.

> **Dois problemas identificados antes de qualquer execução.** (1) **Formato de cabeçalho inconsistente com o padrão do projeto** (STD-001: `Projeto/Query/Versão/Status/Autor/Data/Descrição resumida/Descrição/Regras de Negócio/Pré-requisitos`) — o próprio Fabrício sinalizou que a sessão pareada (ChatGPT) está mostrando sinais de perda da janela de contexto a partir deste ponto, e pediu que a documentação mantenha o padrão já estabelecido; ao reescrever estas Queries para `database/`, os cabeçalhos precisarão ser reformatados. (2) **Bug no Seed `870`: usa o código de Game `POKEMON_TCG`, que não existe no projeto** — o código real e consistentemente usado em todos os seeds já executados (`810`, `820`, `821`, `830`, `831`, `840`, `850`) é `POKEMON`. Se executada como está, a Query `870` falharia no próprio `RAISE EXCEPTION` que ela mesma define. **Não corrigido unilateralmente** — sinalizado para Fabrício corrigir antes de executar.
>
> **Nota técnica adicional**: como `card_asset_type` já existe fisicamente (confirmado na revisão anterior), executar `170` como `CREATE TABLE IF NOT EXISTS` não adicionará retroativamente as novas constraints propostas (`uq_card_asset_type_game_name`, `uq_card_asset_type_game_order`, formato de `code`, índice de `is_active`) à tabela já existente, caso ela não as tenha — seria necessária uma migration/`ALTER TABLE`, não uma nova criação.

### Sequência de Queries planejada (`180`/`181`/`980` recebidas apenas como plano; `880` ainda não abordada)
...
880 - Seed Card Asset                    (planejada para depois — associará cada Card à sua imagem sem depender da Query 860, reforçando a independência Card Asset ↔ Card Variant)
```
