...
> **Nota em aberto, agora com proposta concreta de modelagem física (ainda não aprovada nem executada).** Depois de fechar o pacote técnico de Card (`140`/`141`/`840`/`940`, todos executados), a sessão pareada avançou diretamente para esta entidade usando o termo "Card Variant" — não "Finish"/"Card Finish" — propondo: `card_variant_type` (catálogo de tipos de variante por Game — exemplos dados para Pokémon: `STANDARD`, `REVERSE_HOLO`, `PROMO_STAMPED`, `POKE_BALL_REVERSE`, `MASTER_BALL_REVERSE`) e `card_variant` (relaciona uma `card` a um tipo de variante: `id, card_id, variant_type_id, variant_order, is_default, created_at, updated_at`, com `UNIQUE (card_id, variant_type_id)` e `UNIQUE (card_id, variant_order)`; `is_default` identificaria a versão principal do checklist oficial). Sequência de Queries proposta e confirmada por Fabrício como próximo bloco de trabalho (**nenhuma DDL escrita ou executada ainda**): `150`/`151`/`850`/`950` (Card Variant Type), depois `160`/`161`/`860`/`960` (Card Variant).
>
> **Isto não é uma reintrodução isolada** — é a continuação do mesmo ponto já sinalizado aqui: o campo `stamp` cogitado antes não aparece mais nesta proposta (substituído por `variant_type_id` mais genérico, cobrindo `PROMO_STAMPED` como um valor entre outros, não uma dimensão própria). Mais importante: **os nomes físicos `card_variant`/`card_variant_type` já existem no banco desde antes desta fase de documentação** (parte do conjunto original de 17 tabelas, anterior à decisão conceitual Finish/Card Finish de ADR-010) — então esta proposta pode ser simplesmente a implementação real dessas tabelas físicas já existentes, sob seus nomes originais, sem que a sessão pareada tenha decidido conscientemente reverter Finish/Card Finish. **Continua não avaliado nem aprovado por Fabrício quanto à nomenclatura conceitual** — antes de `150` ser escrita de verdade, Fabrício precisa decidir: (a) adotar "Card Variant"/"Card Variant Type" como os termos finais, revertendo ADR-010; (b) manter Finish/Card Finish como termos conceituais e apenas mapear para as tabelas físicas `card_variant`/`card_variant_type` (a opção já prevista na nota de nomenclatura física acima); ou (c) outra reconciliação. Não decidir isso silenciosamente.
>
> **Nota lateral, também não resolvida:** o exemplo ilustrativo desta mesma discussão (`Card → Card Variant → Inventory Item`, com `ITEM_0003456`) usa "Inventory Item" — termo já renomeado para "Collection Item" neste projeto (ADR-013). Provavelmente um hábito de nomenclatura antiga da sessão pareada, não uma proposta deliberada de reverter o rename — mas sinalizado aqui por precaução, sem nenhuma alteração feita.
