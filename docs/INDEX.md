...
| Documento | Resumo |
|---|---|
| [`development/HANDOFF-2026-08-18.md`](development/HANDOFF-2026-08-18.md) | Handoff vigente — encerramento formal do **Incremento P12 — Exibição de Preços no Frontend** (migrations `3903`/`3904`, 14 critérios de aceite, commits `6324b66`/`0dad413` sincronizados) e, na mesma data, **Incremento P13.1 — Fundação de Orquestração Programada de Pricing** (`FX_REFRESH`/`fx_source_code`/`confirmed_by` opcional/índices de concorrência em `pricing_sync_run`, Migrations `3905`–`3907`, `ADR-031`) e **Incremento P13.2 — Módulo Compartilhado e Execução Manual Auditável da PTAX, formalmente encerrado** (núcleo puro de 12 arquivos, adapter/runner reescritos, migration `3909` — autoridade temporal do PostgreSQL —, reexecução idempotente auditada). **P13 permanece em andamento**; próximo passo P13.3 (Edge Function `ptax-fx-refresh`). Nenhuma extensão de agendamento habilitada, nenhum código JustTCG. Correção de autenticação `/login` fora do escopo. |
