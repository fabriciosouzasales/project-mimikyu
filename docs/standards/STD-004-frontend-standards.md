...
- **`Select`/`Textarea` compartilhados** (2026-08-16): ~9 telas usam `<select>`/`<textarea>` nativos com a mesma combinação de classes repetida (`border-input bg-surface ... focus-visible:ring-2 ring-ring ring-offset-2`) em vez de um primitive único — já herdam a cor correta da consolidação de tokens desta versão, mas não o tratamento de foco mais recente do `Input` (halo sem offset, ver Seção 6). Candidato a um próximo incremento de Design System (extrair `ui/select.tsx`/`ui/textarea.tsx`), não resolvido nesta rodada por ser refatoração estrutural, fora do escopo de uma consolidação puramente visual.
