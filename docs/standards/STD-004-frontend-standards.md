...
**Zoom de carta simplificado**: `/pesquisa` usa o `Dialog` genérico (Seção 6) para exibir a carta ampliada — não reaproveita a transição View Transitions de `CartaZoomDialog` (mecanismo interno de `cartas-gallery.tsx`, não extraído para reutilização neste incremento). Candidato a unificação futura, não resolvido aqui.
