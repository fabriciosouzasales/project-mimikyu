...
**Combobox do header (`components/app-shell/global-search.tsx`)**: padrão ARIA combobox/listbox por `aria-activedescendant` — o foco do DOM nunca sai do `<input>`; a opção "ativa" é só uma marcação visual + `aria-activedescendant` apontando para o `id` do `<li role="option">`, nunca `focus()` programático no item. Lógica de busca (debounce, `AbortController`/descarte de resposta obsoleta, seleção, navegação) extraída em `useCardSearchSuggestions()`, compartilhada pelas duas apresentações abaixo — sem duplicação de regra de negócio entre desktop e mobile. Desktop (`SearchCombobox`): campo inline centralizado no header (`flex-1` entre os grupos de título e ações, que continuam `shrink-0` — nenhum dos dois se desloca), dropdown flutuante clássico, botão de pesquisa avançada (`SlidersHorizontal`) sobreposto dentro do próprio campo.

**Modo imersivo de pesquisa mobile (`MobileSearchOverlay`, reescrito em 2026-08-17, referência funcional pkmn.gg pedida por Fabrício — identidade visual MMKYU integralmente preservada, nada copiado de logotipo/cores/proporções)**: ao tocar no botão de busca (`md:hidden`), o header não é comprimido nem sobreposto por um dropdown — um painel de tela cheia (`@radix-ui/react-dialog`, primitives diretos) ocupa e substitui visualmente a barra do header (mesma altura `h-14`), com campo de busca + botão "Filtros" (pesquisa avançada) + botão "Fechar" como três controles externos e independentes — ao contrário do desktop, nenhum botão fica sobreposto dentro do campo; os dois botões usam toque de 44×44px (`h-11 w-11`). Painel de sugestões abaixo ocupa o espaço restante (`min-h-0 flex-1 overflow-y-auto`, rolagem própria, nunca a da página), limitado a `max-h-[100dvh]` (mesma unidade de viewport dinâmico de `AppShell`/`AuthHeroShell`) para acomodar o teclado virtual, com `env(safe-area-inset-top/bottom)` para notch/Dynamic Island/barra de gestos. Antes de haver termo digitado, mostra uma orientação curta ("Pesquise pelo nome, número ou código do Card Set.") em vez de painel vazio; item de sugestão com altura mínima de 44px de toque; região `aria-live="polite"` oculta anuncia quantidade de resultados. Fecha por `X`, `Escape` (nativo do Dialog, não interceptado no campo — diferente do desktop, que primeiro fecha só sua lista local), navegação concluída, clique fora, e pelo botão Voltar do navegador (`history.pushState` marcador + `popstate`, com guarda contra duplo-mount do React Strict Mode). Nova animação dedicada `search-panel-in`/`out` (`tailwind.config.ts`) — fade + leve deslize vertical, distinta de `drawer-in`/`out` (lateral, menu mobile) e `dialog-in`/`out` (zoom centralizado).
...
| 1.7 | **Modo imersivo de pesquisa mobile reescrito (2026-08-17, mesmo dia, referência funcional pkmn.gg pedida por Fabrício).** Seção 11: parágrafo do combobox mobile substituído por completo — de "overlay dedicado (Radix Dialog, topo da tela) com o mesmo combobox" para um painel de tela cheia que substitui visualmente a barra do header (campo + botão "Filtros" + botão "Fechar" externos, ambos com toque ≥44px; painel de sugestões com rolagem própria, `max-h-[100dvh]`, `env(safe-area-inset-*)`; orientação textual antes do termo mínimo; fechamento por `X`/Escape/navegação/clique fora/botão Voltar via `history.pushState`+`popstate`). Lógica de busca extraída para `useCardSearchSuggestions()`, compartilhada entre `SearchCombobox` (desktop, inalterado no comportamento) e `MobileSearchOverlay` (mobile, novo) — elimina a duplicação que antes existia via prop `variant` num único componente. Nova animação `search-panel-in`/`out` em `tailwind.config.ts`. Identidade visual MMKYU integralmente preservada — pkmn.gg foi referência só de comportamento. Ver `ADR-030` revisão `1.5` para o racional completo. Teste em dispositivo/navegador real pendente (sandbox sem navegador). |
