...
**Fundação física de Pricing completa — dez de dez entidades `CONFIRMADO EXECUTADO`.** Domínio ainda não operacional: seguem pendentes fonte homologada, condições canônicas semeadas, mappings reais, integração/sincronização, ingestão PTAX, frontend e Analytics/Item Valuation — ver `docs/05f-pricing.md` e `ADR-029` para o detalhamento completo.
