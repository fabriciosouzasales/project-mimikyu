...
**Regra de confiabilidade dos dados de Seed:** uma Seed nunca deve inserir dados estimados como se fossem oficiais. Se a existência de um registro é conhecida, mas algum de seus atributos de negócio (quantidades, datas, contagens) ainda não foi validado contra uma fonte confiável, o registro fica de fora da Seed até que a validação aconteça — não se insere um valor aproximado apenas para completar o cadastro. Exemplo real: ao popular `card_set`, o Set `ME3 (Perfect Order)` foi deliberadamente deixado fora da Seed `820`, mesmo com sua existência confirmada pela página oficial do Pokémon TCG, porque suas quantidades de cartas (`base_set_size`/`total_set_size`) ainda não haviam sido validadas.
