...
- `pricing_card_mapping` (`3040`–`3049` tabela/trigger) — `CONFIRMADO EXECUTADO` (Incremento P2), correspondência `card` ↔ fonte externa, tabela vazia. `service_role` recebeu apenas `SELECT` nas duas tabelas do Incremento P2 — capacidade de escrita (`INSERT`/`UPDATE`) deliberadamente adiada para um incremento futuro de sincronização, decisão explícita de Fabrício.
...
`3700–3799`: Seeds — `3700` `CONFIRMADO EXECUTADO` (Incremento P7, 2026-08-17): primeira linha de `pricing_source`, `JUSTTCG`, homologada para piloto técnico (Decisão A aprovada; Decisão B/idioma inconclusiva), `is_active = FALSE` (não produção). `3700` é o primeiro arquivo físico a existir na faixa `3000`–`3999` do repositório — divergência pré-existente de rastreabilidade entre `database/migrations/` e o histórico do Supabase (ver `ADR-030`, "Débitos Técnicos Registrados") não afeta este arquivo, que nasce já versionado. Nenhuma condição canônica semeada (vocabulário ainda não confirmado); demais faixas de `3700`–`3799` seguem livres.
