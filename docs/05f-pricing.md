...
**Piloto real já executado uma vez fora deste sandbox** (por Fabrício, localmente, com `deno run --allow-env` e `JUSTTCG_API_KEY` configurada) — encontrou e motivou as duas correções acima (`confirmed_by` e `findCard()`); ainda não houve uma execução completa sem erro ponta a ponta. Dentro deste sandbox de execução do agente, o piloto real continua não executável: `JUSTTCG_API_KEY` ausente e o runtime Deno em si não está disponível (`deno.land`/`npm:deno` fora da allowlist de rede), mesma classe de limitação já registrada para `npm run build`/`lint` do frontend — a lógica de sanitização, normalização, mapeamento de condição, detecção de idioma e identidade de idempotência segue validada apenas por harness `node` equivalente (16/16 asserções) e por sintaxe TypeScript confirmada bem-formada (`ts.transpileModule`, zero diagnóstico). Fabrício pode rodar `deno run --allow-env scripts/sync-justtcg-pricing.ts --confirmed-by=<admin_user_uuid>` novamente quando quiser reexercitar o piloto real.

**Validação pós-migration**: `card_condition` (5 linhas) e `pricing_condition_mapping` (5 linhas, todas `JUSTTCG`) confirmadas; `pricing_set_mapping`/`pricing_card_mapping` confirmadas com `service_role` = `INSERT`+`SELECT` exatos (via `information_schema.column_privileges`, não só `role_table_grants` — grants de coluna não aparecem na segunda); todas as oito tabelas tocadas (`card_condition`, `pricing_condition_mapping`, `pricing_set_mapping`, `pricing_card_mapping`, `pricing_product`, `pricing_observation`, `pricing_sync_run`, `pricing_sync_run_call`) confirmadas com RLS habilitada e exatamente uma policy cada; `get_advisors` (segurança e performance) sem nenhum achado novo referenciando as tabelas do domínio — os itens de índice não utilizado já esperados, tabelas ainda vazias; contagem de linhas de `pricing_set_mapping`/`pricing_card_mapping`/`pricing_product`/`pricing_observation`/`pricing_sync_run`/`pricing_sync_run_call` confirmada em zero (nenhum piloto real rodou); `git status` confirma exatamente os quatro arquivos novos esperados (três migrations + o script), zero resíduo.
