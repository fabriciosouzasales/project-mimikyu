...
**Piloto real não executado nesta rodada**: `JUSTTCG_API_KEY` ausente no ambiente de execução do agente — comportamento esperado e deliberado do script (`--fixture-check`, automático quando a variável falta), nunca solicita a chave em texto nem declara o piloto real aprovado. A lógica de sanitização, normalização, mapeamento de condição, detecção de idioma e identidade de idempotência foi validada por um harness equivalente (`node`, 16/16 asserções aprovadas) e a sintaxe TypeScript do script confirmada bem-formada (`ts.transpileModule`, zero diagnóstico) — o runtime Deno em si não está disponível neste sandbox de execução (`deno.land` e o pacote `npm:deno` fora da allowlist de rede), mesma classe de limitação já registrada para `npm run build`/`lint` do frontend. Fabrício pode rodar `deno run --allow-env scripts/sync-justtcg-pricing.ts --fixture-check` localmente para a validação nativa completa, e o piloto real (`--confirmed-by=<admin_user_uuid>`, com `JUSTTCG_API_KEY` configurada) quando decidir exercitá-lo.
