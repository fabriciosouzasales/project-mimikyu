...
## Ponto de Retomada — P13.4

Próximo incremento real da frente P13, e único item que falta para o incremento como um todo ser declarado concluído: **habilitação de `pg_cron`/`pg_net`, criação do Vault secret dedicado do lado Postgres, e o Job de agendamento real** chamando `ptax-fx-refresh` (já implantada e validada por piloto real, P13.3) periodicamente, sem intervenção manual. Nenhuma chamada à JustTCG dentro dessa frente — fora de escopo de P13 por completo. O `service_role` nunca deve ser exposto ao chamador da função; a Edge Function usa suas próprias credenciais internas, nunca repassa privilégio elevado para fora. Decisões de implementação em aberto: nome definitivo do Job de Cron; frequência real de agendamento; tamanho de retry/backoff da PRÓPRIA invocação do Cron (distinto do retry HTTP ao BCB, já implementado em P13.2) — nenhuma delas decidida ainda. O frontend nunca deve disparar sincronização, em nenhuma etapa futura — visualização de preço e sincronização permanecem arquiteturalmente desacopladas, mesmo desenho válido desde o P12.

Documentos atualizados neste encerramento: `docs/adr/ADR-031-scheduled-pricing-orchestration.md` (revisões `1.6`/`1.7`), `docs/ROADMAP.md`, `docs/README.md`/`docs/INDEX.md` (se aplicável), handoff vigente, `docs/log.md`, este documento. Nenhuma alteração de código, banco, migration ou configuração nesta rodada de encerramento documental — o deploy e o piloto real em si já haviam sido executados no ciclo anterior desta mesma frente, registrados acima. Nenhum commit/push realizado.
