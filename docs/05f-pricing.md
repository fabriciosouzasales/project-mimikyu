...
## Encerramento Formal — Pricing V1 (2026-08-27)

Em 2026-08-27, dois fechamentos reais na mesma sessão: (1) `npm run build` local aprovado por Fabrício para a integração do histórico manual de preço (migration `3971` — série `MANUAL` combinada ao histórico automático via `UNION ALL` em `admin_get_pricing_report_card`, preservando condição/período/`price_origin`), encerrando **Preços Manuais**, **integração de valuation** e **histórico manual no relatório Preço por Carta**; (2) auditoria final objetiva e read-only de todo o módulo Pricing, escopo explícito de Fabrício: telas/rotas incompletas, funcionalidades sem ação real, inconsistências entre KPIs/filas/relatórios/valuation, gaps de segurança/performance/operabilidade bloqueantes, itens conscientemente adiáveis — sem reabrir Catálogo/Collections, sem melhorias cosméticas sem necessidade.

**Resultado da auditoria — BLOQUEADORES: nenhum encontrado.**

Achados principais (todos NÃO BLOQUEADORES):

- **Rotas/telas**: 13 telas funcionais (10 do nav em `nav-config.ts` + Preço por Carta/Valor por Set/Resolução de Mapeamentos, alcançadas via drill-down). Os 5 relatórios restantes da Central de Relatórios permanecem `ComingSoonPage` — padrão v2 deliberado (badge "Em breve", sem `<Link>`), não são links quebrados.
- **Reconciliação KPI/fila/relatório/valuation** — bate exatamente: `get_pricing_admin_overview()` (`{total:7713, pending:0, confirmed:7695, not_found:18}`) = contagem direta de `pricing_card_mapping` (`GROUP BY match_status`) = fila de Mapeamentos de Cartas (`admin_list_pricing_card_mappings`) = lista de elegibilidade de Preços Manuais (`admin_list_pricing_manual_price_candidates`, total_count `18`). 46 Sets, todos `healthy`, zero `problem`/`processing`. Dispatcher agendado ativo, `last_sync_run.status=COMPLETED`/`triggered_by=SCHEDULED` real (não só configurado). Valuation de SWSH8 (`admin_get_pricing_report_set`: `estimated_value_covered=R$ 11.934,96`, 284/284 cartas, 100% cobertura) confirmada por soma independente — paginação manual (`p_limit=100`, offsets `0`/`100`/`200`) de `admin_get_pricing_report_set_cards` somando `price_display` das 284 linhas retorna exatamente `11934.96` — zero drift entre o agregado da RPC e a soma per-carta.
- **Achado técnico registrado, não bloqueante**: `admin_get_pricing_report_set_cards` clampa `p_limit` internamente em 100 (`v_limit := LEAST(GREATEST(COALESCE(p_limit, 20), 1), 100)`), sem sinalizar isso ao chamador — uma chamada com `p_limit=500` devolve silenciosamente 100 linhas, embora `total_count` reporte o total real corretamente em cada linha. A UI atual nunca solicita mais que sua própria página (paginação client-side menor que 100), então não há impacto em produção — vale como limitação de contrato conhecida da RPC para uma eventual exportação completa futura.
- **Segurança** (`get_advisors(type=security)`, projeto `qjfutqujxrbzgrtkpgkg`): zero achado `ERROR`. 81 `WARN` no total, nenhum específico de Pricing além do padrão sistêmico já aceito e usado em toda a aplicação — `authenticated_security_definer_function_executable` em cada RPC `admin_*` do app inteiro (Catálogo, Card Variant e Pricing), guardada por checagem `is_admin()` interna ao corpo da função, não por `REVOKE` a nível de `GRANT`; arquitetura já validada em rodadas anteriores de hardening (Query `2134` e correlatas). As 4 ocorrências de `function_search_path_mutable` são todas de funções do Catálogo/Card Variant (`enforce_user_profile_invariants`, `validate_card_variant_game_consistency`, `normalize_catalog_variant_import_job`/`_row`), nenhuma de Pricing — todas as RPCs de Pricing auditadas já usam `SET search_path TO ''`.
- **Performance** (`get_advisors(type=performance)`): zero achado `WARN`/`ERROR` específico de Pricing. Únicos achados em tabelas `pricing_*` são `INFO` — foreign keys sem índice de cobertura em tabelas de baixo tráfego administrativo (`pricing_admin_action_log`, `pricing_manual_price`, `pricing_product` — `fk_pricing_product_identity_same_mapping`, `pricing_refresh_policy`, `pricing_set_bootstrap_state`, `pricing_set_refresh_state`, `pricing_source_card_identity`) e 1 índice nunca usado (`ix_pricing_product_language_id`).
- **STD-004**: busca por `<select>`/`<textarea>` nativo em `web/app/pricing/**` e `web/components/pricing/**` retorna zero ocorrência — o débito de ~9 telas documentado em `docs/log.md` é inteiramente fora do módulo Pricing.

Fabrício aprovou a auditoria e declarou explicitamente: **"PRICING V1 ESTÁ CONCLUÍDO."** Instrução: não fazer novas melhorias funcionais ou cosméticas no módulo nesta rodada.

**Itens conscientemente adiados (registrados por decisão explícita de Fabrício, sem cronograma):**

1. Cap de 100 em `admin_get_pricing_report_set_cards` (`p_limit` acima de 100 clampado silenciosamente).
2. Coluna `#` de ranking em Valor por Set (recomendação de remoção pendente de decisão visual — puramente cosmético).
3. Índices/FKs informativos (`INFO`) de `get_advisors(performance)` em tabelas de baixo tráfego.
4. Os 5 relatórios futuros da Central de Relatórios (`ComingSoonPage`): Preço por Condição, Histórico de Preços, Comparativo de Fontes, Cartas Mais Valiosas, Tendências.

Itens preexistentes ao V1, também não bloqueantes: Fase B — Resíduos de Pricing (trabalho operacional contínuo, não um gate), gap de impressão não auditado em `PrecoPorCartaPrintFolha`, migrations `3943`/`3948` sem arquivo físico versionado (conteúdo confirmado).

**Próxima frente comprometida: Collections**, começando por visão de produto/North Star UX antes de qualquer implementação física — decisão explícita de Fabrício, diferente das retomadas anteriores desta mesma frente. Ver `docs/development/HANDOFF-2026-08-27.md` e `docs/ROADMAP.md`, revisão `1.72`.

## A. Estado atual do Pricing

Pricing está **operacional em produção**. `JUSTTCG` é a fonte ativa (`is_active = TRUE` desde P14.1, 2026-08-19) de dados USD/internacionais; PTAX é a fonte ativa de câmbio. O refresh de preços é automático (dispatcher durável por Set, ver seção B), cobrindo os 45 Sets do catálogo no ciclo atual. `PENDING`/`NOT_FOUND` são tratados como backlog administrável (Fase B — Resíduos de Pricing, iniciada em 2026-08-22 e ainda em andamento — ver seção "Incremento — Política de Frequência, Pricing Admin...", abaixo neste documento), nunca como dado silenciosamente ausente — princípio geral do domínio: **"melhor `PENDING` explícito do que identidade econômica errada."** Fase B é trabalho operacional/evolutivo contínuo, não um gate que precisa chegar a zero antes de outras frentes (Collections incluída) poderem avançar.
...
Elegibilidade de um Set para refresh é determinada por **política de frequência** (`pricing_refresh_policy`, migration `3937`, `CONFIRMADO EXECUTADO` em 2026-08-22): `frequency_days` restrito a `1`/`2`/`3`/`5`, seed `JUSTTCG=1`, ajustável por RPC administrativa (`admin_set_pricing_refresh_frequency()`). Mudança de frequência é **lazy** — só vale a partir do próximo `SUCCESS` daquele Set, nunca reagenda retroativamente. `next_due_at` (armazenado em `pricing_set_refresh_state`) é recalculado por `close_pricing_set_refresh_attempt()` no branch `SUCCESS`, somando `frequency_days` ao momento da conclusão (fallback de 1 dia se não houver política configurada). Migration `3938` (`CONFIRMADO EXECUTADO`) corrigiu um `ON CONFLICT` ambíguo introduzido pela `3937`. **Desde P16 (migration `3959`, 2026-08-26)**, `open_pricing_set_refresh_attempt()` exige também `pricing_set_bootstrap_state.status = 'COMPLETE'` para o Set, além da identity `CONFIRMED` PRIMARY/ALTERNATE já exigida — ver subseção I, abaixo, para o onboarding automático que precede esse gate.

## I. Onboarding Automático de Sets — Bootstrap (P16, 2026-08-25→26)

Novo pipeline **Set Mapping → Card Bootstrap → Price Refresh** fecha a lacuna manual entre um Set ganhar correspondência confirmada e ele entrar no ciclo de refresh automático — antes do P16, a descoberta/persistência de mapeamentos de carta de um Set novo exigia execução manual de CLI (P14). Confirmação administrativa do Set (`admin_confirm_pricing_set_mapping()`, migration `3951`) abre `pricing_set_bootstrap_state` (`PENDING`→`ACQUIRING`→`MATCHING`→`COMPLETE`, migrations `3952`/`3953`); a Edge Function `justtcg-set-bootstrap` adquire cartas em staging (`pricing_set_bootstrap_card_staging`, migration `3954`) e, ao concluir, executa matching local reaproveitando o núcleo de P16.2, persistindo em lote (`persist_pricing_bootstrap_card_batch`, migration `3958`) com autoria automatizada via `confirmed_sync_run_id` (migration `3957`). Um novo job `pg_cron` (`justtcg-set-bootstrap-dispatcher`, migration `3960`, `2-59/5 * * * *`, deslocado 2 min do dispatcher de PRICE_REFRESH) automatiza essa execução — serialização `CARD_SYNC`×`PRICE_REFRESH` segue garantida pela `3926` (P15), sem lógica nova. Prova real de ponta a ponta em `SWSH8` (284/284 mappings/identities `CONFIRMED`, captura automática pelo dispatcher de preços já ativo, 2.022 produtos/observações, valuation R$ 11.934,96). Detalhamento completo na seção "Incremento P16", mais adiante neste documento.
...
**Fase B — Resíduos de Pricing, iniciada (2026-08-22).** Objetivo: cada `PENDING`/`NOT_FOUND` residual ter uma causa identificável — nunca um mapeamento forçado só para reduzir a contagem — não uma meta numérica de zerar o backlog. `PENDING`/`NOT_FOUND` explícitos continuam aceitáveis quando economicamente corretos, mesmo princípio geral já registrado na seção "Estado Atual do Pricing", item A: "melhor `PENDING` explícito do que identidade econômica errada." Migration `3936` (`CONFIRMADO EXECUTADO`) consolida o resíduo `SET_LOGO_STAFF_HOLO`→`STAFF_HOLO`; piloto real Baxcalibur (`SVP #019`) validado. **Fase B permanece em andamento, como backlog administrável — não um gate que bloqueia outras frentes** (Collections incluída) — não formalmente encerrada.
...
## Incremento P16 — Onboarding Automático de Sets por Fonte (2026-08-25→26), formalmente encerrado

Fecha a lacuna manual entre um `pricing_set_mapping` ganhar `match_status = 'CONFIRMED'` e o Set efetivamente entrar no ciclo de refresh automático de preços — até aqui, todo Set novo exigia descoberta e persistência de mapeamentos de carta via CLI (P14). Pipeline novo: **Set Mapping → Card Bootstrap → Price Refresh**.

**P16.1 — Elegibilidade e cobertura de Set Mapping** (migration `3950`, `CONFIRMADO EXECUTADO`): `pricing_set_mapping` ganha projeção de elegibilidade/cobertura por `LEFT JOIN`, incluindo Sets locais sem nenhum mapeamento ainda ("Sem mapeamento" no frontend). **P16.2 — Núcleo de matching extraído** (`_shared/pricing-justtcg-matching/`, sem migration): mesma classificação SAFE/AMBIGUOUS/ABSENT do CLI (P14.4.4), agora reaproveitável tanto pelo conector quanto pela Edge Function de bootstrap. **P16.3 — Pré-visualização somente leitura** (`pricing-set-matching-preview`, Edge Function nova, `verify_jwt=true`/admin real): descobre e classifica candidatos de um Set sem escrever nada, habilitando o botão "Sincronizar" no frontend (`/pricing/mapeamentos-sets`). **P16.4 — Confirmação administrativa** (`admin_confirm_pricing_set_mapping()`, migration `3951`, `CONFIRMADO EXECUTADO`): formaliza a promoção de um `pricing_set_mapping` para `CONFIRMED` a partir da pré-visualização, via Server Action/Dialog dedicados. **P16.4.1** (migration não numerada nesta lista — buckets de status derivados) refina a taxonomia de saúde exibida (Visão Geral/Saúde das Fontes/Mapeamentos de Sets) para incluir os novos estados intermediários de onboarding.

**P16.5 — Bootstrap de cartas por Set, ponta a ponta**: uma vez `CONFIRMED`, o Set entra em `pricing_set_bootstrap_state` (`PENDING`→`ACQUIRING`→`MATCHING`→`COMPLETE`, migrations `3952` — taxonomia — e `3953` — tabela de estado, ambas `CONFIRMADO EXECUTADO`) com staging append-only por fonte (`pricing_set_bootstrap_card_staging`, migration `3954`). A Edge Function `justtcg-set-bootstrap` (núcleo `_shared/pricing-justtcg-bootstrap/`, RPCs de abertura/checkpoint/fechamento de tentativa, migration `3955`, `CONFIRMADO EXECUTADO`) executa em fases por invocação — aquisição paginada da JustTCG (só staging, nunca `pricing_card_mapping`) e, ao concluir a paginação, matching local (núcleo de P16.2) persistido em lote por `persist_pricing_bootstrap_card_batch` (migration `3958`, `CONFIRMADO EXECUTADO`), que grava `pricing_card_mapping`/`pricing_source_card_identity` com autoria automatizada via a nova coluna `confirmed_sync_run_id` (migration `3957`, `CONFIRMADO EXECUTADO` — irmã de `confirmed_by`, mutuamente exclusiva por `CHECK`, nunca um ator fictício em `admin_user`) e fecha o `bootstrap_state` em `COMPLETE`, limpando o staging do Set. Migration `3956` faz o backfill de `bootstrap_state=COMPLETE` para os 45 Sets históricos já `CONFIRMED` antes do P16 existir, preservando-os elegíveis ao refresh sem exigir reprocessamento.

**Gate de segurança (migration `3959`, `CONFIRMADO EXECUTADO`)**: `open_pricing_set_refresh_attempt()` só libera um Set para `PRICE_REFRESH` quando `bootstrap_state.status = 'COMPLETE'` **e** já existir identity `CONFIRMED` PRIMARY/ALTERNATE — fecha uma janela de corrida real entre a fase de matching (que já grava identities linha a linha, antes do fechamento formal) e a captura pelo dispatcher de preços. Testado transacionalmente provando a vulnerabilidade real que o gate fecha (identity `CONFIRMED` sintética em `MATCHING` seria elegível sob o gate antigo, não sob o novo); os 45 Sets históricos permanecem 100% elegíveis (45/45 antes e depois).

**P16.5.6 — Agendamento automático do bootstrap** (migration `3960`, `justtcg-set-bootstrap-dispatcher`, `pg_cron 2-59/5 * * * *`, `CONFIRMADO EXECUTADO`, ativado em 2026-08-26): deslocado 2 minutos do dispatcher de `PRICE_REFRESH` (`*/5 * * * *`) para nunca colidir no mesmo tick — puramente operacional, já que a serialização real `CARD_SYNC`×`PRICE_REFRESH` continua garantida pelo índice único parcial da migration `3926` (P15), sem lógica nova. Nasceu `active=false`, ativado só após validação transacional completa; 3 execuções reais observadas pós-ativação, todas `HTTP 200`/`NO_WORK` (nenhum bootstrap pendente no momento), zero `CARD_SYNC` criado sem candidato, dispatcher de preços (`jobid=202`) inalterado.

**Prova real de ponta a ponta — `SWSH8` (Golpe Fusão)**: preview confirmou candidatos sem escrita; confirmação administrativa promoveu o Set a `CONFIRMED`; bootstrap executado em produção — 328 cartas adquiridas, matching 284/284 `SAFE`/`CONFIRMED` (zero `AMBIGUOUS`/`ABSENT`), 284 mappings/284 identities persistidos, `bootstrap_state=COMPLETE`; captura **automática** pelo dispatcher de `PRICE_REFRESH` já ativo em produção (nunca disparado manualmente) assim que o gate da `3959` passou a permitir — 2.022 `pricing_product`/2.022 `pricing_observation`, 284/284 cartas com preço, valuation R$ 11.934,96, Saúde 46/46 consistente nas 4 telas administrativas (Visão Geral, Saúde das Fontes, Mapeamentos de Sets, Sincronizações).

**P16 formalmente encerrado — onboarding de novos Sets agora é automático a partir da confirmação administrativa, sem exigir nenhuma execução manual de CLI.** Nenhuma mudança no núcleo de refresh de preços (P15/`ADR-032`) nem na lógica de matching em si (reaproveitada de P16.2). **Próxima frente comprometida volta a ser Collections** (decisão de 2026-08-23, retomada agora que P16 está concluído).

---

# RESOLVIDA — Bypass de Cardinalidade de Payload em `get_cards_pricing_summary(uuid[])` (2026-09-07)

**Status: RESOLVIDA em 2026-09-07** pela Query **`3972`**, validada pelo harness **`3860`** v1.1 em `18 TOTAL / 18 PASS / 0 FAIL / 0 NOT PROVEN`. Ver "Correção aplicada", ao final desta seção.

Esta seção nasceu como **PENDÊNCIA ABERTA** na mesma data, registrada pelo postcheck de fechamento da Binder/Layout Foundation. O diagnóstico abaixo é preservado integralmente — é o registro do defeito real que motivou a correção.

## Origem do achado

Encontrada pelo postcheck de fechamento da **Binder/Layout Foundation** (`COLLECTIONS-BINDER-LAYOUT-FOUNDATION-IMPLEMENTATION-01`, 2026-09-07), que varreu **todas** as funções `public` com parâmetro `uuid[]` — não apenas as do escopo daquela frente. É um achado adjacente: nada em Pricing foi alterado, nem naquela rodada nem nesta.

## O defeito

`public.get_cards_pricing_summary(p_card_ids uuid[])` — `SECURITY DEFINER`, `search_path = ''`, `owner = postgres`, `EXECUTE` para `authenticated` e negado a `anon` — avalia o teto de entrada assim, **antes** do `unnest`:
...
`array_length(x, 1)` mede **apenas a primeira dimensão**. Um payload multidimensional atravessa o teto:

```
array_fill(uuid, ARRAY[2, 300])
    array_ndims        = 2
    array_length(x, 1) = 2      <- passa pelo teto de 100
    cardinality(x)     = 600    <- o que o unnest realmente processa
```

O teto de 100 existe no papel, não no comportamento: uma RPC pública `SECURITY DEFINER` executa `unnest` + `DISTINCT` + a cadeia de CTEs de resolução de preço sobre um array de tamanho escolhido pelo chamador.

## Classe conhecida, já fechada em outro lugar

É **exatamente** a mesma classe corrigida em 2026-09-07 nas três RPCs bulk canônicas — `set_physical_cards_storage()` (`5024`), `allocate_physical_cards_to_collection()` (`5046`) e `deallocate_physical_cards_from_collection()` (`5047`) — pelas migrations `5138`–`5140`, validadas por `5820` (27/27). A correção lá foi trocar o teto para `cardinality()` e adicionar um contrato de forma `array_ndims(...) <> 1`, ambos avaliados **antes** de qualquer `unnest`/`DISTINCT`/resolve/lock/escrita, deixando **zero `array_length`** no corpo executável.

**Escopo da afirmação de segurança daquela rodada:** a classe está fechada nas RPCs do escopo dela — as três bulk e as quatro do Binder com `uuid[]`. **Não** se afirmou, e não se afirma aqui, ausência do bypass em todo o sistema. `get_cards_pricing_summary` é a exceção conhecida.

## Correção aplicada — Query `3972` (2026-09-07)

**`PRICING-PAYLOAD-CARDINALITY-HARDENING-01`**, rodada própria, seguindo o precedente de `5138`–`5140`.

### Divergência de premissa corrigida antes de implementar

O mandato instruía a preservar `3903`/`3904`/`3918` como migrations históricas. **A lista estava incompleta.** A cadeia real de definição é `3903` → `3904` → `3918` → `3924` → **`3968`**, e a **definição efetiva no banco é a `3968`** (`DROP` + `CREATE`, integração do preço manual: fallback `MANUAL`, colunas `printing_label` e `price_origin`, join com `pricing_source_card_identity`). Escrever a correção a partir da `3918` **teria revertido silenciosamente o preço manual em produção**. A `3972` é cópia verbatim da `3968` com apenas o bloco de guards alterado.

**Fonte primária confirmada para este tipo de correção: a definição efetiva live via `pg_get_functiondef()`**, não o último arquivo de migration que a mente lembra.

### O que a `3972` faz

`CREATE OR REPLACE` (assinatura e retorno idênticos ⇒ sem `DROP`, OID e grants preservados). Guards antes do `RETURN QUERY` e portanto antes de qualquer `unnest`/`DISTINCT`/`LATERAL`:

1. `NULL` → `PRICING_SUMMARY_EMPTY_INPUT`
2. `cardinality(...) = 0` → `PRICING_SUMMARY_EMPTY_INPUT`
3. `array_ndims(...) <> 1` → `PRICING_SUMMARY_INVALID_PAYLOAD_SHAPE` *(código novo, `ERRCODE 22023`)*
4. `cardinality(...) > 100` → `PRICING_SUMMARY_TOO_MANY_CARD_IDS`

**Ordem obrigatória: vazio antes de dimensão.** `array_ndims('{}')` é `NULL` e `NULL <> 1` é `NULL` (não `TRUE`) — sem o teste de vazio primeiro, `'{}'` escaparia do guard de forma.

### Evidência

| Item | Resultado |
|---|---|
| Ledger | `20260907231143` |
| OID | `56577` — preservado |
| Retorno | 6 colunas preservadas |
| `SECURITY DEFINER` / `STABLE` / owner / ACL / `search_path` | preservados |
| `cardinality()` / `array_ndims()` / cap | posições 734 / 899 / 1054 |
| `unnest(p_card_ids)` | posição 1272 — **todos os guards antes** |
| `array_length(p_card_ids` executável | **0 ocorrências** |
| Lógica de negócio após `RETURN QUERY` | **byte-a-byte idêntica** à `3968` (md5 `858e53ff9740b1d3f0ffff829d2986e2`) |
| Harness `3860` v1.1 | **18 / 18 / 0 / 0** |
| Resíduo | zero — harness somente leitura |

**`N06`** — payload multidimensional com `array_length(x,1) = 2` e `cardinality = 600` — rejeitado com `PRICING_SUMMARY_INVALID_PAYLOAD_SHAPE`: **é a prova do fechamento**. `N05` (101 distintos) sozinho não provaria nada, passaria também com o `array_length` antigo.

Defeito real encontrado durante a execução, **no harness e não na migration**: `min(uuid)` não existe em PostgreSQL (`42883`), usado no grupo `R` do `3860` v1.0. Corrigido para `ORDER BY id LIMIT 1` na v1.1, sem alterar nenhum caso de teste nem o gate.

**Nenhuma mudança de comportamento para chamadas legítimas.** O caller único — `web/app/api/cards/pricing/batch/route.ts`, linha 348 — já garante array plano de ≤ 100 UUIDs válidos e deduplicados. Custo adicional no caminho feliz: quatro comparações escalares; `cardinality()` e `array_ndims()` são O(1) sobre o cabeçalho do array.

Staging em `database/proposals/2026-09-07-pricing-payload-cardinality-hardening/`. **`3972` promovida para `database/migrations/` em 2026-09-08** (`SCHEMA-PROMOTION-RECONCILIATION-01`) — destino canônico da faixa `3xxx`, onde o último arquivo anterior era `3971`. Promoção individual, **sem fold-in em `3968`**: a cadeia `3903`→`3904`→`3918`→`3924`→`3968`→`3972` permanece legível como sequência. Corpo executável byte-idêntico ao aplicado (SHA-256 `ab3b14d28365ff34d8c07a9fa4b52903785901ff29c508b07df60bf1f99f6cb6`); o harness `3860` **não** é promovido e permanece na proposal como evidência histórica.

## Ainda em aberto — hardening de contrato `UUID[]` / admin (fora do escopo desta correção)

Registrado **separadamente** e deliberadamente **não** incluído na rodada de Pricing: `admin_decide_catalog_import_row` (o guard `array_length(p_row_ids, 1) <> 1`, que exige "exatamente uma linha", é burlável por um array 2-D com `dim1 = 1` — defeito de **contrato de entrada**, sem vetor de escrita indevida em massa) e os usos que só checam `IS NULL` sem teto algum (`admin_decide_catalog_variant_import_row`, `create_reference_based_pokedex_collection`, `set_collection_pokedex_scope`, `admin_confirm_catalog_import`, `admin_confirm_catalog_variant_import`). Permanecem como **hardening de contrato `UUID[]` / admin**, para rodada própria — não bloqueiam nenhuma frente.
...
| 1.54 | **Micro-rodada complementar de governança documental (2026-08-24), a pedido explícito de Fabrício — exclusivamente documental/rastreabilidade, nenhum código/migration/RPC/Supabase alterado.** (1) Semântica de "Fase B — Resíduos de Pricing" corrigida em dois pontos: o "Objetivo" da seção do Incremento estava redigido de forma ambígua ("nenhum `PENDING` inexplicado"), com risco de leitura como meta numérica de zerar o backlog — corrigido para "cada `PENDING`/`NOT_FOUND` residual ter uma causa identificável, não uma meta numérica", e explicitado que Fase B é backlog administrável/trabalho contínuo, não um gate que bloqueia outras frentes (Collections incluída); a referência solta "(Fase B, ver abaixo)" na seção "Estado Atual" (item A) recebeu apontamento explícito para a seção onde Fase B é de fato detalhada. (2) Migrations `3943` e `3948` — ambas `CONFIRMADO EXECUTADO` mas sem arquivo físico versionado (gap já registrado nesta mesma seção desde a revisão `1.53`) — reconstruídas e versionadas em `database/migrations/`, com SQL extraído diretamente de `supabase_migrations.schema_migrations` no Supabase (fonte primária, não reconstrução por inferência); conteúdo da `3948` confirma integralmente a descrição já registrada nesta seção (conversão FX no histórico por data de observação, `price_display`/`fx_status` explícitos, nunca cotação atual retroativa). (3) `docs/standards/STD-001-database-standards.md` corrigido — trecho stale que ainda descrevia Pricing como "domínio ainda não operacional" (verdadeiro em 2026-08-16, obsoleto desde P14.1/P15) substituído por apontamento ao estado vigente nesta seção. Nenhuma reabertura de decisão passada; nenhuma alteração de comportamento em produção. |
| 1.55 | **Incremento P16 — Onboarding Automático de Sets por Fonte, formalmente encerrado (2026-08-25→26).** Nova subseção I na Síntese Executiva e novo incremento cronológico completo (P16.1–P16.5.6) documentando o pipeline **Set Mapping → Card Bootstrap → Price Refresh**: elegibilidade/cobertura de Set Mapping (migration `3950`); núcleo de matching extraído (`_shared/pricing-justtcg-matching/`); pré-visualização somente leitura (`pricing-set-matching-preview`); confirmação administrativa (`admin_confirm_pricing_set_mapping()`, migration `3951`); `pricing_set_bootstrap_state`/staging (migrations `3952`–`3954`); Edge Function `justtcg-set-bootstrap` e RPCs de bootstrap (migration `3955`); backfill dos 45 Sets históricos (migration `3956`); autoria automatizada via `confirmed_sync_run_id` (migration `3957`) e persistência em lote (migration `3958`); gate `bootstrap_state.status = 'COMPLETE'` em `open_pricing_set_refresh_attempt()` (migration `3959`); agendamento automático do bootstrap (`justtcg-set-bootstrap-dispatcher`, migration `3960`, ativado e validado com 3 execuções reais). Prova real de ponta a ponta em `SWSH8` (284/284 mappings/identities `CONFIRMED`, captura automática pelo dispatcher de preços, valuation R$ 11.934,96, Saúde 46/46 nas 4 telas administrativas). Nenhuma mudança no núcleo de refresh de preços (`ADR-032`) nem na lógica de matching em si. **Próxima frente comprometida volta a ser Collections** (decisão de 2026-08-23, retomada). Ver `ADR-032`, revisão `1.5`, e `docs/development/HANDOFF-2026-08-26.md`. |
| 1.56 | **Pricing V1 formalmente encerrado (2026-08-27), a pedido explícito de Fabrício.** Fechamento do histórico manual de preço (migration `3971`, série `MANUAL` combinada ao histórico automático em `admin_get_pricing_report_card`), aprovado por `npm run build` local completo. Nova seção "Encerramento Formal — Pricing V1", logo após a "Síntese Executiva", registra o resultado da auditoria final objetiva pedida por Fabrício (rotas/telas, reconciliação KPI/fila/relatório/valuation, segurança/performance/operabilidade): **zero bloqueador** — 13 telas funcionais, 5 relatórios `ComingSoonPage` deliberados, reconciliação exata `get_pricing_admin_overview()`/fila de Mapeamentos de Cartas/elegibilidade de Preços Manuais (18=18), valuation de SWSH8 confirmada por soma independente das 284 linhas (`R$ 11.934,96`), zero achado de segurança/performance específico de Pricing nos advisors, zero débito STD-004 no módulo. Achado técnico registrado, não bloqueante: `admin_get_pricing_report_set_cards` clampa `p_limit` em 100 silenciosamente. Fabrício aprovou e declarou "PRICING V1 ESTÁ CONCLUÍDO". Itens conscientemente adiados, sem cronograma: cap de 100 na RPC citada, coluna `#` de Valor por Set, índices/FKs informativos, os 5 relatórios `ComingSoonPage`. **Próxima frente comprometida: Collections, começando por visão de produto/North Star UX antes de qualquer implementação física** — decisão explícita de Fabrício. Ver `docs/development/HANDOFF-2026-08-27.md` e `docs/ROADMAP.md`, revisão `1.72`. |
| 1.57 | **Pendência aberta registrada: bypass de cardinalidade de payload em `get_cards_pricing_summary(uuid[])` (2026-09-07, `COLLECTIONS-BINDER-LAYOUT-FOUNDATION-DOCUMENTATION-CLOSEOUT-01`). Nenhuma correção aplicada a Pricing nesta rodada; nenhum SQL executado.** Nova seção "PENDÊNCIA ABERTA — Bypass de Cardinalidade de Payload em `get_cards_pricing_summary(uuid[])`" documentando o defeito com o mecanismo exato: o teto `IF array_length(p_card_ids, 1) > 100` é avaliado **antes** do `unnest`, e `array_length(x, 1)` mede só a primeira dimensão — `array_fill(uuid, ARRAY[2,300])` tem `array_length(x,1) = 2` (passa pelo teto) e `cardinality(x) = 600` (o que é efetivamente processado). Função é `SECURITY DEFINER`, `search_path = ''`, `EXECUTE` para `authenticated`. **Mesma classe** já fechada em 2026-09-07 nas três RPCs bulk canônicas (`5138`–`5140` sobre `5024`/`5046`/`5047`, validadas por `5820` 27/27), onde o padrão adotado foi `cardinality()` + `array_ndims(...) <> 1` antes de qualquer `unnest`, com zero `array_length` no corpo executável. Achado veio do postcheck de fechamento da Binder/Layout Foundation, que varreu todas as funções `public` com parâmetro `uuid[]`. Próxima ação obrigatória do projeto: **`PRICING-PAYLOAD-CARDINALITY-HARDENING-01`**, gate que precede Bulk Collection Operations. Registrados à parte, fora do escopo de Pricing: `admin_decide_catalog_import_row` (guard `<> 1` burlável por array 2-D com `dim1 = 1` — defeito de contrato, sem escrita indevida em massa) e os usos apenas-`IS NULL` sem teto, como hardening de contrato `UUID[]`/admin. |
| 1.58 | **`PRICING-PAYLOAD-CARDINALITY-HARDENING-01` — `IMPLEMENTED / VALIDATED / CLOSED` (2026-09-07).** A pendência aberta na revisão `1.57` foi **RESOLVIDA** pela Query **`3972`** (`CREATE OR REPLACE`, ledger `20260907231143`): o teto de 100 passou a ser medido por `cardinality()`, com contrato de forma `array_ndims(...) <> 1`, ambos avaliados antes do `RETURN QUERY` e portanto antes de qualquer `unnest`/`DISTINCT`/`LATERAL`; **zero `array_length(p_card_ids`** no corpo executável. Novo código de erro `PRICING_SUMMARY_INVALID_PAYLOAD_SHAPE` (`ERRCODE 22023`, mesmo dos guards pré-existentes) para um estado que antes era impossível de observar. **Divergência de premissa corrigida antes de implementar:** o mandato citava `3903`/`3904`/`3918`, mas a definição EFETIVA no banco é a **`3968`** — escrever a correção a partir da `3918` teria revertido silenciosamente o preço manual em produção; fonte primária passa a ser `pg_get_functiondef()`. Preservados: OID `56577`, assinatura, retorno de 6 colunas, `SECURITY DEFINER`/`STABLE`/owner/ACL/`search_path`, autenticação `28000`, e **toda a lógica de negócio** — provada **byte-a-byte idêntica** à `3968` (md5 `858e53ff9740b1d3f0ffff829d2986e2`). Validação: harness **`3860`** v1.1 = **18 TOTAL / 18 PASS / 0 FAIL / 0 NOT PROVEN**, com `N06` (multidimensional `dim1=2`, `cardinality=600`) rejeitado — a prova do fechamento; zero resíduo, harness somente leitura. Defeito real achado na execução, **no harness e não na migration**: `min(uuid)` não existe em PostgreSQL (`42883`), corrigido para `ORDER BY id LIMIT 1` sem alterar caso de teste nem gate. Staging em `database/proposals/2026-09-07-pricing-payload-cardinality-hardening/`, **não promovido** para `database/schema/`. Permanece fora de escopo, em rodada própria e não bloqueante: hardening de contrato `UUID[]`/admin (`admin_decide_catalog_import_row` e os usos apenas-`IS NULL`). Próxima frente do projeto: **Bulk Collection Operations**. |
| 1.59 | **`SCHEMA-PROMOTION-RECONCILIATION-01` — `CLOSED` (2026-09-08). Rodada exclusivamente de reconciliação de baseline: nenhum SQL executado, nenhuma migration aplicada, nenhuma lógica funcional nova, nenhuma mudança no banco.** Restaurada a representação canônica das migrations já `EXECUTED`/`VALIDATED` que estavam fora do destino canônico: `5104`–`5136` promovidos individualmente para `database/schema/`; **`5137` promovido com o corpo efetivo da correção `5141` foldada** (`5141` sem arquivo próprio em `schema/`); `5138`/`5139`/`5140` **foldados** em `database/schema/5024`/`5046`/`5047`; **`3972` promovida individualmente para `database/migrations/`**, sem fold-in em `3968`. Equivalência do corpo executável provada por SHA-256 em todos os casos. Proposals históricas preservadas intactas; harnesses (`5818`/`5819`/`5820`/`3860`), runbooks e READMEs de staging **não** promovidos; `database/validations/` fora de escopo. `BULK-01` permanece `CLOSED` e não foi reaberto; `BULK-02` **não foi iniciado** — próximo passo funcional é o **`GATE A` de `BULK-02` — `register_physical_cards_bulk`**. Nesta revisão, apenas o registro de destino de `3972`: onde constava "**Não promovido** para `database/schema/`", passa a registrar a promoção para **`database/migrations/`** — destino canônico da faixa `3xxx`, onde o último arquivo anterior era `3971` e onde `database/schema/` não contém arquivo `3xxx` algum. Promoção individual, **sem fold-in em `3968`**, mantendo legível a cadeia `3903`→`3904`→`3918`→`3924`→`3968`→`3972`; corpo executável byte-idêntico ao aplicado (SHA-256 `ab3b14d2…`). O harness `3860` permanece na proposal. A revisão `1.58` **não foi reescrita** — era factual no momento em que foi escrita. Nenhuma mudança de modelo, de RPC ou de banco. |
