...
# PENDÊNCIA ABERTA — Bypass de Cardinalidade de Payload em `get_cards_pricing_summary(uuid[])` (2026-09-07)

**Status: ABERTA. Não corrigida.** Registrada aqui por completude documental; a correção é a próxima ação obrigatória do projeto e **não** foi feita na rodada que a descobriu.
...
**`PRICING-PAYLOAD-CARDINALITY-HARDENING-01`** — rodada própria, seguindo o precedente de `5138`–`5140`: migration incremental (`CREATE OR REPLACE`, sem editar `3903`/`3904`/`3918`), assinatura/retorno/segurança/grants preservados, guards antes do `unnest`, e harness de validação exigindo prova comportamental com payload multidimensional (`array_length(x,1)` pequeno, `cardinality` acima do teto).

Este gate **precede obrigatoriamente** o início de **Bulk Collection Operations** — ver `ROADMAP.md` v1.89 e `README.md` v2.44.

Registrado **separadamente**, fora do escopo do hardening de Pricing: `admin_decide_catalog_import_row` (o guard `array_length(p_row_ids, 1) <> 1`, que exige "exatamente uma linha", é burlável por um array 2-D com `dim1 = 1` — defeito de **contrato de entrada**, sem vetor de escrita indevida em massa) e os usos que só checam `IS NULL` sem teto algum (`admin_decide_catalog_variant_import_row`, `create_reference_based_pokedex_collection`, `set_collection_pokedex_scope`, `admin_confirm_catalog_import`, `admin_confirm_catalog_variant_import`). Ficam como **hardening de contrato `UUID[]` / admin**, para rodada própria.
