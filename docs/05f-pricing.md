...
## Ponto de Retomada — P13.3

Próximo incremento real da frente P13: **Edge Function `ptax-fx-refresh`**, reutilizando exclusivamente o módulo compartilhado `supabase/functions/_shared/pricing-ptax/` (nenhuma lógica de busca/normalização/persistência duplicada). Autenticação serviço-a-serviço por secret key dedicada (Vault), validação manual conservadora e em tempo constante no próprio handler — nunca o pacote `@supabase/server`, ainda em Public Beta (decisão já registrada em `ADR-031`). Nenhuma chamada à JustTCG dentro dessa Edge Function — fora de escopo de P13 por completo. O `service_role` nunca deve ser exposto ao chamador da função; a Edge Function usa suas próprias credenciais internas, nunca repassa privilégio elevado para fora. Sequência recomendada: primeiro validar a invocação manual autenticada da Edge Function (sem agendamento), só depois habilitar `pg_cron`/`pg_net`/criar o Vault secret e agendar o Job real — salvo se o `ROADMAP.md` vigente já definir outra decomposição explícita no momento de retomada. O frontend nunca deve disparar sincronização, em nenhuma etapa futura — visualização de preço e sincronização permanecem arquiteturalmente desacopladas, mesmo desenho válido desde o P12.

Documentos atualizados neste encerramento: `docs/adr/ADR-031-scheduled-pricing-orchestration.md`, `docs/ROADMAP.md`, `docs/README.md` (se aplicável), `docs/INDEX.md` (se aplicável), `docs/development/HANDOFF-2026-08-18.md`, `docs/log.md`, este documento. Nenhuma alteração de código, banco, migration ou configuração nesta rodada — encerramento exclusivamente documental. Nenhum commit/push realizado.
