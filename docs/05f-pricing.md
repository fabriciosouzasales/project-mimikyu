...
**Item E — migration `3927`** (`STATUS: PROPOSTO`, não aplicada): 30 jobs `pg_cron` independentes (um por onda, nunca um job que encadeia todas), disparando diariamente inclusive fins de semana, `url`/`apikey` resolvidos por nome via Vault — mesmo padrão real de `3910` (PTAX). Fabrício decide quando aplicar.
...
**P15 (Itens A–D) formalmente validado em produção. Migration `3927` (agendamento Cron automático) permanece `PROPOSTA`** — o refresh diário automático só começa quando Fabrício autorizar a aplicação e provisionar os dois segredos do Vault; até lá, a Edge Function corrigida está disponível para invocação manual autenticada, exatamente como validado pelo piloto real desta rodada. Ver `ADR-032` para o detalhamento normativo completo.
