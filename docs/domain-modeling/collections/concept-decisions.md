...
**Status:** Aprovada — exemplo Pokédex revisado em 2026-09-03 (`COLLECTIONS-POKEDEX-DOCUMENTATION-FINAL-FIX-01`, ver nota abaixo)
...
- Pokédex: avaliar se a Primary Species da Card corresponde à Species da Position — Species Match. Correspondência exata é validação automática, sem confirmação (`assignment_basis = SPECIES_MATCH`). Mismatch, ou ausência de Primary Species, não bloqueia: o sistema emite aviso e permite confirmação explícita do usuário (`USER_OVERRIDE`), que conta normalmente para completion. Ver LDM-177 (Scope)/LDM-178 (Species Match/Mismatch)/LDM-179 (Position Assignment).

> **Nota (2026-09-03, `COLLECTIONS-POKEDEX-DOCUMENTATION-FINAL-FIX-01`).** O exemplo Pokédex acima foi revisado para eliminar leitura ambígua como bloqueio duro (`hard block`); a decisão central de C-33 (o sistema valida apenas pertencimento ao universo, sem regras de preferência pessoal) não foi alterada. Origem da correção: `COLLECTIONS-POKEDEX-DOCUMENTATION-FINAL-AUDIT-01` (auditoria read-only) identificou o texto anterior como `STALE_CONFLICT` frente ao modelo fechado em `COLLECTIONS-POKEDEX-MODELING-DOCUMENTATION-01` (ver `logical-model.md`, LDM-177 a LDM-179).
...
**Status:** Aprovada — atualizada em 2026-08-31 (`COLLECTIONS-CARD-CONDITION-RECONCILIATION-02`) para refletir a escala física real, confirmada por `COLLECTIONS-CARD-CONDITION-MINT-POSTCHECK-01`.

A escala canônica confirmada fisicamente em `card_condition` é: **M** (Mint), **NM** (Near Mint), **LP** (Lightly Played), **MP** (Moderately Played), **HP** (Heavily Played), **DMG** (Damaged) — `condition_order` 1 a 6, nessa ordem. Os codes longos anteriormente citados nesta decisão (`MINT`, `NEAR_MINT`, `LIGHTLY_PLAYED`, `MODERATELY_PLAYED`, `HEAVILY_PLAYED`, `DAMAGED`) nunca foram os codes físicos/canônicos — eram uma expectativa textual desta decisão, não a forma real gravada em `card_condition.code`. Não devem mais ser usados como codes físicos/canônicos.

> **Nota de fechamento (2026-08-31)**: a discrepância histórica registrada nesta decisão (validação pós-migration mencionando 5 registros vs. documentação listando 6 códigos) está **CLOSED**. A condição física `M`/Mint foi incluída via UI — passo referido por Fabrício como "PRE-PHYSICAL-GATE" —, e a auditoria read-only subsequente `COLLECTIONS-CARD-CONDITION-MINT-POSTCHECK-01` confirmou: (a) as 6 linhas físicas de `card_condition` são exatamente M/NM/LP/MP/HP/DMG, `condition_order` 1..6; (b) nenhum código do Pricing depende de "exatamente 5 condições" ou de um intervalo fixo de `condition_order` — conclusão **SAFE**, zero regressão. Ver `docs/05f-pricing.md` para o estado físico de `card_condition`.

## C-106 — Condition code canônico vs. label localizado

**Status:** Aprovada — exemplo atualizado em 2026-08-31 (`COLLECTIONS-CARD-CONDITION-RECONCILIATION-02`) para usar um code físico real.

O código de Condition (ex.: `NM`) é identidade estável, independente de idioma. O rótulo exibido ao usuário é localizado e traduzido separadamente (ex.: pt-BR "Praticamente Nova", en "Near Mint"). O código interno nunca é amarrado a um rótulo traduzido específico.

## C-107 — Evidência de mercado brasileiro

**Status:** Aprovada — nota de precisão adicionada em 2026-08-31 (`COLLECTIONS-CARD-CONDITION-RECONCILIATION-02`).
...
> **Nota de precisão (2026-08-31)**: as abreviações de mercado listadas acima são vocabulário externo observado, não identidade interna MMKYU. Em particular, o "D" de "D/Danificada" **não é** o code canônico — o code canônico é `DMG` (C-105); a coincidência entre "M" de "Nova" e o code canônico `M` é semântica de mercado, não uma derivação ou origem do code interno. Nomenclaturas externas podem variar por fonte e não criam, alteram nem substituem `card_condition.code`. Qualquer equivalência entre um código externo (de uma fonte de Pricing — ex. JustTCG — ou de qualquer outra origem) e a escala canônica pertence exclusivamente à camada de mapping (`pricing_condition_mapping` ou equivalente futuro), nunca ao vocabulário canônico em si.
...
Adicionado ao final de `COLLECTIONS-ACTIVITY-HISTORY-AUDIT-CONSOLIDATION-01`, encerrando a subfrente `Collections — Activity History / Audit conceptual modeling`, aberta pelo memo `COLLECTIONS-ACTIVITY-HISTORY-AUDIT-MODELING-01` (previamente registrado sem edição de arquivo, revisado por Fabrício, sem rodada de correção intermediária). Não reabre em conteúdo: Collection core semantics, Physical Card/Inventory, Custody/Availability, Storage, Layout/Page/Slot, Lifecycle/Provenance, Favorite, Wishlist, Physical Card Condition, Grading/Certification, Collection Collaboration/Permissions.

### C-166 — Três Camadas

**Status:** Aprovada

`Physical Card Lifecycle/Provenance`, `Collection Activity History` e `Audit Log` são conceitos distintos; nenhuma camada substitui as demais. Um mesmo acontecimento pode eventualmente alimentar mais de uma camada, mas possuem propósito, audiência e exposição diferentes.

### C-167 — Collection Activity History

**Status:** Aprovada

Collection Activity History é a sequência temporal de acontecimentos de domínio significativos que permitem ao usuário compreender como uma Collection evoluiu ou foi operada. É Collection-scoped, user-facing, semanticamente compreensível — não é simples visualização de mutações técnicas.

### C-168 — Activity Trigger

**Status:** Aprovada

Refinamento sobre o memo: Activity não é limitada literalmente a "ação que muda diretamente o estado da Collection" — representa acontecimento de domínio significativo relacionado à evolução/operação da Collection, normalmente decorrente de mudança persistente de estado. Não geram Activity: leitura, navegação, tentativa sem efeito, log técnico interno, mutação irrelevante ao usuário. System outcomes relevantes podem futuramente produzir Activity se satisfizerem o mesmo critério semântico.

### C-169 — Semantic Activity

**Status:** Aprovada

Activity usa linguagem de domínio (ex.: "Card movida do Slot 12 para o Slot 24"), nunca nomes de campo ou schema técnico (ex.: "slot_position changed").

### C-170 — Categorias

**Status:** Aprovada

Reconhecidas como candidatas, sem lista exaustiva: Collection metadata; Layout; Page; Slot/Region; Expected Content; Slot Assignment; Collection Allocation; Membership; Visibility; Archive; operações relevantes do sistema. Catálogo definitivo não fechado nesta rodada.

### C-171 — Actor

**Status:** Aprovada

Activity pode possuir ator quando aplicável: Owner, Editor, System. Viewer e Public User não alteram estado no modelo V1 e, portanto, não geram Activity por simples leitura.

### C-172 — Activity Visibility

**Status:** Aprovada

Activity History é visível no V1 para Owner, Members EDITOR e Members VIEWER. Não é exposta automaticamente a Public User, mesmo quando Collection = PUBLIC. Public Access ≠ acesso ao histórico interno de colaboração.

### C-173 — Member Exit

**Status:** Aprovada

Activity histórica permanece após saída/remoção do Member; a autoria histórica permanece associada ao acontecimento. Isso não concede direito residual do ex-Member sobre a Collection. Questões futuras de account deletion/anonymization ficam fora desta rodada.

### C-174 — Privacy

**Status:** Aprovada

Activity não expõe automaticamente amount paid, seller/buyer, dados financeiros, Storage, private Provenance, Favorite, Wishlist ou outros dados privados. A narrativa user-facing contém apenas informação necessária ao entendimento da mudança na Collection.

### C-175 — Audit (definição)

**Status:** Aprovada

Audit Log é camada separada destinada a governança, segurança, accountability, investigação e suporte. Deve permitir conceitualmente reconstruir quem fez o quê, quando, sobre qual contexto/entidade. Schema físico não fixado nesta rodada.

### C-176 — Audit Coverage

**Status:** Aprovada

Cobertura proporcional a risco e necessidade de accountability. Prioridade obrigatória conceitual para: (A) ações Owner-sensitive — Membership, Visibility, Collection Allocation, Archive/Delete, demais ações sensíveis; (B) ações state-changing executadas por Collaborators — Layout, Expected Content, Slot Assignment, metadata editável. Motivação: Owner deve poder futuramente responder "quem alterou o quê na minha Collection?". Isso não significa auditar toda mutação técnica ou interação de UI.

### C-177 — Activity × Audit

**Status:** Aprovada

Reconhecidas quatro possibilidades: Activity + Audit; Audit-only; Activity-only; nenhuma. Matriz exaustiva não fechada nesta rodada. Exemplo: Visibility PRIVATE → PUBLIC gera Activity + Audit; erro/tentativa negada relevante à segurança gera Audit-only.

### C-178 — Bulk / Grouping

**Status:** Aprovada

Activity deve suportar conceitualmente operações em massa sem obrigar histórico ruidoso (ex.: preferível "7 Slots removidos" a sete linhas idênticas). Não decidido se a implementação será evento agregado, eventos individuais agrupados na apresentação, ou ambos — detalhe futuro.

### C-179 — Immutability

**Status:** Aprovada

Enquanto existirem, registros históricos não são editáveis, reescritos silenciosamente, nem apagáveis livremente por Users comuns. Política de retenção e hard-delete fica fora desta rodada.

### C-180 — Retention

**Status:** Aprovada

Activity History e Audit podem possuir políticas de retenção diferentes. TTL/prazos não definidos nesta rodada.

### C-181 — Archive / Delete

**Status:** Aprovada

Não presumir que Activity, Audit e Physical Card Lifecycle possuem o mesmo ciclo de vida. Physical Card Lifecycle permanece independente da Collection (reafirma C-47/C-67). Destino de Activity/Audit após archive/hard-delete da Collection permanece em aberto.

### C-182 — History ≠ Undo

**Status:** Aprovada

History não implica Undo, Restore ou rollback automático. Reversibilidade exige modelagem própria futura.

### C-183 — History ≠ Current State

**Status:** Aprovada

Activity History e Audit não são fonte única obrigatória para reconstruir estado corrente no V1. Não se adota event sourcing implicitamente. Estado corrente continua modelado pelas entidades de domínio.

### C-184 — System Activity

**Status:** Aprovada

System pode ser actor de Activity quando houver acontecimento semanticamente relevante ao usuário. Log técnico não é transformado em Activity automaticamente.

### C-185 — Activity ≠ Lifecycle

**Status:** Aprovada

Não se duplicam automaticamente fatos de Physical Card Lifecycle. Ownership Entry/Transfer/Exit continuam pertencendo ao lifecycle (C-67–C-81, não reaberto). Se um fato patrimonial também tiver relevância explícita para uma Collection, uma Activity correspondente pode futuramente existir, mas os conceitos permanecem independentes.

### C-186 — Escopo mínimo V1

**Status:** Aprovada

**ACTIVITY HISTORY:** Collection-scoped; user-facing; linguagem de domínio; actor quando aplicável; timestamp; mudanças/acontecimentos relevantes; visível para Owner + Members; não pública por padrão; sem Undo; não é fonte de estado corrente; não substitui Lifecycle. **AUDIT:** camada separada; segurança/governança/accountability; cobre ações Owner-sensitive; cobre mudanças relevantes feitas por Collaborators; não necessariamente user-facing; não substitui Activity/Lifecycle; não é fonte de estado corrente.
...
Cada posição representa uma Pokémon Species canônica.

Para satisfação automática (`SPECIES_MATCH`), a Card utilizada deve possuir Primary Species correspondente à Species da posição. Caso a Card não corresponda, ou não possua Primary Species resolvida, a posição não fica automaticamente bloqueada: o sistema alerta e permite ao usuário confirmar a associação mesmo assim (`USER_OVERRIDE`), que satisfaz a posição normalmente para fins de completion.

> **Nota (2026-09-03, `COLLECTIONS-POKEDEX-DOCUMENTATION-FINAL-FIX-01`).** Formulação revisada — o texto anterior ("A Card utilizada deve representar aquele Pokémon") lia como exigência absoluta/bloqueio duro, incompatível com o modelo fechado por `COLLECTIONS-POKEDEX-MODELING-DOCUMENTATION-01`. Ver LDM-178 (Species Match/Mismatch) e LDM-181 (Completion) em `logical-model.md`.
...
- `Collection Activity History / Audit` — resolvido conceitualmente em 2026-08-30 (Bloco complementar `Collection Activity History / Audit`, C-166–C-186): três camadas conceitualmente distintas — Physical Card Lifecycle/Provenance (não reaberto), Collection Activity History (novo, narrativa user-facing de mudanças significativas na Collection, em linguagem de domínio) e Audit Log (necessidade já registrada em C-164, trilha de governança/segurança); Activity Trigger refinado para "acontecimento de domínio significativo", não mutação técnica; categorias candidatas não exaustivas; actor Owner/Editor/System; visibilidade Owner+Members, nunca automática a Public User; privacidade e immutability reafirmadas; quatro combinações Activity×Audit reconhecidas sem matriz fechada; grouping/bulk, retenção, archive/delete boundary e History≠Undo/Current State registrados como fronteiras, não resolvidos em detalhe. Modelagem física (SQL, tabelas, UUID, RLS, schema de Audit) permanece não iniciada.
- ~~`Pokémon / Subject Reference`~~ — resolvido em 2026-09-03: o adiamento da entidade foi revogado (ADR-011 v1.2), a entidade foi renomeada **Pokémon Species** (com Generation e Pokémon Form/Variety como conceitos subordinados), e a modelagem lógica de Pokédex/`REFERENCE_POSITION` está fechada em `logical-model.md`, LDM-175 a LDM-185 (`COLLECTIONS-POKEDEX-MODELING-DOCUMENTATION-01`). Modelagem física permanece não iniciada.
- engine de progresso
- Undo/Restore e reversibilidade de operações
- event sourcing (não adotado implicitamente)
- algoritmo de agrupamento de Activity em operações em massa
- retenção/TTL de Activity e Audit
- hard-delete policy completa sobre Activity/Audit
...
| 1.14 | **Reconciliação da escala física de Condition, 2026-08-31** (`COLLECTIONS-CARD-CONDITION-RECONCILIATION-02`), decorrente da auditoria read-only `COLLECTIONS-CARD-CONDITION-MINT-POSTCHECK-01` (a condição física `M`/Mint foi adicionada via UI — passo referido por Fabrício como "PRE-PHYSICAL-GATE" — e a auditoria confirmou zero regressão de Pricing). **C-105 atualizada**: a escala canônica deixa de ser descrita pelos codes longos "pretendidos" (`MINT`/`NEAR_MINT`/`LIGHTLY_PLAYED`/`MODERATELY_PLAYED`/`HEAVILY_PLAYED`/`DAMAGED`, nunca gravados fisicamente) e passa a refletir os codes físicos reais confirmados em `card_condition` — `M`/`NM`/`LP`/`MP`/`HP`/`DMG`, `condition_order` 1..6. A pendência histórica de contagem (5 registros citados vs. 6 códigos documentados) registrada por esta decisão desde 2026-08-30 está **CLOSED**. **C-106 atualizada**: exemplo de code trocado de `NEAR_MINT` para `NM`. **C-107 atualizada**: nota de precisão adicionada, distinguindo o "D" de mercado (Danificada) do code canônico `DMG`, e explicitando que equivalências de fonte externa (ex.: JustTCG) pertencem exclusivamente à camada de mapping (`pricing_condition_mapping`), nunca ao vocabulário canônico. Nenhuma nova C-* criada — C-105/C-106/C-107 mantiveram seus números originais, apenas conteúdo/status atualizados. C-01–C-186 não reabertas além do estritamente descrito acima. Pricing e banco permanecem intocados nesta rodada (documental pura). |
| 1.15 | **Correção do conflito documental Pokédex, 2026-09-03** (`COLLECTIONS-POKEDEX-DOCUMENTATION-FINAL-FIX-01`), decorrente da auditoria read-only `COLLECTIONS-POKEDEX-DOCUMENTATION-FINAL-AUDIT-01`, que classificou dois trechos como `STALE_CONFLICT` frente ao modelo fechado em `COLLECTIONS-POKEDEX-MODELING-DOCUMENTATION-01` (`logical-model.md`, LDM-177–LDM-181). **C-33 atualizada**: o exemplo Pokédex ("validar se a Card representa o Pokémon correto para aquela posição") foi revisado para descrever explicitamente Species Match (correspondência exata → `SPECIES_MATCH`, automático) vs. mismatch/ausência de Primary Species (aviso + confirmação → `USER_OVERRIDE`, nunca bloqueio), com pointer para LDM-177/LDM-178/LDM-179; a decisão central de C-33 (validação restrita a pertencimento ao universo, sem regras de preferência pessoal) não foi alterada. **Parte B.4 (Pokédex) atualizada**: a frase "A Card utilizada deve representar aquele Pokémon" foi substituída por formulação que distingue satisfação automática (Primary Species correspondente) de mismatch/ausência (aviso + `USER_OVERRIDE`, conta normalmente para completion), com pointer para LDM-178/LDM-181. **Parte D atualizada**: a entrada `Pokémon / Subject Reference`, antes listada como tema não resolvido, foi marcada como resolvida (riscada, com nota), apontando para a revogação do adiamento em ADR-011 v1.2 e para LDM-175–LDM-185 — texto histórico preservado. Nenhuma outra C-* revisitada. `logical-model.md`, `ADR-011` e banco/schema não foram tocados nesta rodada — fix estritamente documental, escopo idêntico ao dos três conflitos identificados na auditoria anterior. |
