...
230 - Create Asset Import Failure        (planejada — SQL ainda não escrita)
231 - Asset Import Failure Triggers      (planejada)
995 - Validate Asset Import Infrastructure (planejada)

880 - Seed Card Asset                    (planejada — escopo confirmado: apenas CARD_FRONT, card_id direto; camada estrutural 100% pronta; bloqueada até que `asset_import_failure` [`230`/`231`] e a validação consolidada [`995`] também estejam prontas, ver "Query 880"/"Bloqueio 5", abaixo)
```
...
**Bloqueio 5 — em andamento, três de quatro camadas parcialmente concluídas.** Fabrício corrigiu explicitamente a sequência antes proposta ("Não seguiremos agora para: 880 – Seed Card Asset"): o próximo passo é estrutural, construído em camadas, uma de cada vez, com pacote técnico completo antes de avançar — (1) **Asset Source** (`200`/`201`/`900`/`985` — CONFIRMADOS EXECUTADOS, ver seção "Asset Source", abaixo); (2) **Card External Reference** (`210`/`211`/`990` — CONFIRMADOS EXECUTADOS, ver seção "Card External Reference", abaixo; a Seed `910` foi deliberadamente **não criada**); (3) **camada de execução de importação — arquitetura revisada para um modelo híbrido `asset_import_run`/`asset_import_failure`** (em vez de `asset_import_job`/`asset_import_item`, planejado na revisão `0.46` mas nunca escrito em SQL); `asset_import_run` (`220`/`221`) **CONFIRMADOS EXECUTADOS nesta revisão** — ver "Query 220"/"Query 221", abaixo; `asset_import_failure` (`230`/`231`) e a validação consolidada da camada (`995`) **ainda não escritos**; somente então (4) desenvolvimento da Edge Function e um piloto controlado com uma coleção pequena; só depois disso, a carga em escala real de `880`. Este bloqueio absorve e substitui os antigos Bloqueios 1 (fonte oficial de imagens) e 2 (identificador externo).
...
> **⚠️ Ressalva de numeração e nomenclatura (revisão `0.47`) — em grande parte RESOLVIDA nesta revisão.** **Numeração**: a recomendação da revisão `0.47` (`220`/`221` para Create/Trigger de `asset_import_run`, `230`/`231` para `asset_import_failure`) foi adotada exatamente como sugerido pela sessão pareada, evitando a colisão com `920 - Validate Card Set` que teria ocorrido com o plano original da revisão `0.46`; `230`/`231` (Asset Import Failure) e `995` (Validate Asset Import Infrastructure) permanecem reservados, ainda não escritos. **Nomenclatura**: uma captura real do Table Editor, obtida nesta revisão (para investigar a confusão `collection`/`card_set` descrita acima), mostrou a lista de tabelas físicas então existentes — `asset_source`, `card`, `card_asset`, `card_asset_type`, `card_category`, `card_external_reference`, `card_set`, `card_variant`, `card_variant_type`, `expansion`, `game`, `language`, `rarity`, `storage_bucket` — **sem `asset_import_run`, `asset_import_failure` nem `card_set_external_reference`** (a captura foi feita antes da execução de `220`, então a ausência de `asset_import_run` já era esperada; mas `asset_import_failure` e `card_set_external_reference` deveriam aparecer se de fato fossem, como `06-pipeline-importacao.md` alega, tabelas físicas pré-existentes desde o início do projeto — nada neste projeto remove tabelas silenciosamente). Isso é evidência corroborante forte (mesmo padrão já confirmado para `asset_source` na revisão `0.46`) de que a lista de "17 tabelas físicas pré-existentes" de `06-pipeline-importacao.md` está desatualizada ou incorreta para pelo menos estes casos — mas **não é prova absoluta**, já que a captura de tela do painel lateral do Table Editor pode estar truncada/rolável e não ter mostrado a lista completa. Recomendação mantida, com urgência reduzida: repetir a guarda defensiva de `200`/`220` ao escrever `230` (`asset_import_failure`), e, se possível, confirmar a lista completa de tabelas via uma consulta direta a `information_schema.tables` em vez de uma captura de tela do painel.
...
880 - Seed Card Asset                    (bloqueada até `asset_import_failure` [`230`/`231`, ainda não escrita] e a validação consolidada [`995`] estarem prontas e um piloto controlado ser executado, ver "Arquitetura de Importação de Ativos", acima)
```
