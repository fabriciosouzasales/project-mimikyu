...
- **Bloco B — Pipeline de Importação.** Status: **ainda não iniciado.** Este é o primeiro grande trabalho de implementação que falta — as tabelas do Bloco A já sustentam esta camada, mas o worker que efetivamente executa o fluxo ainda não foi escrito. Objetivo: `Fonte Externa (API) → Descoberta e correspondência da Card → Download → Validação → Supabase Storage → card_asset`. Componentes: Edge Functions; integração real com a Pokémon TCG API e com a TCGdex; download; upload; geração automática de registros em `card_asset`; reprocessamento seletivo (via `asset_import_run`); tratamento de falhas (via `asset_import_failure`). Este bloco substitui, na prática, o antigo item "4" do Bloqueio 5 ("Edge Function + piloto controlado").
