...
**Card Asset Type: pacote técnico CONCLUÍDO E EXECUTADO** (`170`/`171`/`870`/`970`, ver "SQL confirmada" abaixo). **Card Asset: estrutura física já existente no Supabase; `180`/`181`/`980` (v1.1) confirmados executados. Governança de idioma e provedor de armazenamento (`193`/`194`) foi escrita e relatada como executada, mas uma inspeção real via Table Editor no mesmo dia (ver "Estrutura Física Real", abaixo) mostra a tabela sem a coluna `storage_provider` — contradizendo a execução relatada de `194` e lançando dúvida sobre `193`. Tratar a integração de `language`/governança de storage como "pendente de confirmação", não como concluída, até Fabrício esclarecer.** Nomenclatura final "Card Asset"/"Card Asset Type" (não "Card Image", nome inicialmente cogitado e depois generalizado — ver `04-domain-model.md` para o raciocínio completo, incluindo o exemplo Bulbasaur/Standard/Reverse Holo que motivou a generalização).
...
2. **`storage_bucket_id`** (FK para `storage_bucket`) no lugar de `storage_provider` (texto livre) — **divergência CONFIRMADA por nova inspeção real via Table Editor (Fabrício, mesmo dia).** A tabela real segue sem coluna `storage_provider` — apenas `storage_bucket_id` (uuid, FK), exatamente como documentado na revisão `0.30`. Isso **contradiz diretamente** a execução reportada da Query `194 - Govern Card Asset Storage Provider` (ver seção "Language", abaixo), cujo próprio bloco de validação de pré-requisito exige a existência de `storage_provider` antes de prosseguir — se a coluna realmente não existisse no momento da execução, a Query deveria ter abortado com `RAISE EXCEPTION`, não ter sido relatada como "Executada com sucesso". **Conclusão provisória, não resolvida unilateralmente**: a evidência real disponível hoje indica que `194` não alterou de fato esta tabela (apesar do relato de sucesso) — mesmo padrão de risco já observado em `170`/`180`, agora confirmado por inspeção direta em vez de apenas suspeitado. Recomenda-se fortemente que Fabrício confirme, no próprio Supabase, se `194` foi executada contra o projeto/schema correto antes de qualquer nova migration sobre `card_asset`.
3. **`language_id`** (FK para `language`) — coluna presente e com propósito explicado (idioma da imagem digital, não tradução editorial nem idioma do exemplar físico — ver seção "Language (Idioma)", abaixo, e "Três Dimensões de Idioma" em `04-domain-model.md`). **Nota importante desta revisão**: a nova inspeção via Table Editor confirma que `language_id` já existia na tabela real *antes mesmo* da Query `193 - Add Language to Card Asset` ser escrita — consistente com a listagem original da revisão `0.30`, que já incluía `language_id` entre as 17 tabelas pré-existentes. O passo `ADD COLUMN IF NOT EXISTS language_id` de `193` teria sido, portanto, um no-op para a coluna em si; as alterações de constraint/índice de `193` (unicidade e ativo principal por idioma) **não são verificáveis a partir de uma captura de colunas** — permanecem como "reportadas executadas", sem confirmação direta de que as constraints reais mudaram.
...
**Ativo principal e unicidade — REPORTADAS COMO EXECUTADAS (Query `193`), confiança rebaixada por evidência contraditória (ver "Estrutura Física Real", item 2, acima).** Regra anterior (sem dimensão de idioma): no máximo um `is_primary = TRUE` por `card_id` + `asset_type_id`, via índice único parcial `uq_card_asset_one_primary`; unicidade lógica por `card_id` + `asset_type_id` + `asset_order`. **Regra pretendida por `193`**: cada combinação `card_id` + `asset_type_id` + `language_id` teria seu próprio ativo principal — no máximo um `is_primary = TRUE` por `card_id` + `asset_type_id` + `language_id` (índice `ux_card_asset_primary_per_card_type_language`); unicidade lógica seria `card_id` + `asset_type_id` + `language_id` + `asset_order` (constraint `uq_card_asset_card_type_language_order`). **Ressalva desta revisão**: uma nova inspeção real via Table Editor (mesmo dia) mostrou a tabela `card_asset` com as mesmas 20 colunas já documentadas na revisão `0.30`, sem `storage_provider` — o que contradiz a execução reportada da Query `194` (que dependia dessa coluna existir) e, por extensão, levanta dúvida sobre se `193` de fato alterou as constraints reais (colunas não revelam constraints, mas o padrão de discrepância já visto com `170`/`180`/`194` recomenda cautela). Tratar como "pretendido e reportado, não confirmado" até nova evidência. Isso permite que a mesma Card tenha, por exemplo, um `CARD_FRONT` principal em português (`asset_order = 1`, `is_primary = TRUE`) e outro `CARD_FRONT` principal em inglês (`asset_order = 1`, `is_primary = TRUE`), sem conflito.
...
980 - Validate Card Asset Structure      (EXECUTADA v1.1 — sem erro, resultados numéricos não confirmados; precisará ser reescrita após 880)
...
193 - Add Language to Card Asset         (RELATADA EXECUTADA, não confirmada por inspeção real — ver "Language")
194 - Govern Card Asset Storage Provider (RELATADA EXECUTADA, CONTRADITA por Table Editor real — coluna storage_provider ausente, ver "Estrutura Física Real"/"Language")
890 - Seed Language                      (EXECUTADA — pt-BR/en, não depende de card_asset)

880 - Seed Card Asset                    (planejada — escopo confirmado: apenas CARD_FRONT, card_id direto; bloqueada pela fonte oficial de imagens, pelo identificador externo E por confirmar se 193/194 de fato aplicaram na tabela real, ver "Query 880", abaixo)
```
...
**Bloqueio 3 — dimensão de idioma — arquiteturalmente decidido, execução real NÃO confirmada.** Ao comparar duas imagens reais da mesma Card (`Rufflet`, `173/217`, ME2.5) — uma em português ("Rufflet do Lauro") e outra em inglês ("Larry's Rufflet") — Fabrício identificou que ambas representam a mesma Card, o mesmo Card Asset Type (`CARD_FRONT`), mas são duas **representações linguísticas distintas do mesmo ativo digital** — não dois Card Assets Types, não duas Cards, não duas Card Variants. Isso disparou a decisão de adicionar `language_id` como dimensão de `card_asset` (ver "Estrutura Física Real", acima, e a seção "Language", abaixo), com SQL escrita e relatada como executada via `190`/`191`/`192`/`193`/`890`. **Ressalva desta revisão**: uma inspeção real via Table Editor (mesmo dia) mostra `card_asset` sem `storage_provider`, o que contradiz a execução relatada da Query irmã `194` — isso não prova que `193` também falhou, mas justifica tratar `193` com a mesma cautela até confirmação. A `880` permanece bloqueada pelos Bloqueios 1 e 2 (fonte de imagens e identificador externo) **e agora também por esta pendência de confirmação**, antes de qualquer nova migration ou seed sobre `card_asset`.

**Bloqueio 4 (novo) — coluna `storage_bucket` decidida, ainda não escrita nem executada; dois riscos sinalizados nesta revisão.** Ver "Arquitetura de Armazenamento", logo abaixo, para o raciocínio completo.

Fabrício havia adiado anteriormente o detalhamento fino desta entidade e de `language`/`card_external_reference`/`card_set_external_reference` (tabelas físicas pré-existentes, ver `06-pipeline-importacao.md`): "Vamos chegar a detalhar essas três mais para frente. Vamos seguir o fluxo." — o detalhamento de `language` foi antecipado por conta do Bloqueio 3 e concluído nesta revisão.
...
**Nova coluna proposta e aprovada por Fabrício ("Vamos fazer essa mudança"): `storage_bucket`.** Racional apresentado pela sessão pareada: um Object Storage moderno tem três camadas — Provider → Bucket → Object (Path) — e o modelo atual só representa duas (`storage_provider`, `storage_path`). Estrutura proposta: `storage_provider = SUPABASE`, `storage_bucket = card-front`, `storage_path = ME1/001.png`; URL final montada pela aplicação como `https://[project_id].supabase.co/storage/v1/object/public/{storage_bucket}/{storage_path}`. **Nenhuma SQL foi escrita ou executada para esta mudança nesta revisão** — é uma decisão de design, não uma migration.

> **Risco 1, sinalizado nesta revisão (cross-check, não levantado por Fabrício nem pela sessão pareada): `storage_bucket` já existe como uma das 17 tabelas físicas pré-existentes do projeto** (ver lista completa em memória/histórico — `storage_bucket` está entre `asset_source`/`asset_import_run`/`asset_import_failure`, ainda não detalhadas neste documento). `card_asset.storage_bucket_id` (confirmado presente na captura real de Table Editor mais recente) é, muito provavelmente, uma FK para essa tabela `storage_bucket` já existente — uma entidade de referência própria, não uma coluna de texto livre. A proposta desta conversa ("adicionar uma coluna `storage_bucket` de texto") corre o risco de duplicar/conflitar com uma entidade que já existe e ainda não foi inspecionada — exatamente o mesmo tipo de erro já cometido com `card_asset`/`card_asset_type` (proposta feita sem checar a estrutura física real, batches 29-30) e possivelmente repetido com `storage_provider` (Bloqueio da revisão `0.42`). **Recomendação forte, não uma decisão**: antes de escrever qualquer migration para `storage_bucket`, inspecionar a tabela física `storage_bucket` real (estrutura, FKs, dados) e decidir se a nova coluna deve ser `storage_bucket_id UUID` (reaproveitando a entidade já existente) em vez de `storage_bucket TEXT` (uma nova coluna paralela).
>
> **Risco 2, sinalizado nesta revisão: a convenção de caminho final, como descrita, não contempla a dimensão de idioma.** Ao evoluir de "um bucket com subpastas por tipo e idioma" para "um bucket por tipo de ativo" (`card-front`/`artwork`/`card-back`), os exemplos finais mostrados (`ME1/001.png`) deixaram de incluir o idioma no caminho. Como `card_asset` agora distingue ativos por `language_id` (Bloqueio 3, acima), dois registros do mesmo Card Asset Type para a mesma Card em idiomas diferentes (ex.: `CARD_FRONT` em `pt-BR` e em `en`) apontariam para o mesmo `storage_bucket`+`storage_path` (`card-front`/`ME1/001.png`), o que é uma colisão — não há como os dois arquivos (potencialmente diferentes, ex.: `Rufflet do Lauro` vs. `Larry's Rufflet`) coexistirem sob o mesmo caminho. **Não resolvido nesta revisão** — precisa de decisão explícita de Fabrício antes de qualquer carga real: reintroduzir o idioma no caminho (ex.: `pt-BR/ME1/001.png`), usar um sufixo no nome do arquivo (ex.: `ME1/001-pt-BR.png`), ou criar buckets por idioma também.

**Fluxo de carga recomendado, ainda não iniciado**: (1) criar o(s) bucket(s); (2) enviar algumas imagens de teste; (3) validar o padrão de caminhos na prática; (4) inserir os respectivos registros em `card_asset` manualmente para essas poucas Cards; (5) só então automatizar a carga em lote (programa que percorre uma pasta local, envia cada imagem ao Supabase Storage e grava o registro correspondente em `card_asset`) — explicitamente recomendado para evitar popular os 859 registros da `880` antes das imagens realmente existirem (o que deixaria o banco apontando para centenas de caminhos vazios).
...
**Camada Language integralmente executada e integrada a `card_asset`.** `190`/`191`/`192`/`890` CONFIRMADOS EXECUTADOS; a integração com `card_asset` (`193 - Add Language to Card Asset`) também CONFIRMADA EXECUTADA — ver "Query 193", abaixo. Surgiu como pré-requisito direto da Query `880`: ao decidir que `card_asset` precisa distinguir o idioma da imagem exibida (ver Bloqueio 3, acima), tornou-se necessário formalizar `language` como um catálogo de referência, em vez de um campo de texto livre em `card_asset` — mesmo padrão já usado para `card_variant_type`/`card_asset_type` (evitar risco de duplicidade como `PT`/`pt`/`pt_BR`/`Português` representando o mesmo idioma).

> **Risco de cross-check sinalizado nas duas revisões anteriores — reforçado como "provavelmente resolvido, não formalmente verificado via Table Editor".** `language` já existe fisicamente entre as 17 tabelas pré-existentes (mesma lista que incluía `card_asset`/`card_asset_type`, ambas divergentes da proposta original quando finalmente inspecionadas). Evidência indireta adicional a favor de que `190` não foi um no-op: além da `192` (que já havia alterado com sucesso a constraint de `code`), a `193` executou com sucesso um `UPDATE public.language ... WHERE code = 'pt-BR'` e um `SELECT ... FROM public.language WHERE code = 'pt-BR'` (para popular `card_asset.language_id`), confirmando que a tabela real aceita e retorna dados consistentes com a estrutura proposta. Ainda assim, nenhuma inspeção direta via Table Editor foi mostrada — recomendação de cautela mantida, agora com risco mais baixo.

> **Discrepância de numeração — RESOLVIDA.** A sessão pareada reconheceu explicitamente a colisão sinalizada na revisão anterior ("eu sugiro utilizar 193, e não 192, porque a 192 acabou de ser utilizada para o refinamento da constraint da tabela `language`. Assim mantemos a numeração das migrations única e sem reutilização.") — a migration que adiciona `language_id` a `card_asset` foi de fato numerada e executada como `193`, confirmando a suposição já registrada na revisão `0.40`.

> **Divergência CONFIRMADA por nova inspeção real (Fabrício, mesmo dia) — a execução da Query `194` está em dúvida.** A Query `194 - Govern Card Asset Storage Provider` (abaixo) valida programaticamente, via `information_schema.columns`, que `card_asset.storage_provider` existe, e foi relatada como "Executada com sucesso". Uma captura de tela real do Table Editor mostra a tabela `card_asset` com exatamente as mesmas 20 colunas já documentadas na revisão `0.30` — sem `storage_provider`, apenas `storage_bucket_id`. Isso **contradiz diretamente** a premissa de que `194` teria passado por sua própria validação de pré-requisito. **Conclusão provisória**: o mais provável, dado o padrão já observado com `170`/`180`, é que `194` não foi de fato aplicada à tabela real, apesar do relato de sucesso — mas isso não foi confirmado nem explicado por Fabrício. Não presumir que `194` está em vigor. Recomendação registrada (não uma decisão unilateral): confirmar com Fabrício contra qual projeto/schema Supabase a Query `194` foi executada antes de prosseguir com `880` ou qualquer nova alteração de `card_asset`.
...
## Query 193 — Add Language to Card Asset (RELATADO COMO EXECUTADO — coluna `language_id` já existia antes; constraints não confirmadas por inspeção)

**Migration estrutural mais importante deste bloco** — integra `language` a `card_asset`, resolvendo a discrepância de numeração já documentada (era esperada como `192`, foi de fato executada como `193`). Passos, todos confirmados via `RAISE EXCEPTION` de pré-requisito + blocos `DO $$` idempotentes: (1) valida pré-requisitos (`card_asset`/`language` existem, `pt-BR` cadastrado); (2) adiciona `language_id UUID` (inicialmente nulo); (3) migra registros existentes para `pt-BR` (classificação retroativa — qualquer Card Asset anterior à introdução do idioma é tratado como português do Brasil); (4) torna `language_id NOT NULL`; (5) cria a FK `fk_card_asset_language`; (6) remove, por introspecção de `pg_constraint`, a antiga `UNIQUE` exata em `card_id`+`asset_type_id`+`asset_order` (sem depender de conhecer o nome real da constraint); (7) remove, por introspecção de `pg_index`, o antigo índice único parcial de ativo principal por `card_id`+`asset_type_id`; (8) cria `uq_card_asset_card_type_language_order` — `UNIQUE (card_id, asset_type_id, language_id, asset_order)`; (9) cria `ux_card_asset_primary_per_card_type_language` — índice único parcial `(card_id, asset_type_id, language_id) WHERE is_primary = TRUE`; (10)-(11) índices de suporte para consulta por idioma; (12) comentários. Confirmado executado por Fabrício ("Executada com sucesso"). Arquivo escrito em `database/migrations/193_add_language_to_card_asset.sql`.

**Resultado — ressalva importante adicionada nesta revisão**: uma nova inspeção real via Table Editor (mesmo dia, ver "Status", acima) mostrou `card_asset` com as mesmas 20 colunas já conhecidas desde a revisão `0.30`, incluindo `language_id` — que **já existia antes desta Query ser sequer escrita**. Ou seja, o passo `ADD COLUMN IF NOT EXISTS language_id` não fez nada de novo. As mudanças de constraint (`uq_card_asset_card_type_language_order`, `ux_card_asset_primary_per_card_type_language`, `fk_card_asset_language`) não aparecem em uma listagem de colunas, então não são nem confirmadas nem refutadas por esta captura — mas, dado que a Query irmã `194` (abaixo) demonstravelmente não teve o efeito relatado, há motivo razoável para tratar as constraints de `193` com a mesma cautela até uma inspeção específica (ex.: `\d card_asset` ou a aba de constraints do Table Editor).

## Query 194 — Govern Card Asset Storage Provider (RELATADO COMO EXECUTADO — evidência real de Table Editor CONTRADIZ a execução)

Proposta pela sessão pareada como uma última melhoria antes da `880`: em vez de criar uma nova entidade `asset_source` (ideia já cogitada e descartada anteriormente por escopo), formaliza `storage_provider` como um enumerador governado por `CHECK`, em vez de texto livre. Passos: (1) valida pré-requisitos, incluindo a existência da própria coluna `storage_provider` — ver a divergência sinalizada acima, "Status"; (2) normaliza valores existentes (aliases como `SUPABASE STORAGE`→`SUPABASE`, `AWS S3`/`AMAZON S3`→`S3`, `CLOUDFLARE R2`→`R2`, `LOCAL STORAGE`→`LOCAL`, `URL`/`EXTERNAL URL`→`EXTERNAL`; valores nulos/vazios classificados como `EXTERNAL` quando há `external_url`, senão `LOCAL`); (3) valida que não sobrou nenhum valor fora do enumerador e que a localização é compatível antes de criar as constraints (evita quebrar em runtime); (4) torna `storage_provider NOT NULL`; (5) cria `ck_card_asset_storage_provider` — `CHECK (storage_provider IN ('SUPABASE','S3','R2','LOCAL','EXTERNAL'))`; (6) cria `ck_card_asset_storage_provider_location` — `EXTERNAL` exige `external_url`, os demais exigem `storage_path`; (7) comentários. Confirmado executado por Fabrício ("Executada sem intercorrências"). Arquivo escrito em `database/migrations/194_govern_card_asset_storage_provider.sql`.

**Provedores que a Query pretendia homologar** (ampliando a discussão original, que citava apenas `SUPABASE`/`EXTERNAL`): `SUPABASE`, `S3`, `R2`, `LOCAL`, `EXTERNAL`. **Mas a evidência real disponível (Table Editor, mesmo dia) mostra `card_asset` sem a coluna `storage_provider`** — a própria Query `194` deveria ter abortado com `RAISE EXCEPTION` ao validar esse pré-requisito, o que não é compatível com o relato "Executada com sucesso, Executada sem intercorrências." **Não presumir que este enumerador está em vigor no banco real.**

## Impacto em `card_asset` — Pretendido por `193`/`194` (execução real não confirmada por inspeção)

`card_asset` teria ganhado, segundo o texto das Queries: `language_id UUID NOT NULL` (FK `fk_card_asset_language`, coluna que na prática já existia antes de `193`); unicidade revisada de `card_id`+`asset_type_id`+`asset_order` para `card_id`+`asset_type_id`+`language_id`+`asset_order` (`uq_card_asset_card_type_language_order`); ativo principal por `card_id`+`asset_type_id`+`language_id` (`ux_card_asset_primary_per_card_type_language`); `storage_provider` obrigatório e restrito a `SUPABASE`/`S3`/`R2`/`LOCAL`/`EXTERNAL` (`ck_card_asset_storage_provider`); compatibilidade de localização por provedor (`ck_card_asset_storage_provider_location`). Isso permitiria, por exemplo, que a mesma Card tivesse `CARD_FRONT` + `ARTWORK`, cada um em `pt-BR` e em `en`, cada combinação com seu próprio ativo principal — exatamente o cenário ilustrado pela sessão pareada com a Card `Rufflet` (ME2.5-173). **Mas, dado que `194` demonstravelmente não teve o efeito relatado na coluna que ela mesma valida como pré-requisito, este parágrafo descreve a intenção documentada, não um fato confirmado sobre o banco real.**

**Avaliação da sessão pareada, registrada como opinião histórica do momento em que foi feita — já contestada por evidência posterior.** A sessão pareada declarou que a camada Card Asset "atingiu um nível de maturidade muito bom" (governança de tipo, idioma, provedor de armazenamento, localização, ativo principal por idioma, ordenação, integridade, triggers, RLS, constraints de negócio) antes de qualquer registro real ser inserido pela `880`. Essa avaliação **não é confiável no estado atual da documentação** — pelo menos a parte de governança de `storage_provider` não está confirmada como realmente aplicada.
...
193 - Add Language to Card Asset         (RELATADO EXECUTADO, coluna language_id já existia antes — constraints não confirmadas por inspeção; database/migrations/193_add_language_to_card_asset.sql)
194 - Govern Card Asset Storage Provider (RELATADO EXECUTADO, CONTRADITO por Table Editor real (mesmo dia) — coluna storage_provider ausente; database/migrations/194_govern_card_asset_storage_provider.sql)
890 - Seed Language                      (CONFIRMADO EXECUTADO — database/seeds/890_seed_language.sql; pt-BR/en, tabela independente de card_asset)

880 - Seed Card Asset                    (bloqueada por fonte oficial de imagens, identificador externo E confirmação de que 193/194 realmente alteraram card_asset, ver "Query 880", acima)
980 - Validate Card Asset                (precisará ser reescrita após 880 e após confirmação real de 193/194, mesmo padrão de evolução já usado em 960 v1.0→v2.0)
```
