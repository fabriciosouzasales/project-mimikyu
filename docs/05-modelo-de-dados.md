...
880 - Seed Card Asset                    (planejada — escopo confirmado: apenas CARD_FRONT, card_id direto; camada estrutural 100% pronta; bloqueada apenas pela fonte oficial de imagens e pelo pipeline de importação automatizado, ver "Query 880", abaixo)
```
...
**Bloqueio 5 (novo) — a `880` não será um `INSERT` estático de URLs externas; será um pipeline de importação automatizado, ainda não desenhado em SQL, com risco crítico de duplicar infraestrutura pré-existente já identificada nesta mesma revisão.** Ver a nova seção "Arquitetura de Importação de Ativos", logo abaixo, para o desenho completo e o risco sinalizado. Este bloqueio absorve e substitui os antigos Bloqueios 1 (fonte oficial de imagens) e 2 (identificador externo) — que deixam de ser tratados isoladamente, pois a solução conceitual proposta (catálogo de fontes + mapeamento de identificadores externos) os endereça diretamente, ainda que nenhuma SQL tenha sido escrita para isso.
...
> **Risco 2 (revisão `0.43`), AINDA NÃO RESOLVIDO: a convenção de caminho final não contempla a dimensão de idioma.** Ao evoluir de "um bucket com subpastas por tipo e idioma" para "um bucket por tipo de ativo" (`card-front`/`artwork`/`card-back`), os exemplos finais mostrados (`ME1/001.png`) seguem sem incluir o idioma no caminho. Como `card_asset` distingue ativos por `language_id` (Bloqueio 3, resolvido), dois registros do mesmo Card Asset Type para a mesma Card em idiomas diferentes (ex.: `CARD_FRONT` em `pt-BR` e em `en`) apontariam para o mesmo `storage_bucket`+`storage_path` (`card-front`/`ME1/001.png`), o que é uma colisão. `197` não resolveu este risco — apenas reorganizou onde a localização é validada (via `storage_bucket`, não mais `storage_provider` solto). **Continua precisando de decisão explícita de Fabrício antes de qualquer carga real**: reintroduzir o idioma no caminho (ex.: `pt-BR/ME1/001.png`), usar um sufixo no nome do arquivo (ex.: `ME1/001-pt-BR.png`), ou criar buckets por idioma também. A nova convenção de caminho discutida no planejamento do pipeline de importação (ver "Arquitetura de Importação de Ativos", abaixo) já reintroduz `{language-code}` no caminho — o que sugere este risco pode estar sendo resolvido na prática, mas isso **ainda não foi declarado explicitamente** como a resposta ao Risco 2; recomenda-se que Fabrício confirme.
...
## Arquitetura de Importação de Ativos — Planejamento Conceitual (NENHUMA SQL executada) — ⚠️ Risco crítico de duplicação com infraestrutura pré-existente
...
**Componentes conceituais propostos para o pipeline** (nenhum criado como tabela ainda):
- `asset_source` — catálogo de fontes externas (`POKEMON_TCG_API`, `TCGDEX`, `MANUAL`), com `code`/`name`/`base_url`/`is_active`/`priority`.
- `card_external_reference` — mapeamento entre uma Card do Project Mimikyu e sua identidade em uma fonte externa (`card_id`, `source_id`, `external_card_id`, `external_set_id`, `source_number`, `metadata`), para evitar correspondências frágeis baseadas apenas no nome do Pokémon.
- `asset_import_job` — controla uma execução completa de importação por coleção (`collection_id`, `source_id`, `status`, `total_items`, `processed_items`, `successful_items`, `failed_items`, `started_at`, `finished_at`).
- `asset_import_item` — estado por Card dentro de um job (`job_id`, `card_id`, `source_url`, `storage_path`, `status`, `attempt_count`, `checksum`, `error_message`, `processed_at`).
- Um worker (Edge Function em TypeScript) responsável por buscar → baixar → validar (é realmente uma imagem?) → calcular checksum → gerar caminho padronizado → verificar se o objeto já existe → enviar ao Supabase Storage → criar/atualizar `card_asset` → registrar resultado.

**Convenção de caminho revisada nesta discussão** (distinta da convenção anterior, que havia removido o idioma — ver Risco 2, acima): `pokemon/{collection-code}/{card-number}/{language-code}/front.png`, simplificada em seguida (já que o bucket físico, ex. `card-front`, já representa parte da localização) para `pokemon/{collection-code}/{card-number}/{language-code}.png`. **Esta versão reintroduz o idioma no caminho** — potencialmente a resposta ao Risco 2, mas isso não foi declarado como tal na conversa; precisa de confirmação explícita.
...
> **⚠️ RISCO CRÍTICO, sinalizado nesta revisão por cross-check — não levantado por Fabrício nem pela sessão pareada: os componentes propostos (`asset_source`, e por extensão o par `asset_import_job`/`asset_import_item`) colidem diretamente com infraestrutura física já documentada como pré-existente.** `docs/06-pipeline-importacao.md` já registra, desde um ciclo muito anterior a este, que o banco físico possui `asset_source`, `asset_import_run` e `asset_import_failure` como tabelas pré-existentes (junto com `card_asset`/`card_asset_type`/`storage_bucket`) — "ainda não foram documentadas em nível conceitual, previsto para um ciclo futuro". `card_external_reference` e `card_set_external_reference` também constam nessa mesma lista de infraestrutura pré-existente, nunca inspecionada. Ou seja:
>
> - `asset_source`, proposto nesta conversa como uma tabela nova a ser criada, **já existe** com esse nome exato.
> - `asset_import_job`/`asset_import_item`, propostos como novos, têm nomes diferentes mas propósito quase idêntico a `asset_import_run`/`asset_import_failure`, que **já existem**.
> - `card_external_reference`, proposto como mapeamento de identificadores externos, **já existe**.
>
> Este é exatamente o mesmo padrão de erro já cometido três vezes neste projeto — propor uma estrutura nova sem checar a estrutura física real primeiro: `card_asset`/`card_asset_type` (batches 29-30, corrigido após inspeção real), `storage_provider` em `card_asset` (revisão `0.42`, corrigido) e a primeira proposta de `storage_bucket` como coluna de texto solta (revisão `0.43`, corrigida antes de qualquer SQL ser escrita — ver "Arquitetura de Armazenamento", acima). **Recomendação forte, não uma decisão**: antes de escrever qualquer SQL para o pipeline de importação, inspecionar via Table Editor a estrutura real de `asset_source`, `asset_import_run`, `asset_import_failure`, `card_external_reference` e `card_set_external_reference` — é bastante provável que essas tabelas já implementem grande parte (ou até a totalidade) do que está sendo desenhado do zero nesta conversa, e a estrutura conceitual proposta aqui deve ser adaptada para reaproveitá-las, não substituí-las.
...
880 - Seed Card Asset                    (bloqueada pela fonte oficial de imagens e pelo pipeline de importação automatizado, ver "Arquitetura de Importação de Ativos", acima)
```
