...
Segundo ciclo vertical, construído sobre os componentes já extraídos no fechamento de `Game` (`useAdminListState`, `AdminToolbar`, `SuccessBanner`). Sem seleção em massa/exclusão — `ADR-023` só recebeu a emenda de exclusão real para `Game`; `Expansion` continua apenas com `create`/`update`.
...
**Frontend (`/catalogo/expansoes`, IMPLEMENTADO — validação pela interface pendente)**: `ExpansoesTable` (`web/components/catalogo/expansoes-table.tsx`) reconstrói a tela sobre `useAdminListState`/`AdminToolbar`/`SuccessBanner` — mesmo padrão de `JogosTable`, mas deliberadamente sem `BulkSelectionBar`/`ConfirmDeleteBar` (nenhuma exclusão prevista para Expansion). Cadastro inline traz um seletor de Jogo (`<select>`, populado por `getGameOptions()`, nova função em `web/lib/catalogo/queries.ts`), pré-selecionado quando a tela chega filtrada por `?game=CODE`; edição mantém `game_id`/`code` desabilitados (imutáveis). Colunas Código/Jogo/Ordem/Card Sets/Criado em/Atualizado em centralizadas, mesmo padrão visual fechado em `Game`. `getExpansoes()`/`ExpansaoRow` ganharam `createdAt`/`updatedAt`/`gameId`; duas novas Server Actions em `web/app/catalogo/expansoes/actions.ts` (`createExpansion`/`updateExpansion`). `tsc --noEmit` confirmado limpo para todo o código novo (os dois erros pré-existentes em `lib/supabase/middleware.ts`/`server.ts` não fazem parte deste incremento).

**Pendência explícita**: teste da tela pela própria interface (listar/cadastrar/editar, inclusive o filtro `?game=`) depende do ambiente de Fabrício — sandbox desta sessão sem acesso a `npm`/CDN.
