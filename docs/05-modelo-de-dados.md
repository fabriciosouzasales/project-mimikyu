...
### Validação — Query `940`, versão desatualizada precisa ser reescrita antes de reexecutar

A versão de `940` recebida no ciclo anterior (18 subconsultas genéricas, descritas na revisão 1.1 acima) foi originalmente redigida para uma tabela `card` vazia. Com as 859 Cards agora carregadas, a mesma Query já pôde ser reexecutada uma vez sobre dados reais — resultados relatados por Fabrício: os cinco Sets aparecem com status `COMPLETE` e as quantidades corretas (188/130/295/124/122, total 859); nenhuma duplicidade de `collector_number`/`collector_order`; nenhuma inconsistência de Game entre Rarity/Card Category; nenhum registro com nome/total/ordem inválidos; nenhuma posição editorial ausente; os dois triggers presentes; RLS habilitado. **Porém a sessão pareada observou uma lacuna**: a versão atual de `940` é genérica — valida a estrutura, mas não contém uma seção canônica explícita comparando contra as quantidades esperadas dos cinco Sets e o total de 859 (equivalente ao que a Query `930` de Rarity já faz com sua CTE de dados canônicos esperados). Fabrício confirmou a intenção de atualizar `940` para incorporar essas validações específicas ("Vamos atualizar query 940 então") — **esta atualização ainda não foi escrita nem executada**, permanece como próximo passo. Por isso `940` **ainda não foi copiada para `database/`** — o texto atual está desatualizado frente ao que será a versão canônica real.
...
- [x] arquivos `140`/`141`/`840` copiados para `database/`;
- [ ] Query `940` reescrita com seção canônica explícita (5 Sets + total 859) — decidido, ainda não escrito;
- [ ] `940` reexecutada com a versão atualizada e copiada para `database/validations/`;
...
940 - Validate Card         (v1.0 executada uma vez sobre dados reais; reescrita com seção canônica específica pendente)
```
