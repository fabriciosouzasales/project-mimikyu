...
Três divergências em relação à proposta, nenhuma resolvida:

1. **Sem `card_variant_id`** — a tabela física não associa um Card Asset a uma Card Variant específica, contradizendo o exemplo de "imagem do Reverse Holo" da proposta original. Não presumido — aguardando esclarecimento de Fabrício.
2. **`storage_bucket_id`** (FK para `storage_bucket`) no lugar de `storage_provider` (texto livre) — mais estruturado que a proposta.
3. **`language_id`** (presumivelmente FK para `language`), ausente da proposta — sugere Card Asset pode variar por idioma, possivelmente ligado à ainda-não-documentada Card Translation. Não presumido — aguardando esclarecimento de Fabrício.

## Queries Planejadas (não executadas)
