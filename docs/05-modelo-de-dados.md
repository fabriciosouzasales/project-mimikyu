...
880 - Seed Card Asset                    (planejada — escopo confirmado: apenas CARD_FRONT, card_id direto; camada estrutural 100% pronta; bloqueada até que a camada de execução de importação [asset_import_run/asset_import_failure, arquitetura revisada, SQL ainda não escrita] também esteja pronta, ver "Query 880"/"Bloqueio 5", abaixo)
```
...
**Bloqueio 5 — em andamento, duas de quatro camadas concluídas nesta revisão.** Fabrício corrigiu explicitamente a sequência antes proposta ("Não seguiremos agora para: 880 – Seed Card Asset"): o próximo passo é estrutural, construído em camadas, uma de cada vez, com pacote técnico completo antes de avançar — (1) **Asset Source** (`200`/`201`/`900`/`985` — CONFIRMADOS EXECUTADOS, ver seção "Asset Source", abaixo); (2) **Card External Reference** (`210`/`211`/`990` — CONFIRMADOS EXECUTADOS nesta revisão, ver seção "Card External Reference", abaixo; a Seed `910` foi deliberadamente **não criada** — ver nota abaixo); (3) **camada de execução de importação — arquitetura revisada nesta revisão**, de um modelo `asset_import_job`/`asset_import_item` (planejado na revisão `0.46`, nunca escrito em SQL) para um modelo híbrido **`asset_import_run`/`asset_import_failure`** (um registro por execução + registros apenas para falhas, não um registro por Card processada com sucesso) — SQL ainda não escrita, ver "Arquitetura de Importação de Ativos", abaixo; somente então (4) desenvolvimento da Edge Function e um piloto controlado com uma coleção pequena; só depois disso, a carga em escala real de `880`. Este bloqueio absorve e substitui os antigos Bloqueios 1 (fonte oficial de imagens) e 2 (identificador externo).
...
> **⚠️ Ressalva de numeração e nomenclatura, sinalizada nesta revisão, NÃO resolvida unilateralmente — dupla atenção necessária antes de escrever `asset_import_run`/`asset_import_failure`.** Primeiro, **nomenclatura**: `asset_import_run` e `asset_import_failure` são exatamente os dois nomes que `docs/06-pipeline-importacao.md` já registra, desde um ciclo muito anterior, como parte das 17 tabelas físicas pré-existentes do projeto (junto com `asset_source`, que a revisão `0.46` provou empiricamente **não** pré-existir, via guarda defensiva da Query `200`). Como a sessão pareada chegou a esses dois nomes por raciocínio arquitetural independente (não por inspeção da lista pré-existente), é plausível que seja apenas uma convergência de bom senso de nomenclatura — mas, dado o histórico deste projeto (mesmo padrão de risco já visto quatro vezes), a Query de criação de `asset_import_run` deve repetir a mesma guarda defensiva usada em `200` (`IF to_regclass(...) IS NOT NULL THEN RAISE EXCEPTION`) antes de criar a tabela, para o mesmo teste empírico ficar registrado. Segundo, **numeração**: o plano da revisão `0.46` reservava `220`/`221`/`222`/`920`/`995` para esta camada (então chamada Asset Import Job/Item) — como o modelo mudou de Job/Item para Run/Failure, esses números precisam ser reavaliados de qualquer forma; adicionalmente, **`920` teria colidido** com a já existente e já executada `920 - Validate Card Set` (Create `120` + 800, ver seção Set) — colisão detectada nesta revisão, antes de qualquer SQL ser escrita com esse número, o que a torna a primeira colisão deste projeto **evitada preventivamente**, e não apenas descoberta depois do fato (diferente das colisões de `970`/`900` em revisões anteriores). Recomendação: escolher números novos dentro da faixa de import (ex. `220`/`221` para Create/Trigger de `asset_import_run`, `230`/`231` para `asset_import_failure`, com Seed/Validate em faixa dedicada ainda a ser definida) e confirmar contra o catálogo real de `database/` antes de gravar qualquer arquivo.
...
880 - Seed Card Asset                    (bloqueada até a camada de execução de importação [asset_import_run/asset_import_failure, arquitetura revisada nesta revisão, SQL ainda não escrita] estar pronta e um piloto controlado ser executado, ver "Arquitetura de Importação de Ativos", acima)
```
