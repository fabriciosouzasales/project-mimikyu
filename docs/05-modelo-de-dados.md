...
2. **`storage_bucket_id`** (FK para `storage_bucket`) no lugar de `storage_provider` (texto livre) — **divergência revisitada, situação agora contraditória.** A Query `194 - Govern Card Asset Storage Provider` (ver seção "Language", abaixo) validou programaticamente que `storage_provider` existe em `card_asset` e executou com sucesso, o que contradiz este item tal como registrado na inspeção original (revisão `0.30`). Não presumir qual das duas é a estrutura real vigente — recomenda-se nova inspeção via Table Editor antes de qualquer nova migration sobre `card_asset`.
...
**Ativo principal e unicidade — EXECUTADAS (Query `193`, ver "Language", abaixo), incluindo `language_id`.** Regra anterior (sem dimensão de idioma): no máximo um `is_primary = TRUE` por `card_id` + `asset_type_id`, via índice único parcial `uq_card_asset_one_primary`; unicidade lógica por `card_id` + `asset_type_id` + `asset_order`. **Regra em vigor, confirmada executada**: cada combinação `card_id` + `asset_type_id` + `language_id` pode ter seu próprio ativo principal — no máximo um `is_primary = TRUE` por `card_id` + `asset_type_id` + `language_id` (índice `ux_card_asset_primary_per_card_type_language`); unicidade lógica é `card_id` + `asset_type_id` + `language_id` + `asset_order` (constraint `uq_card_asset_card_type_language_order`). Adicionalmente, `storage_provider` passou a ser governado por enumerador (`SUPABASE`/`S3`/`R2`/`LOCAL`/`EXTERNAL`) via Query `194`, com compatibilidade de localização obrigatória por provedor. Isso permite que a mesma Card tenha, por exemplo, um `CARD_FRONT` principal em português (`asset_order = 1`, `is_primary = TRUE`) e outro `CARD_FRONT` principal em inglês (`asset_order = 1`, `is_primary = TRUE`), sem conflito.
...
193 - Add Language to Card Asset         (EXECUTADA — resolve a discrepância de numeração, ver "Language")
194 - Govern Card Asset Storage Provider (EXECUTADA — gera nova divergência a esclarecer sobre storage_provider, ver "Estrutura Física Real"/"Language")
890 - Seed Language                      (EXECUTADA — pt-BR/en)

880 - Seed Card Asset                    (planejada — escopo confirmado: apenas CARD_FRONT, card_id direto; bloqueios estruturais resolvidos, ainda bloqueada pela fonte oficial de imagens e pelo identificador externo, ver "Query 880", abaixo)
```
...
**Bloqueio 3 — dimensão de idioma — RESOLVIDO E EXECUTADO.** Ao comparar duas imagens reais da mesma Card (`Rufflet`, `173/217`, ME2.5) — uma em português ("Rufflet do Lauro") e outra em inglês ("Larry's Rufflet") — Fabrício identificou que ambas representam a mesma Card, o mesmo Card Asset Type (`CARD_FRONT`), mas são duas **representações linguísticas distintas do mesmo ativo digital** — não dois Card Assets Types, não duas Cards, não duas Card Variants. Isso disparou a decisão de adicionar `language_id` como dimensão de `card_asset` (ver "Estrutura Física Real", acima, e a seção "Language", abaixo), formalizada e **confirmada executada** via `190`/`191`/`192`/`193`/`890`. Uma governança adicional de `storage_provider` (Query `194`) foi executada no mesmo ciclo, também estrutural. A `880` não está mais bloqueada por esses itens — permanece bloqueada apenas pelos Bloqueios 1 e 2 (fonte de imagens e identificador externo, acima).
...
> **Nova divergência sinalizada, ainda não explicada — cross-check obrigatório.** A Query `194 - Govern Card Asset Storage Provider` (abaixo) valida programaticamente, via `information_schema.columns`, que `card_asset.storage_provider` existe — e a execução teve sucesso, sem levantar a exceção que acusaria sua ausência. Isso **contradiz diretamente** a "Estrutura Física Real" documentada na revisão `0.30` (item 2, seção Card Asset Type/Card Asset, acima), que registrava `storage_bucket_id` (FK) "no lugar de" `storage_provider` (texto livre) na tabela real inspecionada à época. Duas hipóteses possíveis, nenhuma confirmada: (a) a estrutura real mudou entre a inspeção da revisão `30` e agora, por algum caminho não documentado neste repositório; (b) a inspeção da revisão `30` já estava desatualizada ou incompleta. **Não presumir qual estrutura está correta agora** — recomenda-se pedir a Fabrício uma nova inspeção via Table Editor de `card_asset` antes de qualquer nova migration estrutural sobre essa tabela.
...
**Resultado**: a regra de unicidade/ativo principal de `card_asset` já documentada na seção "Card Asset" (acima) como planejada está agora **efetivamente em vigor**, com os nomes reais das constraints/índices registrados neste parágrafo — atualizar a seção "Regras adicionais de `card_asset`" para referenciar estes nomes reais em vez de tratá-los como discussão.

## Query 194 — Govern Card Asset Storage Provider (CONFIRMADO EXECUTADO, gera nova divergência a esclarecer)
...
**Provedores homologados, ampliados em relação à discussão original** (que citava apenas `SUPABASE`/`EXTERNAL`): `SUPABASE`, `S3`, `R2`, `LOCAL`, `EXTERNAL`.

## Impacto em `card_asset` — Consolidado (após `193`/`194`)

`card_asset` ganhou, nesta revisão: `language_id UUID NOT NULL` (FK `fk_card_asset_language`); unicidade revisada de `card_id`+`asset_type_id`+`asset_order` para `card_id`+`asset_type_id`+`language_id`+`asset_order` (`uq_card_asset_card_type_language_order`); ativo principal por `card_id`+`asset_type_id`+`language_id` (`ux_card_asset_primary_per_card_type_language`); `storage_provider` obrigatório e restrito a `SUPABASE`/`S3`/`R2`/`LOCAL`/`EXTERNAL` (`ck_card_asset_storage_provider`); compatibilidade de localização por provedor (`ck_card_asset_storage_provider_location`). Isso permite, por exemplo, que a mesma Card tenha `CARD_FRONT` + `ARTWORK`, cada um em `pt-BR` e em `en`, cada combinação com seu próprio ativo principal — exatamente o cenário ilustrado pela sessão pareada com a Card `Rufflet` (ME2.5-173).

**Avaliação da sessão pareada, registrada como opinião, não como fato verificado por este documento**: a camada Card Asset "atingiu um nível de maturidade muito bom" — governança de tipo (`Card Asset Type`), idioma (`Language`), provedor de armazenamento (`Storage Provider`), localização, ativo principal por idioma, ordenação, integridade Game/Card/Asset, triggers, RLS e constraints de negócio, tudo antes de qualquer registro real ser inserido pela `880`.
...
193 - Add Language to Card Asset         (CONFIRMADO EXECUTADO — database/migrations/193_add_language_to_card_asset.sql; resolve a discrepância de numeração)
194 - Govern Card Asset Storage Provider (CONFIRMADO EXECUTADO — database/migrations/194_govern_card_asset_storage_provider.sql; gera nova divergência a esclarecer, ver "Status")
890 - Seed Language                      (CONFIRMADO EXECUTADO — database/seeds/890_seed_language.sql; pt-BR/en)

880 - Seed Card Asset                    (bloqueios estruturais resolvidos; ainda bloqueada por fonte oficial de imagens e identificador externo, ver "Query 880", acima)
980 - Validate Card Asset                (precisará ser reescrita após 880, mesmo padrão de evolução já usado em 960 v1.0→v2.0; validará também language_id e storage_provider)
```
