...
880 - Seed Card Asset                    (planejada — escopo confirmado: apenas CARD_FRONT, card_id direto; camada estrutural 100% pronta; bloqueada até que Card External Reference e Asset Import Job/Item também estejam prontas, ver "Query 880"/"Bloqueio 5", abaixo)
```
...
**Bloqueio 5 — em andamento, com correção de rota nesta revisão: `880` não será a próxima Query executada.** Fabrício corrigiu explicitamente a sequência antes proposta ("Não seguiremos agora para: 880 – Seed Card Asset"): o próximo passo é estrutural, construído em três camadas, uma de cada vez, com Create/Trigger/Seed/Validate completos antes de avançar — (1) **Asset Source** (`200`/`201`/`900`/`985` — CONFIRMADOS EXECUTADOS nesta revisão, ver seção "Asset Source", abaixo); (2) **Card External Reference** (`210`/`211`/`910`/`990` — planejada, SQL ainda não recebida); (3) **Asset Import Job/Item** (`220`/`221`/`222`/`920`/`995` — planejada, SQL ainda não recebida); somente então (4) desenvolvimento da Edge Function e um piloto controlado com uma coleção pequena; só depois disso, a carga em escala real de `880`. Ver a seção "Arquitetura de Importação de Ativos", logo abaixo, para o desenho completo e a correção do risco de duplicação sinalizado na revisão anterior. Este bloqueio absorve e substitui os antigos Bloqueios 1 (fonte oficial de imagens) e 2 (identificador externo).
...
**Componentes do pipeline, por camada — ordem de construção corrigida nesta revisão** (Fabrício: *"Não seguiremos agora para: 880 – Seed Card Asset. O próximo passo será estrutural [...]"*):
1. **Asset Source** — catálogo de fontes externas. **CRIADO, SEMEADO E VALIDADO nesta revisão** (`200`/`201`/`900`/`985`, todos confirmados executados) — ver seção "Asset Source", abaixo, para o modelo físico completo e a execução real.
2. **Card External Reference** (`210`/`211`/`910`/`990`) — mapeamento entre uma Card do Project Mimikyu e sua identidade em uma fonte externa (`card_id`, `source_id`, `external_card_id`, `external_set_id`, `source_number`, `metadata`), para evitar correspondências frágeis baseadas apenas no nome do Pokémon. **Planejada, SQL ainda não recebida.**
3. **Asset Import Job/Item** (`220`/`221`/`222`/`920`/`995`) — `asset_import_job` controla uma execução completa de importação por coleção (`collection_id`, `source_id`, `status`, `total_items`, `processed_items`, `successful_items`, `failed_items`, `started_at`, `finished_at`); `asset_import_item` guarda o estado por Card dentro de um job (`job_id`, `card_id`, `source_url`, `storage_path`, `status`, `attempt_count`, `checksum`, `error_message`, `processed_at`). **Planejada, SQL ainda não recebida.**
...
880 - Seed Card Asset                    (bloqueada até Card External Reference [210/211/910/990] e Asset Import Job/Item [220/221/222/920/995] estarem prontas e um piloto controlado ser executado, ver "Arquitetura de Importação de Ativos", acima)
```
...
210 - Create Card External Reference       (planejada — SQL ainda não recebida)
211 - Card External Reference Triggers     (planejada)
910 - Seed Card External Reference         (planejada)
990 - Validate Card External Reference     (planejada)

220 - Create Asset Import Job              (planejada)
221 - Create Asset Import Item             (planejada)
222 - Asset Import Triggers                (planejada)
920 - Seed/Test Import Job                 (planejada)
995 - Validate Asset Import Architecture   (planejada)
```
