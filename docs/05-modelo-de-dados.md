...
**Plano real, ainda NÃO executado nesta revisão** (confirmar em ciclo futuro antes de tratar como concluído): recriar o Set promocional como `card_set.code = 'MEP'` (não mais `ME0`), cadastrar `card_set_external_reference` (`MEP` → `mep`), importar as `card`/`card_variant` das cartas promocionais, gerar `card_external_reference` via o identificador `mep`, e então importar as imagens (inglês primeiro, depois português) pelo pipeline já existente (ver `06-pipeline-importacao.md`, "Guia Operacional").

**Questão real, explicitamente aberta, SEM decisão** (Fabrício optou por investigar antes de qualquer mudança no banco): se a entidade `Expansion` está de fato modelando o mesmo conceito que aparece nos catálogos oficiais como "Series"/"Era" (ex.: "Mega Evolution" contendo `ME01`-`ME04` e `MEP` como Sets irmãos) — ou se há uma camada conceitual faltando entre Expansion e Set. **Nenhuma alteração de banco foi proposta ou decidida** — Fabrício declarou explicitamente que quer pesquisar a fundo antes de mexer em qualquer coisa.
