...
**Card Asset Type: pacote técnico CONCLUÍDO E EXECUTADO** (`170`/`171`/`870`/`970`, ver "SQL confirmada" abaixo). **Card Asset: estrutura física já existente no Supabase; `180`/`181`/`980` (v1.1) confirmados executados. Governança de idioma e provedor de armazenamento (`193`/`194`) — CONFIRMADAS EXECUTADAS por Fabrício ("Houve execução real de 193 e 194."), resolvendo a dúvida levantada na revisão anterior a partir da captura de Table Editor que aparentava contradizer `194`.** A arquitetura de armazenamento está, ainda assim, em evolução: uma nova entidade de catálogo `storage_bucket` foi criada e homologada nesta revisão (`195`/`196`/`895`/`975`), e uma migration `197` (planejada, não executada) removerá `storage_provider` de `card_asset` em favor de `storage_bucket_id` — ver "Arquitetura de Armazenamento", abaixo. Nomenclatura final "Card Asset"/"Card Asset Type" (não "Card Image", nome inicialmente cogitado e depois generalizado — ver `04-domain-model.md` para o raciocínio completo, incluindo o exemplo Bulbasaur/Standard/Reverse Holo que motivou a generalização).
...
2. **`storage_bucket_id`** (FK para `storage_bucket`) — coexistia com `storage_provider` (adicionada por `194`, ver seção "Language", abaixo) até esta revisão. **Dúvida da revisão anterior RESOLVIDA**: Fabrício confirmou explicitamente que houve execução real de `193` e `194` ("Houve execução real de 193 e 194."). A captura de Table Editor que motivou a suspeita na revisão `0.42` é tratada como historicamente registrada, mas superada por essa confirmação direta — não presumir mais que `194` falhou. **Nova decisão arquitetural nesta revisão**: `storage_bucket_id` já era, como suspeitado no "Risco 1" da revisão `0.43`, uma FK para uma tabela `storage_bucket` própria (confirmado — ver seção "Storage Bucket", abaixo, criada/homologada nesta revisão via `195`/`196`/`895`/`975`). Como o bucket já carrega seu próprio `storage_provider`, manter `storage_provider` também em `card_asset` é redundante (o mesmo valor, ex. `SUPABASE`, repetido em milhares de linhas). Por isso foi decidido — mas ainda **não executado** — remover `storage_provider` de `card_asset` por meio da migration `197 - Add Storage Bucket to Card Asset` (planejada, ver "Arquitetura de Armazenamento", abaixo), deixando `card_asset` apenas com `storage_bucket_id` + `storage_path`; o provedor passa a ser obtido por `JOIN` com `storage_bucket`.
3. **`language_id`** (FK para `language`) — coluna presente e com propósito explicado (idioma da imagem digital, não tradução editorial nem idioma do exemplar físico — ver seção "Language (Idioma)", abaixo, e "Três Dimensões de Idioma" em `04-domain-model.md`). A coluna já existia fisicamente antes de `193` ser escrita (consistente com a listagem original da revisão `0.30`); o passo `ADD COLUMN IF NOT EXISTS language_id` de `193` foi, portanto, um no-op para a coluna em si. As alterações de constraint/índice de `193` (unicidade e ativo principal por idioma) — agora tratadas como confirmadas, junto com o restante de `193`/`194`, por confirmação direta de Fabrício ("Houve execução real de 193 e 194.").
...
197 - Add Storage Bucket to Card Asset   (PLANEJADA, NÃO EXECUTADA — adiciona storage_bucket_id, migra dados, cria FK, remove storage_provider; ver "Arquitetura de Armazenamento")

880 - Seed Card Asset                    (planejada — escopo confirmado: apenas CARD_FRONT, card_id direto; bloqueada pela fonte oficial de imagens, pelo identificador externo e pela execução de 197, ver "Query 880", abaixo)
```
...
**Bloqueio 4 — coluna `storage_bucket` — RESOLVIDO como entidade de catálogo própria, execução parcial.** A entidade `storage_bucket` foi criada, semeada e homologada nesta revisão (`195`/`196`/`895`/`975`, todas executadas — ver seção "Storage Bucket", abaixo), confirmando o "Risco 1" sinalizado na revisão `0.43`: `card_asset.storage_bucket_id` já era FK para essa tabela pré-existente, e a modelagem correta era mesmo uma entidade de referência (mesmo padrão de `language`/`card_asset_type`), não uma coluna de texto livre. **Pendência remanescente**: a migration `197 - Add Storage Bucket to Card Asset`, que integraria `storage_bucket_id` a `card_asset` e removeria a coluna `storage_provider` (redundante frente ao `storage_provider` já presente em `storage_bucket`), foi **planejada e aprovada por Fabrício ("Vamos em frente"), mas ainda não escrita nem executada**. A `880` permanece bloqueada pelos Bloqueios 1 e 2 (fonte de imagens e identificador externo) e agora também pela execução de `197`, antes de qualquer nova migration ou seed sobre `card_asset`. O "Risco 2" da revisão `0.43` (convenção de caminho sem dimensão de idioma) permanece **não resolvido** — ver "Arquitetura de Armazenamento", abaixo.
...
**Query 197 — Add Storage Bucket to Card Asset (planejada, aprovada por Fabrício "Vamos em frente", NÃO escrita nem executada).** Faria, segundo a proposta discutida: (1) adicionar `storage_bucket_id UUID` a `card_asset`; (2) migrar registros existentes para os buckets corretos; (3) tornar `storage_bucket_id` obrigatório; (4) criar a FK para `storage_bucket`; (5) remover `storage_provider` de `card_asset` (a constraint `ck_card_asset_storage_provider` criada por `194` seria removida junto); (6) atualizar índices; (7) atualizar constraints de compatibilidade de localização (`ck_card_asset_storage_provider_location`, que hoje depende de `storage_provider`, precisará ser reformulada); (8) preparar o terreno para a `880`. **Pré-requisito confirmado nesta revisão**: `895 - Seed Storage Bucket` precisa rodar antes de `197`, para que `card_asset.storage_bucket_id` nunca referencie um bucket inexistente — seguido à risca (`895` executada antes de qualquer proposta de `197`).

> **Risco 2 (revisão `0.43`), ainda NÃO resolvido nesta revisão: a convenção de caminho final não contempla a dimensão de idioma.** Ao evoluir de "um bucket com subpastas por tipo e idioma" para "um bucket por tipo de ativo" (`card-front`/`artwork`/`card-back`), os exemplos finais mostrados (`ME1/001.png`) seguem sem incluir o idioma no caminho. Como `card_asset` distingue ativos por `language_id` (Bloqueio 3, resolvido acima), dois registros do mesmo Card Asset Type para a mesma Card em idiomas diferentes (ex.: `CARD_FRONT` em `pt-BR` e em `en`) apontariam para o mesmo `storage_bucket`+`storage_path` (`card-front`/`ME1/001.png`), o que é uma colisão — não há como os dois arquivos (potencialmente diferentes, ex.: `Rufflet do Lauro` vs. `Larry's Rufflet`) coexistirem sob o mesmo caminho. **Precisa de decisão explícita de Fabrício antes de qualquer carga real**: reintroduzir o idioma no caminho (ex.: `pt-BR/ME1/001.png`), usar um sufixo no nome do arquivo (ex.: `ME1/001-pt-BR.png`), ou criar buckets por idioma também. Recomenda-se resolver este risco antes de escrever `197`, já que `197` é o momento em que `storage_path` passa a ser o único indicador de localização junto ao bucket.
...
**Fluxo de carga recomendado, ainda não iniciado**: (1) criar o(s) bucket(s) físicos no Supabase Storage com os nomes exatos já cadastrados em `895` (`card-front`, `artwork`, `card-back`); (2) enviar algumas imagens de teste; (3) validar o padrão de caminhos na prática, incluindo a resolução do Risco 2 (idioma); (4) executar `197` para integrar `storage_bucket_id` a `card_asset`; (5) inserir os respectivos registros em `card_asset` manualmente para essas poucas Cards; (6) só então automatizar a carga em lote (programa que percorre uma pasta local, envia cada imagem ao Supabase Storage e grava o registro correspondente em `card_asset`) — explicitamente recomendado para evitar popular os 859 registros da `880` antes das imagens realmente existirem (o que deixaria o banco apontando para centenas de caminhos vazios).
...
## Query 194 — Govern Card Asset Storage Provider (CONFIRMADO EXECUTADO — será revertida por `197`)
...
**Vida útil planejada, curta**: a evolução arquitetural desta mesma revisão (ver "Arquitetura de Armazenamento", seção Card Asset Type/Card Asset, acima) decidiu que `storage_provider` é redundante em `card_asset` uma vez que `storage_bucket` (nova entidade, ver seção "Storage Bucket", abaixo) já carrega essa informação por bucket. A migration planejada `197 - Add Storage Bucket to Card Asset` removerá `storage_provider` de `card_asset` — ou seja, esta coluna, embora confirmada executada e correta no momento em que rodou, tem vida útil curta por decisão arquitetural posterior, não por erro de execução.
...
194 - Govern Card Asset Storage Provider (CONFIRMADO EXECUTADO — database/migrations/194_govern_card_asset_storage_provider.sql; será revertida por 197)
...
197 - Add Storage Bucket to Card Asset   (PLANEJADA, NÃO EXECUTADA — remove storage_provider de card_asset, integra storage_bucket_id)

880 - Seed Card Asset                    (bloqueada por fonte oficial de imagens, identificador externo e execução de 197, ver "Query 880", acima)
980 - Validate Card Asset                (precisará ser reescrita após 880 e após 197, mesmo padrão de evolução já usado em 960 v1.0→v2.0)
```
...
197 - Add Storage Bucket to Card Asset   (PLANEJADA, NÃO EXECUTADA — ver seção Card Asset Type/Card Asset, "Arquitetura de Armazenamento")
```
