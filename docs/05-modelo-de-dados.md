...
- [ ] variantes, referências externas de carta e imagens de `MEE`/`MEP` — **pendente**, próximo passo planejado (`MEE` primeiro, como validação do pipeline completo em escala pequena).
...
**Próximo passo planejado (2026-07-24) — extensão a `MEE`/`MEP`, ainda não executada.** Com `MEE`/`MEP` já cadastradas em `card_set` e suas 68 Cards (8+60) já carregadas via `840` v2.2, o próximo passo definido em conjunto com Fabrício é retomar o padrão de Seed por Card Set (em vez de uma única Query consolidada) para as novas variantes editoriais, com uma sequência de letras reorganizada para acompanhar a ordem cronológica real dos Card Sets (`MEE`, `MEP`, `ME1`, `ME2`, `ME2.5`, `ME3`, `ME4`): `860A - Seed Card Variant MEE`, `860B - Seed Card Variant MEP`, `860C - Seed Card Variant ME1` (a antiga `860A`, renomeada para manter a ordem cronológica), `860D - ME2`, `860E - ME2.5`, `860F - ME3`, `860G - ME4`, seguida de `960 - Validate Card Variant`. Também combinado, mas não implementado neste ciclo: nomear os arquivos com o conteúdo explícito no próprio código (`860A - Seed Card Variant MEE`, não apenas `860A`), para facilitar manutenção futura sem depender de memorizar a correspondência letra→Set. Ordem de execução acordada: `MEE` primeiro (8 Cards, valida o pipeline em escala pequena), depois `MEP` (60 Cards), e só então a renomeação/reorganização dos arquivos `860C`-`860G` existentes. **Nenhum arquivo `860A`-`860G` foi criado ou renomeado ainda** — este parágrafo registra o plano acordado, não uma execução.
